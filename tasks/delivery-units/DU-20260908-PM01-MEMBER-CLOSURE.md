...
> 认领提交：`SELF`
> 修改边界：`pms-module-project/src/main/java/cn/iocoder/yudao/module/pms/project/service/projectscope/**;pms-module-project/src/main/java/cn/iocoder/yudao/module/pms/project/dal/mysql/projectscope/**;pms-module-project/src/main/resources/mapper/projectscope/**;pms-module-project/src/test/java/cn/iocoder/yudao/module/pms/project/service/projectscope/**;docs/design/07-authorization-design.md;tasks/delivery-units/DU-20260908-PM01-MEMBER-CLOSURE.md;tasks/delivery-units/README.md`
...
> 集成记录：`d31f7903已交付已有MANAGE范围的成员闭环。需求方已澄清普通用户不能创建项目，管理范围应来自工程管理部/项目管理员角色；原Q-FPROJ-011是实现漏接被误判为业务待决，不再请求创建者例外授权。角色范围修复尚未实施，需与模板会话交接共享授权入口及状态文档；释放本次未实施的范围认领`
...
原交付未完成记录：当时将Q-FPROJ-011误列为需求方待决，未把创建者VIEW改成MANAGE；此业务待决判断由下述用户澄清撤销。Q-TPLACC-001等既有独立验收契约限制保持，未实施S0～S6阶段内业务，也不宣称通知已送达。初次Mockito自附加失败采用显式agent修复；新工作树首次类型检查因缺少Vite自动声明失败，在构建生成后复验通过；最后打包遭自身运行JAR占用，停自身服务后重试。总耗时从21:34:01至本交付提交，超过原效率目标，本次不追加流程治理修改。

2026-09-08需求方纠正：普通用户不能创建项目，只有工程管理部或具有项目管理权限的项目管理员角色可以创建；权限应来自角色管理范围，不是“创建者临时例外”。PRD 2.1、2.3.5和PM-01已规定工程管理部的全国项目管理范围，当前ProjectTreeScope只计入成员/显式授权而漏接该角色范围，应修实现而非新增业务裁决。`pms:project:create`又被V72拆分按钮及F-PROJ-002授权服务经理拆分复用，因此不能把单个create权限直接当作全项目管理范围，已撤回该未经落实的技术映射。

交接请求：模板DU已认领`service/projectmanual/**`、`docs/decisions/open-questions.md`和`tasks/features/F-PROJ-001.md`；两份文档的实际冲突由认领校验检出。本轮避让其边界，未改权限代码、未增加角色/权限码、未改普通成员范围或后端租户机制。请共享入口Owner同步撤销Q-FPROJ-011的业务待决表述，并与管理角色判定、根项目创建及拆分权限的衔接一起修复；不得采用创建者例外或仅凭create权限放大范围。
