...
> Implementation Done Gate：`NOT_REVIEWED_ON_MASTER`
> 当前阻断：`无；PLT迁移证据与IMP/AST实施地点拆出能力不计入本Feature Done`
...
需求方已确认COM-A与COM-B承载不同需求，按Requirement整体合并。统一规格以项目办事处发生时快照作为COM唯一地点事实；COM-B的AST站点/位置迁入IMP/AST，平台迁移证据迁入PLT。历史分支Gate与Done只作来源证据，不能转记master完成状态。
...
- [x] master排他DU和实施计划完成。
- [ ] PRD/SDS/Feature/追溯机器校验全部通过并提交上游基线。

## Task 2：COM-A纵向闭环适配

- [ ] 选择性迁入COM/PROJ/ACC窄契约、Provider、业务实现、REST、UI和测试。
- [ ] 将来源V124～V127重编号为V160～V163并更新全部引用。
- [ ] 在当前master依赖上完成聚焦测试和模块构建，形成可构建增量提交。

## Task 3：COM-B非重复需求能力

- [ ] 实现`CommerceAuthorityIngestApi`批次CAS并迁移受控导入调用。
- [ ] 实现人工候选、不可变依据与ERP Owner对账。
- [ ] 为订单—合同关系补齐稳定来源身份。
- [ ] 实现项目范围版本水位和`DeliveryScopeApi.getAssignedScope`，统一全部写路径的递增与锁序。
- [ ] 对已替代旧接口/旧模型加废弃标记，确认没有新调用继续建立在旧功能上。

## Task 4：拆分与整体验证

- [ ] 确认PLT迁移证据、IMP/AST实施地点不进入COM代码、表、DTO或完成状态。
- [ ] 完成MySQL迁移、权限负向、并发/幂等、前端测试与生产构建。
- [ ] 完成真实浏览器正向闭环及关键负向验收。
- [ ] 更新Requirement矩阵、DU回执和Implementation Done独立裁决。

> 检查点：基线=PRD修订010；当前Gate=Implementation；已通过=需求方Requirement合并方案；阻塞=无；下一步=选择性迁入COM-A闭环并在master适配。
