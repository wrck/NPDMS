...
- Task 10前端候选已形成：新增独立`/pms/engineering/arrival-acceptance`工作台、八操作API契约、批次列表/详情、草稿与更正表单、严格设备/订单型号数量明细、差异处置及PLT文件事实引用组件；仅按服务端`allowedActions`和五项权限投影操作，不从批次状态推导项目里程碑，不复用或修改旧`/pms/eng-arrival`页面。交互聚焦测试12项、TypeScript检查、定向ESLint/Stylelint/Prettier及前端生产构建均通过；当前为`CODE_REVIEW_REQUIRED`，Task 12前不激活生产Controller/Service，也不将组件测试或构建结果作为真实浏览器证据。
