...
- Task 5B实现中已完成V140两列前向迁移候选、列表/详情/allowedActions读模型，以及本人DRAFT PATCH、raise-difference和未确认批次差异追加处置的消费端候选；均未注册生产Bean。实现发现后继DRAFT的`batch_code`来源/生成规则未被正式契约锁定，且直接复制会违反`tenant_id + project_id + batch_code`唯一键，已登记`Q-FIMP002-002`。因此CONFIRMED来源successor、`CORRECT_INFORMATION`和豁免失效内部命令保持`BLOCKED_BY_SPEC`，Task 5B尚不得申请整体Code Review PASS，Task 8继续阻断。
- `Q-FIMP002-002`独立裁决为`GO / OPTION_B`：successor原样继承前驱`batch_code`，新增服务端`batch_root_marker`并以根marker唯一键和前驱唯一键形成线性后继链，不生成新业务编码。当前最近Gate为该身份/唯一模型规格与前向迁移合同的最小独立复审；Gate通过前仍不实现CONFIRMED successor、`CORRECT_INFORMATION`或豁免失效命令。
- Q-FIMP002-002规格与V141前向迁移合同候选已形成：既有可证明初始根确定性写`batch_root_marker=1`，旧批次唯一键替换为根marker唯一键并增加前驱唯一键；已有后继语义在首个ALTER前失败关闭且可repair/rerun。该候选尚待最小独立复审，不回写本Gate PASS。
