...
- Task 12前置依赖排序独立裁决为`NO-GO / BLOCKED_BY_DEPENDENCY`：COM `getAssignedScope`、AST `DeviceScopeFactApi`生产Provider与ACC生产消费者均未形成，禁止生产装配、旧接口降级、Fake/fallback及Job激活。`T-FIMP001-AST-01 Public Machine Contract Gate`锁定提交`c5f7ecda`已独立复审`PASS / GO`；当前只授权进入AST Owner Provider独立Code Review／真实MySQL锁与并发测试Gate，COM仍须由完整COM Owner Feature另行承接。
