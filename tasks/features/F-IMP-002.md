...
- AST `DeviceScopeFactApiAdapter`与ApplicationService、CommandService、FactApi三个消费调用面Gate已独立复审`PASS / GO`（`7fa32fc5`）：SN身份规范化、混合范围完整设备锁定及纯数量AST不适用边界均已闭合，聚焦回归72项通过。
- Task 12继续`NO-GO / BLOCKED_BY_DEPENDENCY`：COM `getAssignedScope`与ACC生产消费者尚未形成，禁止完整ApplicationService/FactApi/Controller装配、Fake/fallback、Job激活、真实浏览器和Implementation Done。
