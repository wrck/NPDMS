...
- Task 7消费端实现候选已形成：`inspect/lockAndRevalidate`仅消费COM已分配范围、AST设备归属水位和IMP已确认来源，按稳定顺序计算范围结果、来源批次、项目事实版本与`reopened`；最大事实版本只在唯一合格差异revision为来源时标记重开，普通确认根为非重开，缺失/重复/损坏或后继根语义不可证明时失败关闭。实现类保持可代理但不注册生产Bean；test-only Spring Bean已在隔离MySQL 8.4验证事务代理、并发重开使旧版本STALE和只读零审计/零第二真值表。COM `getAssignedScope`与AST `DeviceScopeFactApi`生产Provider/Adapter未合入前，本Task继续`IN_PROGRESS / BLOCKED_BY_DEPENDENCY`，不得回写生产完成。
