...
> 硬依赖：`F-IMP-001`、AST设备范围稳定事实Feature

## 当前最小工作单元

- 先锁定F-IMP-001、EXE-01～04和AST稳定设备范围的Owner Feature Spec/公开契约/合入顺序；本Feature不吸收IMP/AST Owner实现。
- 上述设计输入关闭并通过Feature Ready后生成现行Technical Plan，可用受控替身实施CUT侧；生产依赖合入前不声明真实浏览器闭环或Implementation Done。
