...
真实绑定保存暴露并修复定义/引用及图边ID未分配问题，增加写Mapper前必须有应用ID的回归；相关10项测试和运行包构建通过。一次失败后绑定/TASK定义已发布但原模板草稿仍保留未改，符合多HTTP非原子边界；最终保存重试前共享Docker引擎停止、MySQL连接拒绝，未擅自重启共享环境，最终保存成功及刷新持久化暂未复验。不能把配置候选或单测说成运行闭环已完成。
