# Review package: T-CP-008 re-review

Base: 38c5018
Head: eb585e732c970906e4a37826d37d61877a394807
...
```text
 ...
 pom.xml                                            |   4 +
 tasks/todo.md                                      |   6 +-
 .../verify-pms-module-boundaries-test.ps1          | 123 +++++++++++++++++
 .../verify-pms-module-boundaries.ps1               | 153 +++++++++++++++++++++
...
+    Write-Utf8File (Join-Path $fixtureRoot "pms-module-project\$apiModule\src\main\java\ProjectApi.java") "interface ProjectApi {}"
...
+    Write-Utf8File (Join-Path $validFixture "sql\migrations\V001__analytics_snapshot.sql") 'CREATE TABLE pms_analytics_snapshot (id bigint);'
...
+    Write-Utf8File (Join-Path $migrationFixture "sql\migrations\V002__integration_project.sql") 'CREATE TABLE pms_project (id bigint);'
...
+
+function Get-MavenDependencyArtifactIds {
+    param([string] $PomContent)
+
+    foreach ($dependency in [regex]::Matches($PomContent, "(?s)<dependency\b[^>]*>(.*?)</dependency>")) {
+        $artifactId = [regex]::Match($dependency.Groups[1].Value, "<artifactId>\s*([^<\s]+)\s*</artifactId>")
+        if ($artifactId.Success) {
+            $artifactId.Groups[1].Value
+        }
+    }
+}
+
+function Get-ModulePomContent {
+    param([string] $Module)
+
+    Get-Content -Raw -Encoding UTF8 (Join-Path $repoRoot "$Module\pom.xml")
+}
+
...
+    }
+    $apiSources = Get-ChildItem -Path $apiModulePath -Recurse -File |
...
+        $candidateDependencies = Get-MavenDependencyArtifactIds (Get-ModulePomContent $candidate)
+        if ($candidateDependencies -contains $apiArtifactId) {
...
+    param([string] $Module)
+
+    foreach ($artifactId in Get-MavenDependencyArtifactIds (Get-ModulePomContent $Module)) {
+        if ($artifactId -ne $Module -and $artifactId -in $modules) {
+            throw "$Module must not directly depend on PMS module $artifactId; use its stable API contract or domain event."
+        }
+        if ($artifactId -match "^pms-module-.+-biz$") {
+            throw "$Module must not directly depend on PMS -biz module $artifactId."
+        }
+    }
+}
+
...
+
+    $coreTablePattern = "(?i)(?<![A-Za-z0-9_])(?:pms_)?(?:customer|project_task|project|task|device|equipment|asset|spare_part|rma)(?:s)?(?![A-Za-z0-9_])"
+    $corePersistenceTypePattern = "(?i)\b(?:Customer|Project|Task|Device|Equipment|Asset|SparePart|Rma)(?:DO|Entity|Mapper|Repository)\b"
+    return $Content -match $coreTablePattern -or $Content -match $corePersistenceTypePattern
+}
+
...
+    $domain = $Module.Replace("pms-module-", "")
+    $persistenceFiles = @(Get-ChildItem -Path $modulePath -Recurse -File |
+        Where-Object {
+            $_.Extension -in @(".java", ".kt", ".sql") -or
+            ($_.Extension -eq ".xml" -and $_.Name -notin @("pom.xml", ".flattened-pom.xml"))
+        })
+    $migrationPath = Join-Path $repoRoot "sql\migrations"
+    if (Test-Path -LiteralPath $migrationPath) {
+        $persistenceFiles += Get-ChildItem -Path $migrationPath -File -Filter "*.sql" |
+            Where-Object { $_.Name -match "(?i)$domain" }
+    }
+
