# Review package: T-CP-008 final re-review

Base: 38c5018
Head: 5048ebb3c7c4368cee3c919e04a992801f89ca82
...
```text
 ...
 pom.xml                                            |   4 +
 tasks/todo.md                                      |   6 +-
 .../verify-pms-module-boundaries-test.ps1          | 179 +++++++++++++++++++
 .../verify-pms-module-boundaries.ps1               | 196 +++++++++++++++++++++
...
+    $coreMasterTables = @(
+        "pms_customer",
+        "pms_customer_contact",
+        "pms_project",
+        "pms_project_node",
+        "pms_project_task_wbs",
+        "pms_task",
+        "pms_device",
+        "pms_equipment",
+        "pms_asset",
...
+        "pms_rma"
+    )
+    foreach ($table in $coreMasterTables) {
+        $migrationFixture = New-FixtureRepository
+        $fixtures += $migrationFixture
+        Write-Utf8File (Join-Path $migrationFixture "pms-module-integration\src\main\resources\db\migration\V010__create_sync_tables.sql") "CREATE TABLE $table (id bigint);"
...
+        $dependencyContent = $dependency.Groups[1].Value
+        $artifactId = [regex]::Match($dependencyContent, "<artifactId>\s*([^<\s]+)\s*</artifactId>")
+        if ($artifactId.Success) {
+            $scope = [regex]::Match($dependencyContent, "<scope>\s*([^<\s]+)\s*</scope>")
+            $optional = [regex]::Match($dependencyContent, "<optional>\s*true\s*</optional>")
+            [pscustomobject]@{
+                ArtifactId = $artifactId.Groups[1].Value
+                Scope = if ($scope.Success) { $scope.Groups[1].Value } else { "compile" }
...
+        $hasPublicDeclaration = $content -match "(?m)^\s*public\s+(?:interface|class|record|enum|data\s+class|enum\s+class)\b"
...
+        $candidateDependencies = Get-MavenDependencies (Get-ModulePomContent $candidate)
...
+    param([string] $Module)
+
+    foreach ($dependency in Get-MavenDependencies (Get-ModulePomContent $Module)) {
...
+        @{ Name = "project node"; Tables = @("pms_project_node"); Types = @("ProjectNode") },
+        @{ Name = "project WBS"; Tables = @("pms_project_task_wbs"); Types = @("ProjectTaskWbs", "ProjectWbs") },
