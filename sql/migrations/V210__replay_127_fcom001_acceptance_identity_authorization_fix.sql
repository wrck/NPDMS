-- Chronologically replayed from 32092b115a32262070d62b60bb3d429da3e496c6 (codex/f-acc-001-sds), original sql/migrations/V127__fcom001_acceptance_identity_authorization_fix.sql.
...
DROP PROCEDURE IF EXISTS `fcom001_apply_v127_acceptance_identity_fix`;

DELIMITER $$
CREATE PROCEDURE `fcom001_apply_v127_acceptance_identity_fix`()
BEGIN
  ...
  IF managed_user_count <> 1
    ...
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'F-COM-001 V127 managed acceptance identity prerequisite mismatch';
  END IF;
  ...
  IF old_username_count = 1 AND new_username_count = 0
    ...
    IF ROW_COUNT() <> 1 THEN
      SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'F-COM-001 V127 managed username update failed';
    END IF;

    INSERT INTO `system_role_menu`
    (`role_id`, `menu_id`, `creator`, `create_time`, `updater`, `update_time`, `deleted`, `tenant_id`)
    VALUES
    (992002800001, 18069, 'fcom001_v127', NOW(), 'fcom001_v127', NOW(), b'0', 0);
  ELSEIF NOT (old_username_count = 0 AND new_username_count = 1
    ...
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'F-COM-001 V127 managed acceptance identity is partial or conflicting';
  END IF;
  ...
  IF (SELECT COUNT(*) FROM `system_users`
    ...
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'F-COM-001 V127 managed acceptance identity verification failed';
  END IF;
...
CALL `fcom001_apply_v127_acceptance_identity_fix`();
DROP PROCEDURE IF EXISTS `fcom001_apply_v127_acceptance_identity_fix`;
