...
DROP PROCEDURE IF EXISTS `facc001_apply_v170_project_query_permission`;

DELIMITER $$
CREATE PROCEDURE `facc001_apply_v170_project_query_permission`()
BEGIN
  ...
  IF managed_role_count <> 1 OR managed_menu_count <> 1 THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'F-ACC-001 V170 managed role or project query menu prerequisite mismatch';
  END IF;

  IF involved_grant_count = 0 THEN
    INSERT INTO `system_role_menu`
    (`role_id`, `menu_id`, `creator`, `create_time`, `updater`, `update_time`, `deleted`, `tenant_id`)
    VALUES (992004800002, 18067, 'facc001_v170', NOW(), 'facc001_v170', NOW(), b'0', 0);
  ELSEIF NOT (involved_grant_count = 1 AND exact_grant_count = 1) THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'F-ACC-001 V170 project query grant is partial or conflicting';
  END IF;
  ...
  IF (SELECT COUNT(*) FROM `system_role_menu`
    ...
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'F-ACC-001 V170 project query grant verification failed';
  END IF;
...
CALL `facc001_apply_v170_project_query_permission`();
DROP PROCEDURE IF EXISTS `facc001_apply_v170_project_query_permission`;
