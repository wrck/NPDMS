...
DROP PROCEDURE IF EXISTS `facc001_apply_v169_acceptance_report_jobs`;

DELIMITER $$
CREATE PROCEDURE `facc001_apply_v169_acceptance_report_jobs`()
BEGIN
  ...
  IF involved_row_count = 0 THEN
    ...
    (992004900001,'验收报告事件投递',1,'acceptanceReportOutboxDeliveryJob','',
     '0/30 * * * * ?',0,0,0,'facc001_v169',NOW(3),'facc001_v169',NOW(3),b'0'),
  ...
  ELSEIF NOT (involved_row_count = 2 AND exact_row_count = 2) THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'F-ACC-001 V169 acceptance report job state is partial or conflicting';
  END IF;
  ...
  IF (SELECT COUNT(*)
    ...
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'F-ACC-001 V169 acceptance report job verification failed';
  END IF;
...
CALL `facc001_apply_v169_acceptance_report_jobs`();
DROP PROCEDURE IF EXISTS `facc001_apply_v169_acceptance_report_jobs`;
