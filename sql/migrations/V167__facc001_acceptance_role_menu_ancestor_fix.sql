...
-- V166保持不可变；本迁移不新增权限键、菜单、角色或业务角色模板。

DROP PROCEDURE IF EXISTS `facc001_apply_v167_role_menu_ancestor_fix`;

DELIMITER $$
CREATE PROCEDURE `facc001_apply_v167_role_menu_ancestor_fix`()
BEGIN
  ...
  IF managed_role_count <> 1 OR managed_ancestor_menu_count <> 6 THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'F-ACC-001 V167 managed role or menu ancestor prerequisite mismatch';
  END IF;

  IF managed_ancestor_grant_count = 0 AND managed_ancestor_grant_row_count = 0 THEN
    INSERT INTO `system_role_menu`
    (`role_id`, `menu_id`, `creator`, `create_time`, `updater`, `update_time`, `deleted`, `tenant_id`)
    SELECT 992004800002, grant_row.`menu_id`, 'facc001_v167', NOW(),
           'facc001_v167', NOW(), b'0', 0
    FROM (
  ...
  ELSEIF NOT (managed_ancestor_grant_count = 6 AND managed_ancestor_grant_row_count = 6) THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'F-ACC-001 V167 managed role menu ancestor state is partial or conflicting';
  END IF;
  ...
  IF (SELECT COUNT(*) FROM `system_role_menu`
    ...
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'F-ACC-001 V167 managed role menu ancestor verification failed';
  END IF;
...
CALL `facc001_apply_v167_role_menu_ancestor_fix`();
DROP PROCEDURE IF EXISTS `facc001_apply_v167_role_menu_ancestor_fix`;
