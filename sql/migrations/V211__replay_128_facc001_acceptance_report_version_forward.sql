-- Chronologically replayed from 229e9f4b946c3933405b43ca2cd11e519d7d921c (codex/f-acc-001-sds), original sql/migrations/V128__facc001_acceptance_report_version_forward.sql.
...
DROP PROCEDURE IF EXISTS `facc001_preflight_v128`;
DELIMITER $$
CREATE PROCEDURE `facc001_preflight_v128`()
BEGIN
  ...
  IF partial_pair_count<>0 OR invalid_state_count<>0 OR invalid_contract_count<>0
    ...
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='F-ACC-001 V128 preflight found partial, invalid or conflicting legacy input';
  END IF;
  ...
  IF (SELECT COUNT(*) FROM `proj_project` WHERE `tenant_id`=0 AND (`id`=992004000001 OR `project_code`='FACC001-ACCEPTANCE-001'))<>0
    ...
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='F-ACC-001 V128 managed identities already occupied';
  END IF;
END$$
DELIMITER ;
CALL `facc001_preflight_v128`();
DROP PROCEDURE `facc001_preflight_v128`;
...
SELECT CASE WHEN pair.`project_id`=992004000001 THEN 992004500001 ELSE -pair.`initial_task_id` END,
 ...
 'v128-facc001','v128-facc001',b'0',pair.`tenant_id`
...
SET contract.`effective_to`=NOW(3),contract.`updater`='v128-facc001';
...
DROP PROCEDURE IF EXISTS `facc001_verify_v128`;
DELIMITER $$
CREATE PROCEDURE `facc001_verify_v128`()
BEGIN
  IF (SELECT COUNT(*) FROM `acc_acceptance`
    ...
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT='F-ACC-001 V128 managed conversion or permission facts are incomplete';
  END IF;
END$$
DELIMITER ;
CALL `facc001_verify_v128`();
DROP PROCEDURE `facc001_verify_v128`;
