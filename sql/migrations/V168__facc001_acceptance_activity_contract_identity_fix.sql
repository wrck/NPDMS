-- F-ACC-001 / ACC-03：纠正V166活动与当前ACC执行契约的身份关系。
-- V166、V167保持不可变；本迁移不改变活动、任务、报告或契约的状态与版本。

DROP PROCEDURE IF EXISTS `facc001_apply_v168_activity_contract_identity_fix`;

DELIMITER $$
CREATE PROCEDURE `facc001_apply_v168_activity_contract_identity_fix`()
BEGIN
  ...
  DROP TEMPORARY TABLE IF EXISTS `_facc001_v168_contract_mapping`;
  CREATE TEMPORARY TABLE `_facc001_v168_contract_mapping` (
  ...
  FROM `acc_acceptance`
  WHERE `creator` = 'v166-facc001'
    AND `deleted` = b'0';

  IF target_activity_count = 0 THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'F-ACC-001 V168 V166 activity input is missing';
  END IF;

  INSERT INTO `_facc001_v168_contract_mapping`
  ...
  IF allowed_input_count <> target_activity_count THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'F-ACC-001 V168 activity contract input is partial or conflicting';
  END IF;

  UPDATE `acc_acceptance` activity
  JOIN `_facc001_v168_contract_mapping` mapping
  ...
  IF (SELECT COUNT(*)
    ...
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'F-ACC-001 V168 activity contract identity verification failed';
  END IF;

  DROP TEMPORARY TABLE `_facc001_v168_contract_mapping`;
  COMMIT;
END$$
DELIMITER ;

CALL `facc001_apply_v168_activity_contract_identity_fix`();
DROP PROCEDURE IF EXISTS `facc001_apply_v168_activity_contract_identity_fix`;
