-- Chronologically replayed from 486727a3a856fe5de19683f3e7eef1d38b88f6a0 (codex/f-acc-001-sds), original sql/migrations/V134__restore_project_task_assign_permission.sql.
...
DROP PROCEDURE IF EXISTS `facc002_apply_v134_project_task_assign_permission`;

DELIMITER $$
CREATE PROCEDURE `facc002_apply_v134_project_task_assign_permission`()
BEGIN
  ...
  IF customer_menu_count <> 1 OR task_menu_count <> 1 THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'F-ACC-002 V134 customer or project task menu prerequisite mismatch';
  END IF;

  IF target_id_count <> 0 OR active_assign_count <> 0 THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'F-ACC-002 V134 project task assign menu identity conflict';
  END IF;
  ...
  (930926, '项目任务指派', 'pms:project-task:assign', 3, 50, 18014, '', '',
   NULL, NULL, 0, b'1', b'1', b'1', 'facc002_v134', NOW(), 'facc002_v134', NOW(), b'0');
  ...
  IF (SELECT COUNT(*) FROM `system_menu`
    ...
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'F-ACC-002 V134 project task assign menu verification failed';
  END IF;
  ...
  IF (SELECT COUNT(*) FROM `system_menu`
    ...
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'F-ACC-002 V134 customer menu history changed unexpectedly';
  END IF;
...
CALL `facc002_apply_v134_project_task_assign_permission`();
DROP PROCEDURE IF EXISTS `facc002_apply_v134_project_task_assign_permission`;
