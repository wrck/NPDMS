...
class RequirementAnalysisExecutionAccessTest {
    ...
    @Test void sameRoundAllowsCurrentVersionsButLocksObservedContext() {
        RequirementAnalysisRevisionDO root = root();
    ...
    @Test void sharedWriteGuardDenialCannotFallBackToOriginOrStandaloneResolution() {
        var root = root();
        String original = root.getExecutionSnapshot();
    ...
    void explicitlySharedHandlingUsesTheChosenNodeWithoutRewritingOrigin(String kind) {
        var root = root();
        String origin = root.getExecutionSnapshot();
        ...
        assertEquals(origin,root.getExecutionSnapshot());
        if ("TASK".equals(kind)) {
    ...
    @Test void stageFreezesItsOwnIdentityAndBeginsRealHandlingWithoutBorrowingATask() {
        ...
        var restored = JsonUtils.parseObject(service.freeze(frozen), RequirementAnalysisExecutionAccess.Frozen.class);
    ...
    @Test void stageReworkResolvesCurrentRoundButDoesNotRewriteBusinessOrigin() {
        var stage = stageBinding();
        var root = new RequirementAnalysisRevisionDO(); root.setProjectId(100L);
        root.setExecutionSnapshot(service.freeze(new RequirementAnalysisExecutionAccess.Frozen(stage, null, stageContext(500L, 1))));
        String original = root.getExecutionSnapshot();
    ...
    @Test void invalidOrUnavailableStageCannotFallBackToTaskHandling() {
        var stage = stageBinding();
        assertThrows(RuntimeException.class, () -> service.freeze(new RequirementAnalysisExecutionAccess.Frozen(stage, context(400L,500L,1,true), null)));
    ...
    private RequirementAnalysisRevisionDO root() {
        var root = new RequirementAnalysisRevisionDO(); root.setProjectId(100L);
        root.setExecutionSnapshot(service.freeze(binding, context(400L, 500L, 1, true)));
        return root;
    }
