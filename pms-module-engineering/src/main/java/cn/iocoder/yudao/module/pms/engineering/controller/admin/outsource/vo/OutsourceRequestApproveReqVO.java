...
/**
 * 管理后台 - PMS 外包申请审批 Request VO（FR-ENG-002）。
 * <p>
 * 用于 approve 接口。审批动作 approveAction 决定状态流转：
 * PASS 通过 / REJECT 驳回 / RETURN 退回 / TRANSFER 转签 / COUNTERSIGN 会签。
 */
@Schema(description = "管理后台 - PMS 外包申请审批 Request VO")
@Data
public class OutsourceRequestApproveReqVO {

    @Schema(description = "外包申请编号", requiredMode = Schema.RequiredMode.REQUIRED, example = "1024")
    @NotNull(message = "外包申请编号不能为空")
    private Long id;
