 ...
 * <p>
 * 路径前缀 {@code /pms/sol-schedule-backward}，对应菜单权限 {@code pms:sol-schedule-backward:*}。
 * 提供倒排记录 CRUD、计算（逆序推算阶段日期并校验）、应用（更新到项目阶段）能力。
 */
@Tag(name = "管理后台 - PMS 工期倒排")
@RestController
@RequestMapping("/pms/sol-schedule-backward")
@Validated
public class ScheduleBackwardController {
    ...
    @PreAuthorize("@ss.hasPermission('pms:sol-schedule-backward:create')")
    ...
    @PreAuthorize("@ss.hasPermission('pms:sol-schedule-backward:update')")
    ...
    @PreAuthorize("@ss.hasPermission('pms:sol-schedule-backward:delete')")
    ...
    @Parameter(name = "id", description = "倒排记录编号", required = true, example = "1024")
    @PreAuthorize("@ss.hasPermission('pms:sol-schedule-backward:query')")
    ...
    @Parameter(name = "id", description = "倒排记录编号", required = true, example = "1024")
    @PreAuthorize("@ss.hasPermission('pms:sol-schedule-backward:calculate')")
    ...
    @Parameter(name = "id", description = "倒排记录编号", required = true, example = "1024")
    @PreAuthorize("@ss.hasPermission('pms:sol-schedule-backward:apply')")
