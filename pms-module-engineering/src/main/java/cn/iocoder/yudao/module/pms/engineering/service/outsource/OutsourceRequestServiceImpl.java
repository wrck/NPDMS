...
/**
 * PMS 外包申请 Service 实现（FR-ENG-002）。
 * <p>
...
public class OutsourceRequestServiceImpl implements OutsourceRequestService {
    ...
    @Override
    public OutsourceRequestDO getOutsourceRequest(Long id) {
        return outsourceRequestMapper.selectById(id);
    }

    @Override
    public OutsourceRequestDO validateOutsourceRequestExists(Long id) {
        OutsourceRequestDO entity = outsourceRequestMapper.selectById(id);
        if (entity == null) {
            throw exception(OUTSOURCE_NOT_EXISTS);
        }
        return entity;
    }

    @Override
    public PageResult<OutsourceRequestDO> getOutsourceRequestPage(OutsourceRequestPageReqVO pageReqVO) {
        return outsourceRequestMapper.selectPage(pageReqVO);
    }
     ...
     */
    private void updateStatus(OutsourceRequestDO entity, int newStatus,
    ...
    private void validateCodeUnique(String code, Long excludeId) {
        ...
        OutsourceRequestDO existing = outsourceRequestMapper.selectByCode(code);
        if (existing == null) {
            return;
        }
        if (excludeId == null || !Objects.equals(existing.getId(), excludeId)) {
            throw exception(OUTSOURCE_CODE_DUPLICATE, code);
        }
    }
    ...
    private void validateProjectExists(Long projectId) {
        ...
        // 若项目不存在，抛出 exception(OUTSOURCE_PROJECT_NOT_EXISTS)
    }
    ...
    private void validateStatus(OutsourceRequestDO entity, int... allowedStatuses) {
