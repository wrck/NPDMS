...
- Phase 3 PRD验收基线：WHEN 用户上传同一业务对象的新文件；THEN 平台生成新版本并保留旧版本、校验摘要和版本说明；WHEN 审批单引用文件；THEN 审批记录保存文件版本ID，文件后续更新不影响历史审批；WHEN 无权用户访问文件；THEN 平台拒绝下载并记录拒绝原因；WHEN 部署未启用文件病毒扫描且其他文件校验全部通过；THEN 平台不调用扫描Provider，生成新版本并记录`SKIPPED`及空Provider事实，不表示为扫描通过；WHEN 部署启用病毒扫描但Provider不可用、异常或未通过；THEN 平台拒绝入库且不得自动降级为`SKIPPED`
