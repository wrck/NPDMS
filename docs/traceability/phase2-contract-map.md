...
- 外部集成：N/A（平台内部契约）
- 文件契约：FileArtifact
- 工作流/状态：字段/附件完整只形成证据；明确通过且顺序合法、当前范围完整覆盖才满足门禁；同类型一个当前报告，A+B须新总体明确通过
- 授权与数据范围：ProjectStageScope、FileBusinessScope
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；文件上传/下载/版本/恶意内容与权限回源测试
...
- 外部集成：N/A（平台内部契约）
- 文件契约：FileArtifact
- 工作流/状态：索引有效来源/版本/哈希/结论与范围；失败验收证据可归档但不满足通过门禁；下载权限不改变客观齐套
...
- Phase 3业务守卫断言：按“索引有效来源/版本/哈希/结论与范围；失败验收证据可归档但不满足通过门禁；下载权限不改变客观齐套”执行；PRD验收基线中的非法状态、版本冲突、重复请求或无效输入由对应业务守卫拒绝，原有效业务事实保持不变
...
- 外部集成：现有采集平台子应用
- 文件契约：FileArtifact
- 工作流/状态：V1 P3暂存不推进；有效提交才P4；设备采集绑定清单版本/采集项/设备，技术成功不替代CUT结果；V2增加授权清单导出及受控流程跳转配置优化，不绕过必填、版本和权限门禁
...
- Phase 3业务守卫断言：按“V1 P3暂存不推进；有效提交才P4；设备采集绑定清单版本/采集项/设备，技术成功不替代CUT结果；V2增加授权清单导出及受控流程跳转配置优化，不绕过必填、版本和权限门禁”执行；PRD验收基线中的非法状态、版本冲突、重复请求或无效输入由对应业务守卫拒绝，原有效业务事实保持不变
...
- 外部集成：N/A（平台内部契约）
- 文件契约：FileArtifact
- 工作流/状态：文件身份/版本/权限；外部来源认证+契约验签+对象授权+幂等均须成立；扫描默认关闭真实SKIPPED
...
- Phase 3业务守卫断言：按“文件身份/版本/权限；外部来源认证+契约验签+对象授权+幂等均须成立；扫描默认关闭真实SKIPPED”执行；PRD验收基线中的非法状态、版本冲突、重复请求或无效输入由对应业务守卫拒绝，原有效业务事实保持不变
