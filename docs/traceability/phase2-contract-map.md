...
- 工作流/状态：PROJ冻结模板Fact和业务时点→MANDATORY初始化revision1并冻结原始source→指派→受控客户提交→不可变判定→未达标/失效追加RemediationFact并触发同collectionKey下一revision→仅有效达标Result精确进入D-SAT-REPORT/T-SAT-SURVEY来源根→归档失败补偿；Todo或送达不替代提交/通过
...
- Phase 3业务守卫断言：按“PROJ冻结模板Fact和业务时点→MANDATORY初始化revision1并冻结原始source→指派→受控客户提交→不可变判定→未达标/失效追加RemediationFact并触发同collectionKey下一revision→仅有效达标Result精确进入D-SAT-REPORT/T-SAT-SURVEY来源根→归档失败补偿；Todo或送达不替代提交/通过”执行；PRD验收基线中的非法状态、版本冲突、重复请求或无效输入由对应业务守卫拒绝，原有效业务事实保持不变
