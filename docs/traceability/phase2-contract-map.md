...
- Phase 3验收断言：按PRD PM-01业务验收标准验证“项目创建、来源匹配与指派守卫”；越权按“ProjectTreeScope；来源字段只读”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
...
- Phase 3验收断言：按PRD PM-02业务验收标准验证“无环移动、完整投影版本切换”；越权按“ProjectTreeScope；后代范围服务端计算”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
...
- Phase 3验收断言：按PRD PM-03业务验收标准验证“模板内StageDefinition/TaskDefinition发布、实例冻结、必填WorkBinding（默认TASK_NATIVE）/PermissionPolicy/CompletionRule/GateRef校验和阶段门禁”；越权按“项目模板维护权限；项目阶段范围；非TASK_NATIVE绑定目标权限不得越权”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
...
- 授权与数据范围：ProjectTreeScope；后代范围服务端计算
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试
- Phase 3验收断言：按PRD PM-04业务验收标准验证“无环移动、完整投影版本切换”；越权按“ProjectTreeScope；后代范围服务端计算”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据
...
- Phase 3验收断言：按PRD PM-05业务验收标准验证“处理中→部分失败/待处理→已完成；全部成功后源项目只读归档”；越权按“同时具备源/目标项目管理权限；原敏感对象权限继续生效”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
...
- Phase 3验收断言：按PRD PM-06业务验收标准验证“群组成员增删、唯一期次、无环和派生版本”；越权按“同时有权的期次可维护；查询按各期权限裁剪”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试
- Phase 3验收断言：按PRD PM-07业务验收标准验证“自动识别结果确认与留痕”；越权按“项目管理范围；来源证据只读”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试
- Phase 3验收断言：按PRD PM-08业务验收标准验证“V1手动指派、V2规则候选确认”；越权按“项目管理范围；仅PRD角色”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试
- Phase 3验收断言：按PRD PM-09业务验收标准验证“批量逐项变更并保留有效区间”；越权按“项目管理范围；逐项目校验”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试
- Phase 3验收断言：按PRD PM-10业务验收标准验证“受控回退保持ACTIVE；异常关闭置EXCEPTION_CLOSED；正常闭环仅由CLO-02产生NORMAL_CLOSED”；越权按“ProjectTreeScope；状态命令权限与门禁”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据
...
- Phase 3验收断言：按PRD PM-11业务验收标准验证“ProjectTask内必填WorkBinding/CompletionRule与Stage→ProjectTask工作台投影、任务任意层级移动；TASK_NATIVE按任务自身事实执行，其他类型回源绑定事实并追加完成判定后完成”；越权按“ProjectTreeScope；TASK_NATIVE任务范围；其他类型由服务端合并目标业务对象权限”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
...
- Phase 3验收断言：按PRD PRE-01业务验收标准验证“工期基线、变更申请与审批”；越权按“ProjectStageScope；计划审批节点”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD PRE-02业务验收标准验证“工勘/需求填写、提交、确认和退回”；越权按“ProjectStageScope；字段与文件权限”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3验收断言：按PRD PRE-03业务验收标准验证“换货申请、外部处理映射与恢复对账”；越权按“ProjectStageScope；物料范围”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
...
- 授权与数据范围：ProjectStageScope；字段与文件权限
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD PRE-04业务验收标准验证“工勘/需求填写、提交、确认和退回”；越权按“ProjectStageScope；字段与文件权限”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD PRE-05业务验收标准验证“交底书生成、版本提交和确认”；越权按“ProjectStageScope、FileBusinessScope”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试
- Phase 3验收断言：按PRD PLN-01业务验收标准验证“计划计算候选、预警和显式应用”；越权按“ProjectStageScope；计划只读/维护分离”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录
...
- 授权与数据范围：ProjectStageScope；计划只读/维护分离
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试
- Phase 3验收断言：按PRD PLN-02业务验收标准验证“计划计算候选、预警和显式应用”；越权按“ProjectStageScope；计划只读/维护分离”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录
...
- 授权与数据范围：ProjectStageScope；计划只读/维护分离
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试
- Phase 3验收断言：按PRD PLN-03业务验收标准验证“计划计算候选、预警和显式应用”；越权按“ProjectStageScope；计划只读/维护分离”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD PLN-04业务验收标准验证“施工计划提交、审批、驳回与换版”；越权按“ProjectStageScope；审批人按PRD”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD SCH-01业务验收标准验证“方案导入/编审、重大复审和发布版本”；越权按“ProjectStageScope；方案审批与文件权限”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- 授权与数据范围：ProjectStageScope；方案审批与文件权限
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD SCH-02业务验收标准验证“方案导入/编审、重大复审和发布版本”；越权按“ProjectStageScope；方案审批与文件权限”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD SCH-03业务验收标准验证“配置脚本上传、解析、发布和版本冻结”；越权按“ProjectStageScope、FileBusinessScope；已发布模板使用权限”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- 授权与数据范围：ProjectStageScope；方案审批与文件权限
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD SCH-04业务验收标准验证“方案导入/编审、重大复审和发布版本”；越权按“ProjectStageScope；方案审批与文件权限”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- 授权与数据范围：ProjectStageScope；方案审批与文件权限
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD SCH-05业务验收标准验证“方案导入/编审、重大复审和发布版本”；越权按“ProjectStageScope；方案审批与文件权限”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3验收断言：按PRD EXE-01业务验收标准验证“草稿、差异处理、项目经理最终确认”；越权按“ImplementationProjectBatchScope；项目经理确认”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD EXE-02业务验收标准验证“提交、项目经理确认/退回和整改”；越权按“ImplementationProjectDeviceScope；设备归属与项目权限”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3验收断言：按PRD EXE-03业务验收标准验证“采集回调、框板解析、待匹配/人工绑定和业务消费确认”；越权按“ImplementationProjectDeviceCollectionScope、BusinessObjectDeviceCredentialScope；设备关系维护权限”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD EXE-04业务验收标准验证“采集回调、联调结论和问题引用”；越权按“ImplementationProjectDeviceCollectionScope、BusinessObjectDeviceCredentialScope”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD EXE-05业务验收标准验证“风险提出、处置和关闭”；越权按“ImplementationProjectDeviceScope；风险范围”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试
- Phase 3验收断言：按PRD EXE-06业务验收标准验证“就绪门禁汇总、快照发布与CUT消费”；越权按“ImplementationProjectCutoverScope”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据
...
- 授权与数据范围：ProjectStageScope、FileBusinessScope
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD ACC-01业务验收标准验证“培训/报告提交、确认和问题留痕”；越权按“ProjectStageScope、FileBusinessScope”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD ACC-02业务验收标准验证“冻结模板→指派→客户提交→判定→整改后新版本重收→归档”；越权按“ProjectStageScope；客户一次性实例范围；答案/签字不可改写”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；文件哈希、版本、扫描、引用与权限拒绝记录
...
- 授权与数据范围：ProjectStageScope、FileBusinessScope
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD ACC-03业务验收标准验证“培训/报告提交、确认和问题留痕”；越权按“ProjectStageScope、FileBusinessScope”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD ACC-04业务验收标准验证“齐套检查、审核和归档分离”；越权按“ProjectStageScope、FileBusinessScope；ACC归档”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD CLO-01业务验收标准验证“门禁校验、冻结流程审批、整改和闭环；不创建回访节点”；越权按“ProjectStageScope；全部后代项目门禁与审批范围”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；文件哈希、版本、扫描、引用与权限拒绝记录
...
- 授权与数据范围：ProjectStageScope；全部后代项目门禁与审批范围
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD CLO-02业务验收标准验证“门禁校验、冻结流程审批、整改和闭环；不创建回访节点”；越权按“ProjectStageScope；全部后代项目门禁与审批范围”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD SUB-01业务验收标准验证“平台内转包审批、价格审批与版本冻结”；越权按“OrganizationSupplierScope；项目/供应商范围”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- 授权与数据范围：OrganizationSupplierScope；项目/供应商范围
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD SUB-02业务验收标准验证“平台内转包审批、价格审批与版本冻结”；越权按“OrganizationSupplierScope；项目/供应商范围”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD SUB-03业务验收标准验证“付款前置满意度事实、批准版本和财务确认”；越权按“OrganizationSupplierScope；付款门禁权限；满意度只读引用”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- 授权与数据范围：OrganizationSupplierScope；付款门禁权限；满意度只读引用
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD SUB-04业务验收标准验证“付款前置满意度事实、批准版本和财务确认”；越权按“OrganizationSupplierScope；付款门禁权限；满意度只读引用”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- 授权与数据范围：OrganizationSupplierScope；项目/供应商范围
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD SUB-05业务验收标准验证“平台内转包审批、价格审批与版本冻结”；越权按“OrganizationSupplierScope；项目/供应商范围”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试
- Phase 3验收断言：按PRD CUS-01业务验收标准验证“客户同步、临时客户受控合并和联系人关系”；越权按“OrganizationCustomerScope；CRM字段只读”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录
...
- 授权与数据范围：OrganizationCustomerScope；CRM字段只读
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试
- Phase 3验收断言：按PRD CUS-02业务验收标准验证“客户同步、临时客户受控合并和联系人关系”；越权按“OrganizationCustomerScope；CRM字段只读”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录
...
- 授权与数据范围：OrganizationCustomerScope；CRM字段只读
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试
- Phase 3验收断言：按PRD CUS-03业务验收标准验证“客户同步、临时客户受控合并和联系人关系”；越权按“OrganizationCustomerScope；CRM字段只读”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录
...
- 授权与数据范围：OrganizationCustomerScope；CRM字段只读
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试
- Phase 3验收断言：按PRD CUS-04业务验收标准验证“客户同步、临时客户受控合并和联系人关系”；越权按“OrganizationCustomerScope；CRM字段只读”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD EQP-01业务验收标准验证“设备档案、配置Log引用、框板关系、扫码和唯一归属”；越权按“ProjectDeviceScope；当前归属、框板关系维护与祖先范围”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；文件哈希、版本、扫描、引用与权限拒绝记录
...
- 授权与数据范围：ProjectDeviceScope；当前归属、框板关系维护与祖先范围
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD EQP-02业务验收标准验证“设备档案、配置Log引用、框板关系、扫码和唯一归属”；越权按“ProjectDeviceScope；当前归属、框板关系维护与祖先范围”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；文件哈希、版本、扫描、引用与权限拒绝记录
...
- 授权与数据范围：ProjectDeviceScope；当前归属、框板关系维护与祖先范围
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD EQP-03业务验收标准验证“设备档案、配置Log引用、框板关系、扫码和唯一归属”；越权按“ProjectDeviceScope；当前归属、框板关系维护与祖先范围”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试
- Phase 3验收断言：按PRD EQP-04业务验收标准验证“MES来源版本幂等同步与冲突隔离”；越权按“ProjectDeviceScope；MES字段只读”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录
...
- 授权与数据范围：ProjectDeviceScope；当前归属、框板关系维护与祖先范围
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD EQP-05业务验收标准验证“设备档案、配置Log引用、框板关系、扫码和唯一归属”；越权按“ProjectDeviceScope；当前归属、框板关系维护与祖先范围”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；文件哈希、版本、扫描、引用与权限拒绝记录
...
- 授权与数据范围：ProjectDeviceScope；当前归属、框板关系维护与祖先范围
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD EQP-07业务验收标准验证“设备档案、配置Log引用、框板关系、扫码和唯一归属”；越权按“ProjectDeviceScope；当前归属、框板关系维护与祖先范围”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试
- Phase 3验收断言：按PRD RPT-02业务验收标准验证“项目状态指标口径计算、快照发布和水位展示”；越权按“OrganizationReportScope；字段级脱敏”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD CUT-01业务验收标准验证“任务创建、分级评估、风险/调研矩阵”；越权按“CutoverTaskScope；项目/设备范围”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；文件哈希、版本、扫描、引用与权限拒绝记录
...
- 授权与数据范围：CutoverTaskScope；项目/设备范围
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD CUT-02业务验收标准验证“任务创建、分级评估、风险/调研矩阵”；越权按“CutoverTaskScope；项目/设备范围”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD CUT-03业务验收标准验证“CutoverTask内P3清单版本在同一工作台动态匹配、人工填写/上传、采集任务下发、结果版本选择和配置缺口留痕；不复制DAC技术状态”；越权按“CutoverTaskScope、BusinessObjectDeviceCredentialScope；服务端按清单项和设备范围裁剪”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD CUT-04业务验收标准验证“方案编审、分级审批和版本冻结”；越权按“CutoverTaskScope；分级审批权限”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；文件哈希、版本、扫描、引用与权限拒绝记录
...
- 授权与数据范围：CutoverTaskScope；分级审批权限
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD CUT-05业务验收标准验证“方案编审、分级审批和版本冻结”；越权按“CutoverTaskScope；分级审批权限”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD CUT-06业务验收标准验证“P6闭环填写、INT-12证据引用、提交归档和结果回流；不建立逐步骤执行或稳定观察”；越权按“CutoverTaskScope、BusinessObjectDeviceCredentialScope”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- 授权与数据范围：CutoverTaskScope；分级审批权限
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD CUT-07业务验收标准验证“方案编审、分级审批和版本冻结”；越权按“CutoverTaskScope；分级审批权限”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3验收断言：按PRD CUT-08业务验收标准验证“备件申请映射、回调、门禁和对账”；越权按“CutoverTaskScope；外部备件范围”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
...
- 授权与数据范围：CutoverTaskScope；项目/设备范围
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD CUT-09业务验收标准验证“任务创建、分级评估、风险/调研矩阵”；越权按“CutoverTaskScope；项目/设备范围”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；文件哈希、版本、扫描、引用与权限拒绝记录
...
- 授权与数据范围：CutoverTaskScope；项目/设备范围
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD CUT-10业务验收标准验证“任务创建、分级评估、风险/调研矩阵”；越权按“CutoverTaskScope；项目/设备范围”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD INS-01业务验收标准验证“方式选择、预检/执行、业务消费和归档门禁”；越权按“AssignedProjectDeviceScope、BusinessObjectDeviceCredentialScope”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- 授权与数据范围：AssignedProjectDeviceScope、BusinessObjectDeviceCredentialScope
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD INS-02业务验收标准验证“方式选择、预检/执行、业务消费和归档门禁”；越权按“AssignedProjectDeviceScope、BusinessObjectDeviceCredentialScope”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试
- Phase 3验收断言：按PRD INS-03业务验收标准验证“规则配置、发布版本和任务冻结”；越权按“AssignedProjectDeviceScope；规则维护/使用分离”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录
...
- 授权与数据范围：AssignedProjectDeviceScope、BusinessObjectDeviceCredentialScope
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD INS-04业务验收标准验证“方式选择、预检/执行、业务消费和归档门禁”；越权按“AssignedProjectDeviceScope、BusinessObjectDeviceCredentialScope”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD INS-05业务验收标准验证“报告生成、回调校验、发布版本”；越权按“AssignedProjectDeviceScope、FileBusinessScope”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD INS-06业务验收标准验证“问题标注、误报、整改复核和关闭”；越权按“AssignedProjectDeviceScope；问题责任范围”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；文件哈希、版本、扫描、引用与权限拒绝记录
...
- 授权与数据范围：AssignedProjectDeviceScope、BusinessObjectDeviceCredentialScope
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD INS-07业务验收标准验证“方式选择、预检/执行、业务消费和归档门禁”；越权按“AssignedProjectDeviceScope、BusinessObjectDeviceCredentialScope”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- 授权与数据范围：AssignedProjectDeviceScope；问题责任范围
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD INS-08业务验收标准验证“问题标注、误报、整改复核和关闭”；越权按“AssignedProjectDeviceScope；问题责任范围”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；文件哈希、版本、扫描、引用与权限拒绝记录
...
- 授权与数据范围：AssignedProjectDeviceScope；规则维护/使用分离
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试
- Phase 3验收断言：按PRD INS-09业务验收标准验证“规则配置、发布版本和任务冻结”；越权按“AssignedProjectDeviceScope；规则维护/使用分离”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试
- Phase 3验收断言：按PRD INT-01业务验收标准验证“双源同步、待映射、字段Owner裁决和对账”；越权按“集成服务账号限Owner字段；工程管理部映射权限”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试
- Phase 3验收断言：按PRD INT-02业务验收标准验证“版本同步、来源冲突隔离和对账”；越权按“集成账号限ITR字段；ProjectDeviceScope查询”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试
- Phase 3验收断言：按PRD INT-05业务验收标准验证“必要人员组织同步复用基础平台主数据、已有同步批次和来源键映射；待办链接和通知回执不接入打卡/工时”；越权按“TenantOrganizationProjectScope；目录身份不直接等于项目角色”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；外部集成映射、超时/重试/对账/降级测试
- Phase 3验收断言：按PRD INT-09业务验收标准验证“认证断言校验、目录映射和平台会话”；越权按“平台RBAC/DataScope；目录组不直授项目角色”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；脱敏请求响应、幂等键、重试/对账与降级记录
...
- Phase 3验收断言：按PRD INT-12业务验收标准验证“授权校验→下发→回调→业务消费→完成/失败；独立中心按成功回调终态”；越权按“BusinessObjectDeviceCredentialScope；创建人默认权限和五元组授权”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
...
- 授权与数据范围：OrganizationCustomerScope；CRM字段只读
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试
- Phase 3验收断言：按PRD INT-03业务验收标准验证“客户同步、临时客户合并与字典映射”；越权按“OrganizationCustomerScope；CRM字段只读”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试
- Phase 3验收断言：按PRD INT-04业务验收标准验证“公告镜像、版本同步和业务引用”；越权按“ProductDeviceProjectScope；V2只读”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD INT-06业务验收标准验证“外部申请/结果映射、回调和对账”；越权按“业务对象范围；完整授权码不可见”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD INT-07业务验收标准验证“批准费用出向、结果查询和人工对账”；越权按“OrganizationSupplierScope；财务结果复核”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试
- Phase 3验收断言：按PRD INT-10业务验收标准验证“节点通知、受理/送达分离和兜底”；越权按“业务对象接收人范围；模板变量白名单”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录
...
- Phase 3验收断言：按PRD NFR-01业务验收标准验证“性能水位、安全审计和兼容性验证”；越权按“TenantOrganizationProjectScope；服务端强制范围”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
...
- Phase 3验收断言：按PRD NFR-02业务验收标准验证“授权校验→下发→回调→业务消费→完成/失败；独立中心按成功回调终态”；越权按“BusinessObjectDeviceCredentialScope；创建人默认权限和五元组授权”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
...
- Phase 3验收断言：按PRD NFR-03业务验收标准验证“节点通知、受理/送达分离和兜底”；越权按“业务对象接收人范围；模板变量白名单”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；外部集成映射、超时/重试/对账/降级测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD AUT-01业务验收标准验证“申请、OA审批引用、外部授权确认和查询”；越权按“申请对象/设备/产品范围；授权码脱敏”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；脱敏请求响应、幂等键、重试/对账与降级记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- 授权与数据范围：申请对象/设备/产品范围；授权码脱敏
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；外部集成映射、超时/重试/对账/降级测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD AUT-02业务验收标准验证“申请、OA审批引用、外部授权确认和查询”；越权按“申请对象/设备/产品范围；授权码脱敏”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；脱敏请求响应、幂等键、重试/对账与降级记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD CHG-01业务验收标准验证“变更申请、审批、目标版本校验和执行”；越权按“TenantOrganizationProjectScope；变更对象权限”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试
- Phase 3验收断言：按PRD PLT-01业务验收标准验证“统一待办接入、完成回调Owner再校验”；越权按“TenantOrganizationProjectScope；待办责任人范围”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据
...
- Phase 3验收断言：按PRD PLT-02业务验收标准验证“初始化上传、内容校验、版本提交、引用和归档”；越权按“FileBusinessScope；下载实时回源业务权限”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试
- Phase 3验收断言：按PRD PROJ-12业务验收标准验证“组合成员、发布快照和下钻”；越权按“ProjectTreeScope；组合汇总不扩权”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试
- Phase 3验收断言：按PRD ANA-01业务验收标准验证“组合指标快照生成与只读展示”；越权按“OrganizationReportScope；下钻回项目权限”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试
- Phase 3验收断言：按PRD COM-01业务验收标准验证“ERP订单行同步、范围主记录及明细分配/释放、明细合计一致性和超分配门禁”；越权按“ContractProjectScope；ERP核心字段只读”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试
- Phase 3验收断言：按PRD SOL-01业务验收标准验证“Schema发布后只读、实例按版本校验”；越权按“ProjectStageScope；Schema维护与实例填写分离”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD IMP-01业务验收标准验证“提交→复核→整改→再复核”；越权按“ImplementationProjectBatchScope；复核权限”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；外部集成映射、超时/重试/对账/降级测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD RES-01业务验收标准验证“服务商档案、资质版本和可用状态”；越权按“OrganizationSupplierScope；资质文件字段权限”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；脱敏请求响应、幂等键、重试/对账与降级记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD ACC-06业务验收标准验证“交接门禁、静态交接快照和接收确认；不创建持续服务跟踪项”；越权按“ProjectStageScope；交接双方项目/服务范围”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；文件哈希、版本、扫描、引用与权限拒绝记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试
- Phase 3验收断言：按PRD SRV-01业务验收标准验证“客观维保/停产停维状态计算与提示”；越权按“ProjectDeviceScope；客观事实只读”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录
...
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试；文件上传/下载/版本/恶意内容与权限回源测试
- Phase 3验收断言：按PRD AST-01业务验收标准验证“RMA替换、设备归属校验和维保事实衔接”；越权按“ProjectDeviceScope；设备与来源范围”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录；文件哈希、版本、扫描、引用与权限拒绝记录
...
- 授权与数据范围：ProjectDeviceScope；客观事实只读
- Phase 3测试类别：业务规则/聚合单元测试；API契约与输入边界测试；服务端授权拒绝测试；状态/异常恢复测试；幂等与并发冲突测试；数据库约束与迁移测试；事件Outbox/Inbox、重复/乱序/重放测试；外部集成映射、超时/重试/对账/降级测试
- Phase 3验收断言：按PRD AST-02业务验收标准验证“客观维保/停产停维状态计算与提示”；越权按“ProjectDeviceScope；客观事实只读”拒绝，适用的非法状态、版本冲突或无效输入由对应业务守卫拒绝，且不产生业务副作用
- Phase 3证据类型：自动化测试报告（用例ID、业务对象ID、断言与结果）；数据库迁移/约束验证记录；事件消息ID、Outbox/Inbox及消费水位证据；脱敏请求响应、幂等键、重试/对账与降级记录
