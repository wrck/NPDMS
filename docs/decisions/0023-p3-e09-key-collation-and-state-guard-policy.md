...
ACCEPTED（Q01～Q08）；全量 Reviewer 签署待完成
...
当前DDL哈希`F788F5...C31DAA`下的数据架构技术不变量按推荐批量接受：
...
当前108个普通索引按推荐接受为SDS候选基线，而不是性能验收结论：
...
- 全量 Reviewer 对1,626项逐项登记的签署及非空`approvedDdlSha256`。
- 迁移Owner、源库水位、脏数据数量、对账结果、回退与切换运行证据。
- Q08要求的Feature查询计划与P3-E06近生产性能验证结果。

上述项目不得因Q01～Q08已经确认而推定通过。Q07/Q08解决模型选择问题，不替代独立复核、迁移运行证据或性能验收。
...
6. 机器校验必须精确识别Q07的222项技术约束和Q08的108项候选索引；DDL哈希、数量或分类变化时原决策自动失效并重新评审。
...
- Q07技术不变量已确认；Q08候选索引进入Feature查询计划和P3-E06压测闭环，不阻塞当前模型选择。
