...
PARTIALLY_EFFECTIVE：Q01～Q06继续有效；本ADR旧哈希下的Q07/Q08按第8节自动失效，ADR-0028保留历史确认锚点，ADR-0030覆盖当前六表差量。当前DDL与item集合已完成整体一致性独立复审，P3-E09为`MODEL_BASELINE_READY`；不以全量Reviewer签署作为模型输入前置条件。
...
- ADR-0028保留`5EB974...E4249`下Q07/Q08、Q09～Q14及V1.7的历史决策证据；ADR-0030精确接受六表差量，当前逐项寄存器`DEFER=0`。正式独立复审已GO、模型基线已发布，当前哈希整体一致性已关闭。
- P3-E09当前为`MODEL_BASELINE_READY`。只有Release包含历史迁移实施或数据切换时，`AI-MIG-000`才作为Release前置门禁，须经真实批次验证并只在批准窗口内执行；普通功能Release为`NOT_APPLICABLE`。
