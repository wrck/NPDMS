...
PARTIALLY_EFFECTIVE：Q01～Q06继续有效；本ADR旧哈希下的Q07/Q08已按第8节自动失效，当前哈希九组完整清单已由ADR-0028重新确认。P3-E09当前新候选待fresh review；上一轮GO不覆盖本轮复审校验变更，保持`MODEL_BASELINE_REVIEW_PENDING`；不再以全量Reviewer签署作为模型输入前置条件。
...
- 旧哈希下Q07技术不变量和Q08候选索引结论已因当前哈希变化失效；当前哈希下的Q07/Q08及Q09～Q14、V1.7已由ADR-0028接受并纳入待fresh review的候选。
- P3-E09当前模型不得作为SDS和后续Feature输入；`AI-MIG-000`、历史迁移实施和数据切换继续`OPEN`并受真实批次验证阻断。
