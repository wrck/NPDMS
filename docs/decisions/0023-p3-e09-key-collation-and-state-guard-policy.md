...
PARTIALLY_EFFECTIVE：Q01～Q06继续有效；本ADR旧哈希下的Q07/Q08已按第8节自动失效，当前哈希九组完整清单已由ADR-0028重新确认。P3-E09整体一致性独立复审已`GO`，当前为`MODEL_BASELINE_READY`；不再以全量Reviewer签署作为模型输入前置条件。
...
- 旧哈希下Q07技术不变量和Q08候选索引结论已因当前哈希变化失效；当前哈希下的Q07/Q08及Q09～Q14、V1.7已由ADR-0028接受并通过整体一致性独立复审。
- P3-E09当前模型已为`MODEL_BASELINE_READY`，可作为SDS和后续Feature输入。只有Release包含历史迁移实施或数据切换时，`AI-MIG-000`才作为Release前置门禁，须经真实批次验证并只在批准窗口内执行；普通功能Release为`NOT_APPLICABLE`。
