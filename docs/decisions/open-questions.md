...
- V1.7 DDL delta: ADR-0025；已按`ConfigurationCollectionResult`、`SatisfactionCollection`、`CutoverSupportTask`、`ResponsibilityInterval`、`HistoricalWorkOrderRecord`、`HistoricalTimeRecord`、`DeviceComponentRelation`、`DirectorySyncSnapshot`补齐精确13表差量。当前候选DDL为63表、1,242列、隔离MySQL登记335项约束，SHA-256为`03D0819B92964948044F186829D1F2E6A77C25A7B66126B12A5141D97535B125`，并已在MySQL 8.4.10隔离执行通过。该结果只补齐候选证据；P3-E09仍为`BLOCKED_BY_REVIEW`，等待全量Reviewer签署与`approvedDdlSha256`，不授权生产迁移、切换或旧`dppms`写入。
