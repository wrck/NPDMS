...
- Blocking scope: 仅阻断重复编号PRD修订成为全局基线、依赖该编号的全局验收引用定稿及整支合并；不阻断F-INS独立代码候选、适用测试或新DU内不依赖编号裁决的实施。 当前docs/prd-v1.8-p0-p1-fixes的SDS只消费PRD Blob 4b7bd7a4b099e18edb7e5a10c8c27615118c9d7b，不导入上述并行未裁决修订；其全局编号/整支合并阻塞继续保留，不作为本次局部分册内容验证的未决业务输入。
