...
1. 当前DDL、目标字段目录、对象—目标表映射、字段映射和约束清单由P3-E09模型基线保持一致；只有Release包含历史迁移或数据切换时，`AI-MIG-000`真实批次才须引用这些不可变制品及其哈希，并生成该批次的校验和release manifest。
...
- P3-E09模型基线与`AI-MIG-000`迁移执行授权分离；前者可作为SDS/Feature输入。只有Release包含历史迁移或数据切换时，`AI-MIG-000`才在Release前阻断对应操作并绑定批准窗口；普通功能Release不适用。
