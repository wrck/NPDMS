...
5. PLT加性公开`FileArtifactApi.archiveReferenceSets`窄接口：`ArchiveFileReferenceSetsCommand`包含`operationId/archiveBatchId/businessDecisionRef/actorUserId`、附件集合键、归档集合键、期望`scopeVersion`和完整期望公共文件事实。`actorUserId`只能取报告发布时已通过ACC服务端鉴权并冻结的认证用户；PLT通过SYSTEM `PermissionApi.hasAnyPermissions(actorUserId, "pms:file:archive")`重验既有功能权限，并按同一租户和该用户重验文件Owner范围。随后持锁重验附件ACTIVE集合，按每项相同artifactId/versionNo/referenceKey在`ACCEPTANCE_REPORT_ARCHIVE`集合创建独立归档引用，追加不可变`FileArchiveRecord(archivedBy=actorUserId)`并把归档引用置`ARCHIVED`；不得改变附件集合引用状态。整组同事务、同批同摘要幂等，部分集合、版本漂移、撤权或Provider不可用失败关闭；不得伪造Web登录上下文、使用当前Job线程用户或取消服务端鉴权。
...
10. 报告首次发布或替换时，ACC把服务端认证用户冻结为报告版本`publisher_user_id`并写入`AcceptanceReportVersionChanged.publisherActorUserId`；DRAFT不预填，撤销和补偿不得改写。ACC用`PlatformCommandExecutionApi`与报告事务同提交写Outbox；`AcceptanceReportOutboxDeliveryJob`通过`PlatformOutboxDeliveryApi`只领取`AcceptanceReportVersionChanged`，来源投影事务成功后才`markDelivered`，异常则先保留未交付状态并`scheduleRetry`。该Job不得领取或标记`ClosureGateRecheckRequested`已投递；CLO Feature未交付前该事件继续待消费。

## 物理差量

`acc_acceptance_report_version`加性保存`publisher_user_id bigint NULL`：DRAFT为空，首次发布或替换时写入服务端认证用户，EFFECTIVE/SUPERSEDED/REVOKED必须非空且不可改。`acc_acceptance_report_attachment`与`acc_project_deliverable_source_attachment`均保存`file_artifact_id bigint`、`file_version_no int unsigned`、`reference_key varchar(64)`、`artifact_version/reference_version/availability_version int unsigned`、`scope_version bigint`和`file_hash char(64)`；不保存PLT内部`file_version_id/reference_id`。来源版本、当前唯一和历史约束不变。
