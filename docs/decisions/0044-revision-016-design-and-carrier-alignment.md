...
> 证据：`docs/engineering/gates/phase-1/revision-016-gate-repair.md`

## 背景与本轮结论

PRD修订012/014/016已确认模板图、三类退出和同项目范围追加。旧SDS仍保留数字相邻推进、强制S6闭环及PM-06多期群组；旧校验器又固定匹配修订007批准和没有版本切片的Owner列，不能证明现行设计正确。
...
原CORE_MIGRATION_SUBSET DDL未改，旧P3-E09只证明其同哈希范围；参考Schema在隔离MySQL8.4.11创建并通过42项约束测试不等于存量升级、历史迁移、业务Provider、UI或生产验收。受影响对象的历史来源明确为已有证据或NONE_NEW，不按旧表名猜测恢复多期业务。
