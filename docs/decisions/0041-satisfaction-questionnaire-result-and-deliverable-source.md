...
> 受控grant上传Response身份补充：`PROPOSED_FOR_INDEPENDENT_REVIEW`<br>
> 日期：2026-08-30<br>
...
7. PLT加性公开受信`FileArtifactApi.initializeBusinessGrantUpload/completeBusinessGrantUpload/lockAndRevalidateBusinessGrantFiles`，仅供ACC已验证的ACTIVE访问授权上传签字或附件。ACC先以`questionnaireId+requestId`作为唯一Response预留意图，通过现有`PlatformCommandExecutionApi`在固定作用域`ACC_SATISFACTION_RESPONSE_RESERVATION`持久化服务端预分配`responseId`；同grant、同requestId和同规范化摘要重放返回同一ID，异摘要冲突。初始化返回该ID；后续上传与最终提交都必须重验该预留，最终Response直接使用它且不得再次生成ID。文件初始化另以`operationId`幂等并由PLT返回服务端槽位身份，客户端文件Fact只作待核对句柄。ACC文件策略Provider以授权版本、问卷/Response预留身份、租户和项目范围判定，PLT仍执行内容、大小、类型、病毒、引用版本和审计规则。内部现场协助继续使用现有认证上传REST。
...
23. grant专用文件策略不扩展通用`FileBusinessObjectPolicyQuery/FileBusinessObjectPolicyRevalidationQuery`。`FileBusinessObjectPolicyProvider`加性提供初始化、完成和最终文件集合锁定重验的类型化默认失败方法；PLT Registry仍按`ACC/SATISFACTION_RESPONSE`唯一分派。ACC Provider按grant→Questionnaire→Task锁序验证租户、版本、ACTIVE/有效期、用途、预留responseId、requestId、文件槽位及`ProjectScopeApi(PROJECT_EDIT)`，最终返回的规范文件事实必须精确匹配PLT实际Artifact/Version/Reference、grantId/version、responseId、policyKey、scopeVersion和服务端槽位。任一不一致在Response/ResponseFile写入前失败，不能直接信任客户端回传Fact。
24. `BusinessGrantUploadPolicyFact`冻结`grantIssuerUserId`，唯一来源为当前grant创建时保存的正式授权用户`creator`且必须可解析为正数；不得使用updater、客户输入、grantId或固定值代替。PLT只把该用户作为受控grant的内部责任/授权主体写入既有`created_by/creator/updater/OperationAudit.actorId`，审计detail固定记录`subjectType=BUSINESS_GRANT`、grantId、grantVersion、questionnaireId、responseId、policyKey和服务端槽位。外部提交主体仍是grant，不建立SecurityContext、不调用登录上传Controller、不把grant当用户ID，也不解释为客户拥有`pms:file:upload`。
