...
- 本ADR只批准物理模型设计，不创建或修改DDL/Flyway，也不声称P3-E09已覆盖新表。
- 进入Phase 3后，应以本ADR和SDS生成新的前向迁移，重新执行P3-E09模型一致性校验；只有对应Feature实现包含历史迁移或数据切换时，`AI-MIG-000`才在Release批准窗口内成为前置门禁。
