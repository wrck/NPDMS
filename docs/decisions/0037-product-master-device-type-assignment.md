...
- 来源分支只保留CUT消费端口；本次接收继续复用master已有F-AST-002实现，不复制或重做Owner。
- CUT单元/集成测试可在`src/test`显式组装受控替身，返回与公开结果同形的完整正向事实，以推进任务创建快照和P3匹配闭环。
- 替身不得进入生产装配，不得冒充AST权威事实或真实浏览器证据。
- 来源时点记录的“F-AST-002代码尚未集成”已由master后继接收覆盖；生产原子接线与真实运行仍按当前Feature复核。本次接收不自动放行生产Adapter、生产任务创建或Implementation Done，也不阻断已有CUT内核及聚焦测试。
