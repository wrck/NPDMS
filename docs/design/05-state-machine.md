...
## F-PROJ-008 通用相邻推进候选差量（IN_REVIEW）

| 当前阶段 | 唯一允许结果 | 所有权 |
|---|---|---|
| S0/S1/S2/S3 | 全部冻结EXIT Gate通过后进入相邻S1/S2/S3/S4 | F-PROJ-008通用推进 |
| S4 | 不由通用命令推进；进入S5走`enter-acceptance-stage` | F-COM-001专用路径 |
| S5 | 不由通用命令推进 | CLO闭环路径 |

通用成功在同一事务将当前Stage置`DONE`、下一Stage置`ACTIVE`并CAS更新Project.current_stage；不得跳级、指定非相邻目标、从非ACTIVE生命周期推进或用客户端门禁结论改变状态。业务未满足、依赖未知、事实版本冲突均保持原阶段不变。

已发布模板的S0～S3每个阶段必须至少有一个EXIT Gate，且每个EXIT Gate必须至少有一个受控引用；发布时不满足即整版拒绝。运行时实例缺Gate或缺引用属于数据/依赖损坏，稳定失败关闭，不使用“全部零项满足”的空集规则。
