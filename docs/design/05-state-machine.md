...
修订019成员操作边界：同次或分次指派、增补经理和主责切换均为项目级责任变化；各业务阶段可按权限办理，不新增或推进ProjectTask，不以固定任务状态作为前置。项目经理为多成员集合，保留一个当前主责；仅有效主责服务经理与当前主责项目经理均存在才为ASSIGNED。指派不修改current_stage/lifecycle_status，已关闭项目仍按原退出保护，不自动重开或回填历史。
