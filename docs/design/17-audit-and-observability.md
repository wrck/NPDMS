...
【方向已确认：P3-E05】按ADR-0004使用OpenTelemetry统一采集并接入企业现有后端。具体日志、指标、Trace、告警、安全事件后端、访问角色、留存和归档仍由运维/安全在部署或专项验收阶段登记；这些运行实例不前置阻断本逻辑设计候选，但未形成可复核证据前不得通过可观测验收或生产发布门禁。
