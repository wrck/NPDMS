...
> Requirement ID：NFR-01～03，以及全部100项正式Requirement、111个目标版本切片的业务审计、异常、集成、事件、性能和发布证据
> Owner：SDS Phase 3可观测性架构；业务审计事实仍归各Owner Context
> 前置设计：11、12、15、16、14分册

修订007差量可观测复核已完成：自动匹配、依赖维护、通知、KPI、导出、提前时间判断、ITR/OA协作和巡检超时统一记录`operationId/correlationId`、规则或配置版本、sourceKey、重试/对账及最终业务结果；通知送达和外部HTTP成功仍不等于业务完成，未新增PRD外指标或告警阈值。
...
【方向已确认：P3-E05】按ADR-0004使用OpenTelemetry统一采集并接入企业现有后端。具体日志、指标、Trace、告警、安全事件后端、访问角色、留存和归档仍由运维/安全在部署或专项验收阶段登记；这些运行实例不前置阻断本逻辑设计基线，但未形成可复核证据前不得通过可观测验收或生产发布门禁。
...
| 共享动态表单 | 模板创建/元数据更新、草稿保存、发布、启用/停用、手工实例及业务实例创建/值保存/复制 | template/revision/instance/Owner ID、动作、前后状态/版本、引擎版本、变化字段键、文件引用身份摘要和operationId；消费Context记录业务前后事实，PLT仅为实际新增引用写事件；不复制完整函数源码、富文本、接口/iframe正文、完整表单值或文件正文 |
| 实施就绪快照 | 评估、发布、查询拒绝、CUT锁定重验 | snapshot/project/device/solution稳定ID与版本、EXE-01～04来源事实版本/水位、`READY/NOT_READY/STALE`、未满足项、操作者与时间；不记录文件、配置Log、凭证或附件正文 |
...
| F-PLT-002审计 | 创建/修改/发布/启停模板并创建、保存手工实例，核对成功、拒绝、重放和CAS冲突 | 成功事实含受控前后摘要与operationId，重放不重复；审计、日志和Trace不含完整模板代码、富文本、接口响应、iframe或文件正文 |
...
逻辑设计、留存/采样策略和验收契约已完成修订007差量复核并纳入SDS基线。P3-E05具体后端与实测证据在部署/可观测验收/生产发布前关闭，不前置阻断逻辑设计基线。
