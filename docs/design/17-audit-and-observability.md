...
> Requirement ID：NFR-01～03，以及全部115项需求的业务审计、异常、集成、事件、性能和发布证据
...
| 项目治理 | 创建、层级移动、模板发布、负责人/成员、回退、关闭、PM-05/06关系 | 前后父节点/版本、来源/目标、逐项结果 |
...
业务审计与业务事务原子提交；跨Context过程各Owner记录自己的动作并共享correlationId。审计记录追加写，业务用户和普通管理员无修改/删除API；Word正文不做内容级审计。

### 5.2 审计查询

- 审计人员按授权tenant、时间、主体、对象类型、动作和结果查询；敏感对象遵守字段级脱敏。
- 业务管理员只能查询本人管理范围的业务留痕，不能查询凭证取密详情、跨项目拒绝或安全事件原文。
- 导出审计本身产生审计；大范围导出使用异步任务、范围快照和文件权限。
...
- 采样策略【待P3-E05登记】；无论常规采样率如何，错误、关键状态、凭证安全、高风险拒绝和发布迁移必须保留可关联证据。
...
日志、Trace、指标和审计的在线留存、归档、删除、法律/合同要求【待P3-E05由安全/运维/业务Owner确认】。任何留存策略不得允许普通业务用户删除审计或使秘密进入低保护归档。

## 10. 验证矩阵

| Requirement/Gate | 操作 | 通过标准 |
|---|---|---|
| NFR-01审计 | 执行登录、权限、层级、审批、凭证、文件、集成补偿和状态命令 | 审计字段完整、前后值/快照可追溯、业务用户不可修改删除 |
...
| Telemetry后端、访问、留存和告警Owner | BLOCKED_BY_EVIDENCE（P3-E05） |

逻辑设计可供18～20分册使用；P3-E05关闭且Telemetry实测通过前，本分册不得转为`BASELINE`。
