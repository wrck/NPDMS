...
| CutoverSpareApplicationReference | CUT-08只保存CUT平台请求意图、INT-06外部请求标识和外部申请号引用；首次申请号绑定必须匹配受信租户、平台请求、外部系统和外部请求标识，不承载库存、审批、到货或领用生命周期 |
| CutoverSpareStatusRevision | 外部申请原始状态按正数来源版本只追加；同版本异载荷冲突、旧版本不回退当前投影，CUT不解释为本地备件业务状态 |
| CutoverSpareManualEvidence | 只追加PLT不可变文件事实引用与人工说明；人工证据不得生成外部申请号、状态版本或P6门禁事实 |
