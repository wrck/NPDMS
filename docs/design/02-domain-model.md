...
| Acceptance | ACC初验/终验活动根与PROJ `ProjectTask/WorkBinding`保持稳定身份关联但不复制任务真值；报告草稿形成只追加有效版本，同项目同类型仅一个当前有效版本，换版或撤销只关闭当前区间并保留历史；活动完成时当前版本必须具备验收时间、结论、验收人和固定文件版本附件；项目进入验收阶段不创建或补齐报告，报告状态不得触发或反推验收范围绑定 |
