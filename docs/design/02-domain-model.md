...
| Acceptance & Closure | 培训、验收、满意度收集、交付件齐套校验、审核、统一归档和闭环交接；以独立于初验/终验报告的不可变绑定记录项目阶段驱动进入验收范围的DeliveryScope分配版本 | 项目阶段快照、实施证据、方案、问题、文件、DeliveryScope公开契约 | 外部合同/财务事实、现场实施原始证据、COM交付范围主记录、PROJ项目阶段事实 |
| Cutover | CUT-01核心任务、问卷评估、调研清单、CUT-07后台配置版本、方案、分级审批和P6闭环结果 | 项目、设备、采集任务、基础平台字典 | 采集执行引擎、设备凭证明文、通用工单及WO-06保障工单 |
...
| Customer & Relationship | 客户同步副本、临时客户、联系人、客户关系和服务等级时态版本 | 合同、项目、设备、基础平台等级字典 | CRM权威字段、项目交付状态、历史业务等级快照 |
| Asset Management | 设备同步副本、设备档案、归属、维保基本信息、ConfigurationLog原始文件、不可变解析版本和设备关联 | 项目、客户、实施采集业务结果 | MES/ITR权威字段、项目交付状态 |
| Contract & Fulfillment | ERP合同、销售订单、订单行同步副本及项目交付范围分配；冻结分配发生时目标项目所属SYSTEM办事处快照 | 项目、SYSTEM组织部门、ERP来源版本、验收范围锁定事实 | 财务系统账务、AST站点事实；当前不建立独立履约回写/对账业务聚合 |
| Supplier & Subcontract | 服务商、转包、付款门禁 | 项目、合同、问卷 | 财务付款结果 |
| Analytics | 指标快照、组合视图 | 各域只读事实 | 任何业务交易 |
| 基础平台能力（非业务 Context） | 待办、文件、授权、变更、字典、审计、共享动态表单模板修订与通用实例值 | 各域标识和事件 | 业务域聚合内部状态、业务完成/审批/版本语义 |
...
| CutoverConfigurationRevision | CUT-07配置按草稿→已发布→已停用受控流转；已发布版本不可覆盖，发布前校验稳定编码、字典引用和动态绑定条件，已生成业务实例继续按消费版本解释 |
| CutoverClosure | 保存P6四类结果、回退说明、附件、遗留项文本及INT-12证据引用；不建立逐步骤执行、稳定观察或遗留项独立生命周期 |
| InspectionTask | 在线/离线模式互斥；规则版本冻结到任务；报告生成和问题闭环可追溯 |
| SatisfactionCollection | ACC拥有问卷模板修订、领域任务、受控访问授权、冻结问卷、客户答卷、签字/附件公共文件事实、不可变判定和整改版本；PROJ只拥有ProjectTask、冻结业务时点和项目范围；客户提交与达标不由Todo、送达或旧回访状态替代；未达标/失效必须整改后创建新任务、新问卷和新判定 |
| Acceptance | ACC初验/终验活动根与PROJ `ProjectTask/WorkBinding`保持稳定身份关联但不复制任务真值；报告按DRAFT→EFFECTIVE形成只追加版本，替换把旧当前置SUPERSEDED，撤销把当前置REVOKED且不恢复旧版，同项目同类型至多一个当前有效版本；活动完成时当前版本必须具备验收时间、结论、验收人和固定文件版本附件集合；项目进入验收阶段不创建或补齐报告，报告状态不得触发或反推验收范围绑定 |
...
| ConfigurationCollectionResult | 配置Log采集业务结果、实施解析状态、结果引用 | ConfigurationLog原始文件及不可变解析版本、设备连接执行、凭证明文、通用任务下发 | 采集执行由外部平台负责；IMP发布业务结果，AST/EQP-02统一管理配置Log |
