...
F-CUT-007割接首页KPI复用`pms:cutover-task:query`并先取当前用户`ProjectScopeApi.ACTION_VIEW`可见项目。查询权只决定KPI可见范围，不自动产生待办；待办还必须复用P2～P6当前服务端写动作守卫，P5固定为现有`myTodos`资格与KPI可见范围的交集。空项目集合返回全0且不得扩大查询，Owner不可用或事实损坏时整份KPI失败，不返回部分计数。
...
- CUT-05审批投影分为`FULL`、`FINAL_RESULT_ONLY`与`REASSIGNMENT_ONLY`：原发起人具备当前项目`ACTION_VIEW`或当前节点审批人通过节点Owner/项目范围重验时才可读取完整冻结审批页；其他只读项目成员仅在终态读取不含来源快照、候选事实和评审正文的最终结果。审批管理员通过独立改派候选队列与REASSIGNMENT_ONLY详情只读取实例、任务、未处理节点、当前指派及版本元数据，改派响应仍使用该投影；权限本身不授予完整业务详情。当前审批人的通过/驳回还须具备功能权限、根/节点可执行状态和节点特定范围（发起人`ACTION_EDIT`，其余`ACTION_VIEW`）；审批管理员只有改派权，不能代理审批。
