...
### 2.1.1 COM-01合同管理员公司范围

- SYSTEM拥有`UserCompanyDepartmentScope`，COM通过现有`OrganizationScopeApi.getActiveScopes(subjectUserId)`读取当前租户、当前时点的有效授权行；COM不得读取SYSTEM表、复制有效期算法或新增合同授权事实。
- `ContractProjectScope`对合同管理员的合同侧范围，是所有返回行中非空`companyCode`按原值精确去重后的集合。部门ID/编码、部门树、主范围标记和`scopeRole`只作授权上下文，不扩大或缩小该公司集合；项目关系和DeliveryScope不得反推首次合同可见性。
- 合同目录、详情、销售订单、订单行和项目—合同关系维护统一使用该公司集合。列表查询必须把非空集合转换为场景化Query的必选公司编码条件，使用精确字符串相等；空集合返回空。详情和写操作必须同时命中合同当前ERP所属公司编码，否则按无权处理且不泄露对象是否存在。
- SYSTEM返回空、异常、超时、空公司编码或无法取得当前事实时不回退为租户全量、组织树或既有项目关系：列表返回空并记录Owner不可用审计，详情/写操作拒绝。若部分返回行无有效公司编码，仅这些行不授权；其余有效行仍按公司编码并集生效。
- 项目—合同关系写入在业务写入前重新调用`getActiveScopes`；命中同一公司多行时，把按scope ID稳定排序的全部命中`scopeId/version`记录到既有`AuditRecord.authorizationSnapshot`，不在关系表保存第二份SYSTEM事实。写后撤权不删除既有关系或历史，但下一次查询和维护必须按新当前范围重新判定。
- 功能权限、公司数据范围、项目范围和字段权限独立。合同管理员查询仍需`pms:commerce:contract:query`，关系维护仍需`pms:commerce:contract:relate`；合同金额及已标记商务敏感字段的明文另需`pms:commerce:contract:sensitive-read`，否则列表和详情脱敏或不返回。敏感错误不得包含金额、订单数量、项目关系或命中的scope ID。
...
| 合同管理员无当前公司范围或OrganizationScopeApi不可用时查询合同目录 | 返回空并记录授权Owner不可用，不省略公司条件、不泄露合同存在性 |
| 合同管理员使用过期scope版本建立项目—合同关系 | 写前重新回源后拒绝，不写关系、成功幂等或成功审计 |
| 仅有合同查询或公司范围但无敏感字段权限读取合同金额 | 脱敏或不返回，不通过详情、错误或导出旁路泄露 |
