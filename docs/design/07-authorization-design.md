...
| CutoverTask | 一线工程师办理来源任务或自建；集成账号仅按来源契约创建 | 一线工程师提交P2问卷/等级、填写P3/P4和P6；仅可操作授权项目与设备 | P5用服经理必须来自PROJ当前唯一PRIMARY `SERVICE_MANAGER_L1/L2`冻结事实；零个或多个候选时不任选人员；A/B按冻结路由增加二线/研发 | 项目树、设备当前归属、CUT任务、方案版本、审批节点；全局角色或项目范围不能单独替代服务经理Owner事实；项目只读角色不可编辑 |
...
- 巡检规则维护与发布分权：巡检-管理员维护本租户草稿；`pms:inspection-rule:security-review`专用权限表达PRD审批/任务角色组的审核资格。租户访问拦截器完成目标租户上下文切换后，Inspection从受信认证上下文取得当前审核人，直接调用System现有`PermissionApi.hasAnyPermissions`对该专用权限重新判定；目标租户角色—菜单授权或System超级管理员任一返回`true`均允许记录审核。`SecurityFrameworkService`的`skipPermissionCheck`只服务租户切换，不能单独作为审核结论；`false`、无上下文或System异常失败关闭。审核事实记录`RBAC_PERMISSION`，布尔结果不提供贡献路径且可能来自超级管理员，因此`authorizationSourceId`为空，不伪造角色—菜单来源。维护权与发布权不自动替代专用审核权限。具有独立发布权限的巡检-管理员仅在最后审核结论有效且内容摘要一致时发布。发布时由Inspection专用只读适配器按受信租户调用AST产品类型查询，未知、停用或契约不可用时失败关闭；工程师选择规则时按当前用户授权设备范围查询产品类型，无权设备不泄露存在性。Inspection不直读`system_*`或AST业务表，不新增Yudao System接口，只保存稳定产品类型编码和发布名称快照。
...
| 二级服务经理读取其他实施地点 | 拒绝 |
| 租户访问已切换但审核入口只命中`SecurityFrameworkService`的`skip` | 拒绝记录审核；在目标租户上下文直接调用`PermissionApi`重新判定专用权限 |
| 目标租户普通用户无专用权限，或System权限调用异常 | 失败关闭，不追加审核事实 |
| 目标租户超级管理员命中现有布尔权限接口 | 允许审核；记录`RBAC_PERMISSION`且`authorizationSourceId`为空 |
...
依据修订019，成员指派/调整是项目级操作：支持服务经理与项目经理同次指派或先服务经理后单独指派项目经理，在项目各业务阶段校验对应功能权限、项目范围及SYSTEM有效人员资格，不以S0、T-ASSIGN-PM、已有项目经理或任务完成为操作前置。联合请求逐项满足各类角色操作权限，不能借组合入口扩大权限。

同一项目可有多名项目经理，并保留一个当前主责项目经理。完整成员集合与当前主责是不同事实：主责指针不能代表全部经理名单；主责切换不自动删除其他成员，历史角色/审批快照不回写。同一项目内相同项目角色操作权限相同，主责与其他有效PROJECT_MANAGER成员不因主责标识获得不同权限；授权必须核对有效角色成员，不能仅比较manager_id。Q-FPROJ-010已确认同公司、可跨部门的有效在职人员，并通过SDS10上游通用公司/角色用户资格查询筛选；不限定同办事处。现有租户、数据范围和关闭状态保护保持生效。

## 修订018：独立验收与业务视图授权

Requirement：PM-03、PM-11、ACC-03、COM-01。终验独立于阶段/任务，不独立于租户、项目及业务范围权限。

- ACC独立入口、Stage绑定入口、Task绑定入口和审批/业务事实触发均调用ACC受控命令。可信主体、项目/业务范围、实体状态及文件权限在服务端校验；存在任务来源时继续校验适用任务范围，不把特定ProjectTask存在作为所有终验的通用身份前提。
- WorkBinding/PermissionPolicy只声明所需操作，不授予权限；BusinessView只呈现交集后的允许动作，不按组件可见性判定授权。
- 自动触发必须明确定义执行身份及其来源和适用范围，不能借用Job账号、事件中的任意用户ID或伪造Web登录上下文。具体身份契约纳入Q-TPLACC-001，不擅自创建新角色或授权模型。
- 一节点一个当前主绑定保持不变。多个节点引用同一结果不自动获得该实体编辑权；复用视图不复制实体，也不产生第二套导航和完成事实。
- 项目级独立办理重验项目编辑范围和ACC操作权限；从Stage/Task办理还须与该节点当前允许动作求交集。COM是否作为业务证据依赖由规则决定，不改变项目授权要求；配置零附件也不允许伪造文件或越过实际附件集合授权。
- 新来源的COMPLETED只保存历史活动完成事实，不自动禁止所有报告换版，也不自动赋予换版/撤销权；具体命令仍由ACC按项目可编辑性、当前身份、期望版本和报告状态校验。归档补偿沿用报告原发布人并重验权限，不能用其他关系已归档或Job身份替代。
