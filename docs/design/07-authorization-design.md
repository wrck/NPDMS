...
| DynamicFormTemplate / ManualInstance | 具备模板维护权限的管理员创建/编辑草稿；获权用户从当前启用发布修订创建手工实例 | 具备独立发布权限的高信任管理员发布、停用/启用；手工实例仅创建者按CAS编辑 | 不新增模板审批节点；发布命令只执行结构、版本和权限校验 | 租户隔离；模板发布权限允许发布客户端事件/函数/API/iframe配置，但不授予任何目标业务接口或数据范围权限 |
