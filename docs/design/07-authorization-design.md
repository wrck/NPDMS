...
同一项目可有多名项目经理，并保留一个当前主责项目经理。完整成员集合与当前主责是不同事实：主责指针不能代表全部经理名单；主责切换不自动删除其他成员，历史角色/审批快照不回写。同一项目内相同项目角色操作权限相同，主责与其他有效PROJECT_MANAGER成员不因主责标识获得不同权限；授权必须核对有效角色成员，不能仅比较manager_id。Q-FPROJ-010仅确认项目经理候选组织范围，不把PM-08的公司+办事处过滤照搬给PM-01。现有租户、数据范围和关闭状态保护保持生效。
