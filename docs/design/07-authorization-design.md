...
> 文档状态：`BASELINE`
> 适用基线：PRD V1.8及批准修订001—013；巡检授权差量引用`CHG-PRD-2026-09-02-013`
...
- 巡检规则维护与发布分权：巡检-管理员维护本租户草稿；`pms:inspection-rule:security-review`专用权限包是审批/任务角色组的正式机器映射；安全审核主体必须取得System公开只读事实API返回的当前认证租户、当前认证用户经有效用户—角色—菜单关系对该单一精确权限码的显式`RBAC_PERMISSION`事实，超级管理员身份、`skipPermissionCheck`、维护权或发布权均不得隐式放行。System拥有并校验`system_*`权限事实，Inspection只消费公开契约，不跨模块查表或硬编码角色；System自然取得稳定`userRoleId`与`roleMenuId`时，`authorizationSourceId`保存为`RBAC_ROLE_MENU:{userRoleId}:{roleMenuId}`，确实无法稳定提供关系主键时为空，不得以角色编码、角色名称或猜造值替代。审核结论对同一revision和当前内容摘要按`reviewed_at DESC, id DESC`最后一条事实生效，仅当前结论为`PASSED`时允许发布；历史事实不可覆盖，权限后续撤销不追溯改写既有审核。发布时由Inspection专用只读适配器建立ADR-0036定义的受控进程内服务调用主体，并按当前租户调用AST产品类型编码批量查询，未知、停用或契约不可用时失败关闭；工程师选择规则时由Inspection服务端解析当前用户为委托用户，AST按该用户授权设备范围查询当前产品类型，无权设备不泄露存在性。服务主体权限不能替代委托用户范围，委托用户范围也不能替代服务主体；Inspection只保存稳定编码和发布名称快照，不因快照产生AST设备或产品类型访问权。
...
- AST产品类型受控导入由独立`pms:asset-product-type:controlled-import`功能权限控制，只授予当前已认证维护用户并由服务端取得租户和操作者；该权限不默认绑定任何角色，不复用旧`pms:equipment:update`或普通设备更新权限，Inspection只读主体不得执行导入。
...
| 缺少ADR-0036受控调用主体、主体动作不匹配或尝试覆盖租户 | 拒绝，不查询或返回产品类型事实 |
| Inspection服务调用主体批量校验其他租户产品类型编码 | 拒绝，不返回名称、来源版本或存在事实 |
| 已认证工程师按空范围、跨租户或无权设备选择巡检规则 | 返回空，不泄露设备及产品类型存在性 |
| 按设备查询缺少或非法委托用户 | 失败关闭，不把服务主体当作工程师范围 |
| AST产品类型未知、停用或查询契约不可用时发布/选择规则 | 失败关闭，不使用旧字典、自由文本、`conpType`或名称快照替代当前事实 |
| 安全审核仅由超级管理员、skip上下文、维护权或发布权命中，或System显式授权事实契约不可用 | 拒绝记录审核并失败关闭，不直读`system_*`表或提供默认放行 |
| 同一revision与摘要最后一条审核为`REJECTED`，但历史存在`PASSED` | 拒绝发布；历史`PASSED`不覆盖最后结论 |
| 任务使用过期授权 | 拒绝下发 |
