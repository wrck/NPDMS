...
- 巡检规则维护与发布分权：巡检-管理员维护本租户草稿；PRD定义的审批/任务角色组只能对当前规则revision的命令与正则记录安全审核结论；具有独立发布权限的巡检-管理员仅在审核结论有效且内容摘要一致时发布。发布时由Inspection专用只读适配器按受信租户调用AST产品类型查询，未知、停用或契约不可用时失败关闭；工程师选择规则时按当前用户授权设备范围查询产品类型，无权设备不泄露存在性。服务主体权限不能替代委托用户范围，Inspection只保存稳定编码和发布名称快照。
