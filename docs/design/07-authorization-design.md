...
- CUT-05审批详情分为`FULL`与`FINAL_RESULT_ONLY`：原发起人具备当前项目`ACTION_VIEW`或当前节点审批人通过节点Owner/项目范围重验时才可读取完整冻结审批页；其他只读项目成员仅在终态读取不含来源快照、候选事实和评审正文的最终结果。当前审批人的通过/驳回还须具备功能权限、根/节点可执行状态和节点特定范围（发起人`ACTION_EDIT`，其余`ACTION_VIEW`）；审批管理员只有改派权，不能代理审批。
