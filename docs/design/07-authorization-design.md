...
- 巡检规则维护与发布分权：巡检-管理员维护本租户草稿；PRD定义的审批/任务角色组只能对当前规则revision的命令与正则记录安全审核结论；具有独立发布权限的巡检-管理员仅在审核结论有效且内容摘要一致时发布。发布时由Inspection服务身份按当前租户调用AST产品类型编码批量查询，未知、停用或契约不可用时失败关闭；工程师选择规则时由服务端合并本人授权设备范围，并通过AST按授权设备查询当前产品类型，无权设备不泄露存在性。Inspection只保存稳定编码和发布名称快照，不因快照产生AST设备或产品类型访问权。
...
| Inspection服务身份批量校验其他租户产品类型编码 | 拒绝，不返回名称、来源版本或存在事实 |
| 工程师按空范围、跨租户或无权设备选择巡检规则 | 返回空，不泄露设备及产品类型存在性 |
