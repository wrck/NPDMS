...
## F-PROJ-008 阶段推进控制点候选差量（IN_REVIEW）

`POST /api/v1/pms/projects/{id}/actions/advance-stage`必须同时校验现有 `pms:project:update`、`ProjectScopeApi.ACTION_MANAGE`（`PROJECT_MANAGE`）对目标项目的当前范围事实以及当前有效 `PROJECT_MANAGER`关系。tenant与actor取服务端认证上下文；全局角色、历史成员、前端按钮可见性或单独拥有其中任一条件均不能替代三项合取。角色—权限映射继续由正式授权配置维护，不新增权限键。
