...
| `ddl-drift-review.md` | 当前 DDL 哈希`D87DD2...2B01D5`与旧目录哈希`2B2069...BF33`不同；ADR-0019完成名称归一化，ADR-0020固化项目编码命名空间，ADR-0021固化客户市场行业四维分类及迁移映射；其他约束、表选项及最终Reviewer签署仍为`DEFER` | 数据概念与已确认业务规则可继续设计；任何实际 DDL/迁移发布必须先完成`AI-MIG-000`剩余裁决 | 仅凭字段目录重建或旧`passed=true`宣称当前迁移可执行 |
