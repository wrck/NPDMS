...
同一Project可以有多个PROJECT_MANAGER关系，各成员独立保存生效区间；保留一个当前主责引用，必须指向有效经理成员。当前主责及其显示投影不能取代成员集合；主责切换、增补或调整分别保留历史，不隐式移除其他经理，不重新绑定任务。联合服务经理/项目经理操作由PROJ在同一事务维护两类责任和双主责状态。主责标识不附加权限，同一项目角色操作权限相同；候选范围尚见Q-FPROJ-010，不在数据模型中臆造岗位/角色层级。
