...
| `ddl-drift-review.md` | 当前核心迁移 DDL 哈希`8F3CF5...50930`与旧目录哈希`2B2069...BF33`不同；ADR-0019完成名称归一化，ADR-0020固化项目编码命名空间，ADR-0021固化客户市场行业四维分类，ADR-0022明确核心子集、跨域逻辑引用和V3表排除；隔离MySQL 8.4.10执行PASS，其他约束、表选项及最终Reviewer签署仍为`DEFER` | 数据概念与已确认业务规则可继续设计；任何实际 DDL/迁移发布必须先完成`AI-MIG-000`剩余裁决 | 把核心迁移子集误称为平台全量模型，或仅凭语法执行成功宣称当前迁移已获业务批准 |
