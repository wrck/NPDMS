...
| `ddl-drift-review.md` | 当前 DDL 哈希`9CAE49...D1411`与旧目录哈希`2B2069...BF33`不同；经ADR-0019名称归一化后表/字段集合一致，约束、表选项和最终Reviewer签署仍为`DEFER` | 数据概念与已确认业务规则可继续设计；任何实际 DDL/迁移发布必须先完成`AI-MIG-000`剩余裁决 | 仅凭字段目录重建或旧`passed=true`宣称当前迁移可执行 |
