...
| `ddl-drift-review.md` | 当前核心迁移 DDL 哈希`F788F5...C31DAA`与旧目录哈希`2B2069...BF33`不同；ADR-0019～ADR-0023已落地Q01～Q08，Q07接受222项技术约束，Q08接受108项候选索引并保留Feature/P3-E06性能验证；隔离MySQL 8.4.10执行PASS，最终Reviewer签署仍为`REVIEW_PENDING` | 数据模型已完成需求方选择，可进入独立复核；任何实际 DDL/迁移发布必须先形成`approvedDdlSha256`并完成对应运行证据 | 把候选索引误称为性能已验收，或仅凭语法执行成功宣称当前迁移已获批准 |
