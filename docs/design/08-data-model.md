...
### 修订019 PM-01项目经理成员与主责

同一Project可以有多个PROJECT_MANAGER关系，各成员独立保存生效区间；保留一个当前主责引用，必须指向有效经理成员。当前主责及其显示投影不能取代成员集合；主责切换、增补或调整分别保留历史，不隐式移除其他经理，不重新绑定任务。联合服务经理/项目经理操作由PROJ在同一事务维护两类责任和双主责状态。主责标识不附加权限，同一项目角色操作权限相同；候选范围按已关闭Q-FPROJ-010及SDS10通用公司/角色查询，不在数据模型中臆造岗位/角色层级。
...
| DeliveryScopeDetail | 交付范围按产品/设备类型、序列号和批次拆分的明细 | 一个当前DeliveryScope可包含多条明细；明细数量合计必须等于主记录分配数量。地点唯一冻结在DeliveryScope主记录的目标项目办事处部门发生时快照；AST站点/位置由IMP/AST实施地点能力独立维护。形成独立负责人、计划、验收或闭环边界时，不继续堆叠明细，而是分配到独立子项目 |
