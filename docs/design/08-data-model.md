...
| `ddl-drift-review.md` | 当前 DDL 哈希`528E6F...35DAA3`与旧目录哈希`2B2069...BF33`不同；ADR-0019名称归一化后，ADR-0020明确新增3个项目编码字段和4个相关约束，其他约束、表选项及最终Reviewer签署仍为`DEFER` | 数据概念与已确认业务规则可继续设计；任何实际 DDL/迁移发布必须先完成`AI-MIG-000`剩余裁决 | 仅凭字段目录重建或旧`passed=true`宣称当前迁移可执行 |

数据元、旧库结构和 PRD 发生冲突时：业务语义以 PRD/批准决策为准；旧库物理事实以最终只读一致性抽取为准；无法裁决的记录进入迁移问题，不由实现按字段名、后缀、最大 ID 或任意候选值猜测。

### 1.2 迁移证据反推的核心业务不变量

1. 客户、联系人、项目的编码、名称、行业、联系方式、客户项目名称、办事处、公司和部门是独立语义；项目行业不得代替客户行业，公司与部门必须作为同一业务上下文关系共同解析。
...
7. 主 SN—附加 SN 是合同维度的设备关系；附加 SN 是独立设备身份，不能作为主设备第二物料字段。RMA 替换关系与附加 SN 关系类型分离。
8. 旧状态值、旧审计字段和旧主键只形成映射/血缘证据；不直接成为新状态代码、新平台审计主体或目标主键。
...
| DeliveryScope | 订单行到项目/设备/阶段的范围分配 | 对应历史迁移语义`ProjectOrderLineScope`；至少保存订单行、项目、分配数量、范围状态和来源证据。同一订单行可拆分多个项目但有效分配量不得超配；缺数量为待补数量，不计入交付统计；分配、释放均留历史 |
