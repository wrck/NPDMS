...
F-PROJ-002只消费DeliveryScope的版本化查询、预览和分配公开契约。拆分可按订单行、数量、子项目办事处部门编码和SN自由组合；COM必须通过PROJ公开契约取得目标项目及其当前SYSTEM部门事实并冻结发生时快照，不调用AST地点契约，也不得从地址、站点、名称或订单字段推断办事处。COM校验有效订单量、其他项目有效分配量、单位精度、取消/退货/ERP减量和并发版本，PROJ不得直接访问COM表。ERP权威数量缺失时范围状态为待权威确认，不计入可分配量，也不得为完成项目拆分而臆造数量。
