...
| 聚合/实体 | Owner 事实 | 不变量 |
|---|---|---|
| Acceptance | 初验/终验活动根、不可变报告版本、验收时间、结论、验收人和固定附件版本引用 | 活动根以PROJ ProjectTask/WorkBinding为唯一外部身份，ACC不写PROJ任务；进入验收阶段不创建报告，活动完成时对应当前有效报告必须四项完备；报告不拥有项目阶段或验收范围，验收结论不覆盖现场原始证据 |
...
| DeliveryArtifact | `acc_project_deliverable`应交实例、来源记录/版本、固定文件版本、齐套结果、审核、归档和补偿状态 | F-ACC-001只维护初验/终验来源；来源版本变化只切换当前引用并保留历史，归档失败保留有效报告且标记待补偿；不得形成第二应交清单或修改来源文件历史 |
