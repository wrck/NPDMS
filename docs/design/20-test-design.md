...
> 文档状态：`BASELINE`
> 适用基线：PRD V1.8修订013、SDS Phase 1/2/3 BASELINE；巡检规则安全差量引用`CHG-PRD-2026-09-01-011`，名称唯一性差量引用`CHG-PRD-2026-09-01-012`，安全审核生效与显式RBAC授权事实引用`CHG-PRD-2026-09-02-013`
...
| INS-03@V2 / INS-09@V2消费 | 稳定身份创建要求检测ID和规则名称且名称裁决不变；草稿可保存其余字段为空或不完整；发布要求八字段、命令、`NUMBER`阈值、适用产品和当前摘要安全审核全部完整；同revision同摘要多次审核按`reviewed_at DESC, id DESC`最后结论生效，仅最后`PASSED`允许发布；System显式授权事实必须来自当前认证租户、当前认证用户的有效用户—角色—菜单关系，专用权限包作为审批/任务角色组机器映射；多路径按稳定顺序择一，跨租户/用户代理查询、超级管理员、skip、维护权和发布权不自动放行，契约异常失败关闭；发布重新校验AST当前存在/启用并冻结稳定编码与显示名称；工程师按授权设备当前产品类型选择；未知、停用、来源缺失或契约不可用失败关闭；旧发布revision不回写 | 草稿空值/部分填写与发布完整性对照、`NUMBER`正向及非`NUMBER`负向、`PASSED→REJECTED`与`REJECTED→PASSED`最后事实测试、同时间以ID兜底测试、摘要变化重新审核、权限撤销不改写历史、System显式事实Consumer contract、当前身份绑定、多路径稳定择一、超级管理员/skip/无关系/停用关系/契约异常负向、发布revision及产品类型快照、选择请求的设备范围证据、AST故障注入、发布/选择前后数据库对照及旧revision查询结果 |
