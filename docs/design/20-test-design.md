...
| SOL | 工期/计划/方案提交审批、业务版本冻结、共享动态表单修订引用、文件引用和退回历史；业务完成/审批不得由PLT实例保存代替 |
...
| PLT/KNO/ANA | 待办不代业务成功、文件身份、授权、变更、ITR公告只读、指标/组合不回写交易事实；共享动态表单完整FormCreate配置、不可变发布修订、停用/启用、手工实例CAS、FileArtifact字段与普通上传值边界 |
