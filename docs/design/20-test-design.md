...
验证成员名单、主责ID/姓名/工号显示与服务端权限事实一致；验证主责和非主责的有效PROJECT_MANAGER在同一项目具有相同角色操作权限，缺少角色或范围仍拒绝；不能只用manager_id决定权限。候选范围正反向用例待Q-FPROJ-010具体化。无对应权限、跨租户/越界、无效候选、关闭项目保护、审计/Outbox写入失败及通知投递失败分别验证。成员操作不创建/完成ProjectTask，不修改阶段或历史批准事实；已有TASK_NATIVE及其他Owner绑定仍按其冻结规则工作。当前只有设计要求，未执行业务API、MySQL或浏览器验证，不宣称Feature Done。
