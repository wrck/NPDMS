...
| INS-03@V2 / INS-09@V2消费 | 草稿可离线编辑；发布重新校验AST当前存在/启用并冻结稳定编码与显示名称；工程师按授权设备当前产品类型选择；未知、停用、来源缺失或契约不可用失败关闭；旧发布revision不回写 | Consumer contract、发布revision及产品类型快照、选择请求的设备范围证据、AST故障注入、发布/选择前后数据库对照及旧revision查询结果 |
