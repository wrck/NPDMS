...
修订007的100项正式Requirement、111个目标版本切片及11个补充V2切片的原复核作为历史保留；修订017新增断言的执行证据尚待对应Feature提交；这只表示测试契约足以供Feature使用，不表示测试已经执行或发布验收通过。V3、OUT_OF_SCOPE及后置工单/历史迁移内容只能作为边界负向测试或后续门禁输入。
...
F-INS-001规则名称、发布与安全专项：

- 名称唯一性覆盖同租户同名创建拒绝、跨租户同名允许、停用/软删除后仍拒绝复用、复制revision沿用原名称、revision改名拒绝及并发同名创建最多一个成功；失败请求不得留下孤立稳定身份或revision。
- 草稿/发布覆盖稳定身份字段必填、其余字段不完整仍可保存，以及发布八字段、命令、`NUMBER`阈值、适用产品类型和当前摘要安全审核全量完整；只有`PASSED`允许发布。
...
依据修订019验证项目级成员管理：服务经理与多名项目经理同次指派、服务经理已指派后单独添加项目经理、各实际阶段的获权调整，以及无固定指派任务时正常办理。保留一个当前主责项目经理，主责必须是有效经理成员；切换主责不自动删除其他成员。按人员分别验证不重叠责任区间、同键重放、异载荷/版本冲突和联合指派失败全回滚。

验证成员名单、主责ID/姓名/工号显示与服务端权限事实一致；验证主责和非主责的有效PROJECT_MANAGER在同一项目具有相同角色操作权限，缺少角色或范围仍拒绝；不能只用manager_id决定权限。候选验证同公司、角色精确匹配和跨部门可选；不同公司或角色、无效/过期/停用授权不命中，多条部门记录不重复用户。候选及提交重验均消费同一上游通用查询。无对应权限、跨租户/越界、无效候选、关闭项目保护、审计/Outbox写入失败及通知投递失败分别验证。成员操作不创建/完成ProjectTask，不修改阶段或历史批准事实；已有TASK_NATIVE及其他Owner绑定仍按其冻结规则工作。当前只有设计要求，未执行业务API、MySQL或浏览器验证，不宣称Feature Done。
...
| 领域 | 场景 |
|---|---|
| IMP | 到货部分签收/差异、安装确认/退回、配置解析、联调、风险事实、质量整改、就绪门禁；就绪覆盖四项满足、单项未满足/重开、设备或方案变化、Provider失败、并发序号、幂等重放和越权；不新增安全聚合 |

F-CUT-007验收须分别证明四项计数的精确谓词、`taskId`去重及允许重叠；待办数据集必须包含“仅可见不可写”“同任务多动作”“P5 `myTodos`交集”和“hold/终态无动作”样本。空可见范围必须全0且不扩大查询，ProjectScope或动作事实不可用时必须整体失败。测试作用域可使用确定性跨模块替身推进正常正向闭环，但替身不得注册生产Bean或作为真实浏览器、Implementation Done证据。

## 修订018：模板业务规则与独立终验验收设计

Requirement：PM-03、PM-11、ACC-03、ACC-04、CLO-01/02；关联PM-01、PM-06、COM-01。以下是待实现后的业务验收义务，全部NOT_RUN，不用旧Task通过证明本修订完成。

| 场景 | 应验证的业务结果 | 当前证据 |
|---|---|---|
| 有S5、未配置终验依赖 | 不隐式创建终验或生成缺失项，按其他冻结规则判定 | NOT_RUN |
| 无S5、配置终验绑定/触发及结果依赖 | 能办理真实ACC终验，结果影响显式消费者，不伪造S5或固定任务 | NOT_RUN |
| 配置初验前置/未配置初验前置 | 分别执行配置，不按签约方式硬加依赖 | NOT_RUN |
| 新增模板任务并保存、发布、实例化 | UI提交完整执行契约，支持声明的原生默认，不靠SQL补齐业务字段 | NOT_RUN |
| 同一审批事实重复投递 | 重放同一业务意图目标，审批通过不等于终验通过 | NOT_RUN |
| 刷新视图、readiness查询和规则重算 | 零实体创建和零业务状态写入 | NOT_RUN |
| 创建时点晚于自身完成前置 | 发布时拒绝不可执行的循环依赖 | NOT_RUN |
| 报告有效/活动完成/明确通过分别被引用 | 谓词语义不混同，未知或失败不满足通过条件 | NOT_RUN |
| 多节点引用同一结果 | 不复制终验；各节点独立授权、单一主绑定和版本判定 | NOT_RUN |
| S5外验收范围确认、A到A+B变更 | 范围与版本真实，旧报告不自动覆盖新增范围，依赖判定重算 | NOT_RUN |
| 触发、范围绑定、推进任一步失败 | 按锁定命令边界回滚或保留已成功独立事实，不伪报项目/阶段完成 | NOT_RUN |
| 已有项目与新规则版本并存 | 旧冻结语义和历史不改，新项目按新版本，换模须独立授权 | NOT_RUN |

真实验收应覆盖ACC独立入口及Stage/Task绑定入口，验证实体/报告/范围的真实持久化与权限；截图、组件测试或HTTP 200不能代替闭环。具体API/Schema/并发用例待Q-TPLACC-001定稿，不能先写假Provider或降低校验换取通过。
