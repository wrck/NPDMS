...
- `AssetProductTypeApi.getByCodes`逐项断言存在、启用/停用、显示名称、来源版本、同步状态、最近成功同步时间和最近成功副本标记；未知编码不返回猜测名称，跨租户服务身份拒绝且不泄露存在事实；
- `getAuthorizedDeviceProductType`覆盖有权设备、空请求设备集合、空设备范围、跨租户、未知/冲突/未解析和停用，并断言相同降级字段；空范围、跨租户或无权设备统一返回空结果，不泄露设备存在性；
- 带完整来源证据的受控导入覆盖首次、更早`sourceUpdatedAt`拒绝、同水位且`sourceVersion + payloadHash + targetProductTypeCode`全同幂等、同水位任一事实不同冲突、更晚水位更新、同源多目标、空响应和部分失败；断言`sourceVersion`不参与字符串/数字/语义版本排序，最近成功副本不被清空，冲突摘要在导入拒绝后仍可查询；
