...
- 带完整来源证据的受控导入覆盖首次、同版本同载荷幂等、同版本异载荷冲突、旧版本乱序、同源多目标、空响应和部分失败；最近成功副本不被清空；
...
| 范围 | 必须证明的业务断言 | 最小证据 |
|---|---|---|
| EQP-01@V1 / F-AST-002 | 受控来源形成稳定产品类型副本；编码不可复用；设备当前引用不猜测；批量与授权设备查询执行租户和数据范围；停用阻止新消费但历史可解释；来源失败不清空最近成功副本 | 前向迁移与真实MySQL约束、来源版本幂等/冲突记录、两个AssetProductTypeApi契约测试、空范围/跨租户/无权设备负向结果、停用与最近成功副本前后对照 |
