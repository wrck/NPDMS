...
| `/cutover-tasks/{id}/plan`、`/plan/actions/{create-draft|download-draft|submit|revise}` | P4 detail、save、download、submit、revise | F-CUT-004拥有方案正文、步骤、风险措施、文件引用、保障人员和revision；上传分支不强制解析在线模板；submit必须通过`CutoverApprovalStartPort`与CUT-05审批实例创建同成同败 |
...
F-CUT-004用户REST、P4判别联合、文件事实、revision、错误与幂等由`specs/features/F-CUT-004-api-contract.json`锁定。内部`CutoverApprovalStartPort.start`以`MANDATORY`加入P4提交事务，只有不可变方案revision与CUT-05审批实例同时成功才推进到P5；`CutoverApprovalResultPort.inspect/lockAndRevalidate`提供PENDING/APPROVED/REJECTED/TERMINATED只读审批事实和版本。审批实例、路由、节点、意见、待办、通知及`CutoverApproved`均由F-CUT-005拥有，F-CUT-004不得实现或直接写其表。生产Provider缺失时只允许`src/test`受控替身，不注册生产Fake/fallback。
