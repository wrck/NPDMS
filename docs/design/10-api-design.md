...
| `/cutover-tasks/{id}/approval`、`/cutover-approvals/todos` | GET | 返回冻结来源、串行节点、五项评审、服务经理等级复核和服务端allowedActions；待办只来自当前PENDING节点 |
| `/cutover-tasks/{id}/approval-actions/reassign` | POST | 审批管理员仅改派WAITING/PENDING节点；目标按节点类型锁定PROJ/SYSTEM候选及项目范围，改派不授予代批权 |
| `/cutover-tasks/{id}/closure` | save、submit、detail | 保存P6结果与INT-12证据引用；提交即归档；失败不发布CutoverCompleted |
| `/cutover-tasks/{id}/spare-support`、`/spare-support/actions/initiate` | GET、POST | CUT-08返回是否需要备件、外部申请引用、只读状态快照、人工证据和服务端动作；发起命令只允许任务一线负责人在项目/设备授权仍有效时调用，并以Idempotency-Key保证同意图重放 |
| `/cutover-tasks/{id}/spare-support/applications/{applicationId}/actions/refresh` | POST | 通过INT-06查询既有外部申请状态并追加只读版本快照；Provider不可用保留原事实和失败记录，不伪造成功 |
| `/cutover-tasks/{id}/spare-support/evidence` | POST | 关联已由PLT冻结的人工证据文件事实；人工证据显式标记MANUAL，不冒充接口回填 |
...
F-CUT-004用户REST、P4判别联合、文件事实、revision、错误与幂等由`specs/features/F-CUT-004-api-contract.json`锁定。`createDraft`按编辑方式使用封闭请求联合：在线模式只接收`editMode`，完整文件模式同时接收文件期望事实和人工归属确认，并在创建事务内以PLT锁定事实为唯一持久来源；创建还锁定重验当前最终评估和适用清单，分别以`ASSESSMENT_STALE`及仅A/B/C可达的`CHECKLIST_STALE`表达来源变化。`PlanSourceSnapshot`同时冻结完整CUT-03失败风险集合，后续不得从草稿缺行反推Owner事实。内部`CutoverApprovalFactApi`精确签名和DTO由`specs/features/F-CUT-005-approval-owner-contract.json`锁定：`start`以`MANDATORY`加入P4提交事务；P4外层事务先分配唯一服务端`planSubmittedAt`并传入`start`，审批创建成功后同值原样写入方案revision，只有方案`SUBMITTED`与PENDING审批实例同时成功才推进P5；`inspect/lockAndRevalidate`提供PENDING/PAUSED_SOURCE_INVALIDATED/APPROVED/REJECTED事实和版本；`pauseForSourceInvalidation`与方案INVALIDATED及F-CUT-004任务P5→P4同事务，恢复办理通过新revision的`start(previousApprovalInstanceId)`建立替代审批链，旧事实不恢复。审批实例、路由、节点、意见、待办、通知及`CutoverApproved`均由F-CUT-005拥有，F-CUT-004不得实现或直接写其表。生产Provider缺失时只允许`src/test`受控替身，不注册生产Fake/fallback。
...
F-CUT-010只在CUT任务工作台增加备件协同区，并把安全摘要加入P5 `FULL`审批详情。发起动作统一调用INT-06预留`SpareApplicationGateway.initiate`，响应可同时包含外部申请号和受控跳转地址；只有跳转地址时按平台requestId和INT externalRequestId等待受信`bindExternalReference`首次绑定，不以HTTP成功解释为外部申请成功。外部状态通过已绑定申请号的回调或显式刷新进入同一版本化追加路径。P5摘要沿用F-CUT-005既有FULL可见性，不额外要求独立备件查询权限，只含外部系统/申请号/原始状态/观察时间及人工证据展示名称、说明和上传时间；不含`allowedActions`、跳转地址或PLT内部身份，FINAL_RESULT_ONLY/REASSIGNMENT_ONLY不返回。CUT不提供外部状态编辑接口，P5显示不改变五项评审、审批动作或P6门禁。精确请求、响应、WireLong/时间、错误与恢复动作见`specs/features/F-CUT-010-api-contract.json`。
...
AST产品类型公开契约由F-AST-002的`AssetProductTypeApi.getAuthorizedDeviceProductType`拥有。CUT预留内部消费端口，输入为服务端受信操作人和已解析设备ID集合，结果仅映射公开合同已有的`deviceId/productTypeCode/enabled/sourceVersion/resolutionStatus/syncStatus/lastSuccessfulSyncTime/fromLastSuccessfulCopy`。CUT不得追加`sourceKey/assignmentVersion/watermark`，不得以现有接口的查询结果冒充写事务锁定重验；生产接线须独立过集成Gate。F-CUT-002只冻结编码与来源版本，F-CUT-003只消费冻结编码；`src/test`受控替身不进入生产装配。
