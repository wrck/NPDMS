...
- `DeviceScopeFact`返回`tenantId/projectId/devices/scopeWatermark`。设备项保留`deviceId/sn/currentProjectId/projectAssignmentVersion`，并加性返回产品主数据当前投影`deviceTypeCode/deviceTypeSourceKey/deviceTypeSourceVersion/deviceTypeAssignmentVersion`；水位按`deviceId`升序保存`deviceId/projectAssignmentVersion/deviceTypeAssignmentVersion`，不使用哈希、摘要、伪全局版本或新表。产品主数据拥有产品到类型的赋值，SYSTEM只校验`pms_device_type`允许值，AST不得从`productCode/productModel/conpType`推导。
- `lockAndRevalidate`接收包含同样设备类型事实的完整期望设备项与调用方专用`ExpectedScopeWatermark`，按`deviceId`升序锁`ast_device`当前投影并校验集合。集合仍完整有效但任一归属版本或设备类型赋值版本变化时返回`STALE`和当前完整事实；同一`deviceId`的当前Owner SN比较键或产品主数据来源身份损坏时抛`OWNER_DATA_CORRUPTED`。缺失、状态不可用、错项目返回`INVALID`，不返回部分有效事实。
- F-CUT-002在任务创建写入前必须要求每台设备的设备类型编码、来源键、来源版本和赋值版本完整，编码命中SYSTEM当前有效值域，并把锁后事实原样冻结到任务设备范围；缺失、冲突或字典无效统一为业务门禁且任务零写入。F-CUT-003只消费冻结编码集合，不在P3重新查询产品主数据或AST当前分类。其他DeviceScope消费者可不把设备类型缺失解释为自身业务门禁。
