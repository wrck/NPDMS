...
| `ExportTaskApi.request/getFact/retry`、`ExportBusinessDataProvider` | ACC-02、ADR-0014/0016 | PLT | ACC及其他受控业务Owner | PLT拥有唯一异步Task/Audit与文件TTL；消费Context Provider拥有查询语义并在申请、生成、显式重试、下载时重验功能/数据/字段/文件/租户范围；只允许原actor把可重试FAILED按version CAS恢复为REQUESTED；F-ACC-002固定`ACC/SATISFACTION_RESULT`，不得建立第二导出真值 |
