...
| `CutoverSpareCallbackApi.bindExternalReference` | CUT-08、INT-06 | CUT | INT-06 Integration ACL | 仅用于首次发起只返回跳转地址后的外部申请号绑定；按受信租户、CUT平台请求ID、INT外部请求ID和外部系统锁定原意图，同键重放、身份错配或跨任务绑定失败关闭 |
