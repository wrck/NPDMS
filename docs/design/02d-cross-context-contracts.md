...
| `ProjectCutoverServiceManagerFactApi` | CUT-05、PM-08 | Project / F-CUT-005物理Owner支撑Task | Cutover / F-CUT-005 | 按受信tenant/project解析当前唯一PRIMARY `SERVICE_MANAGER_L1/L2`及project/participant事实版本并锁定重验；零个或多个返回明确NOT_UNIQUE，不任选人员。服务经理节点改派目标只能等于当前唯一事实，不能用全局角色或项目范围单独替代。该支撑Task可复用ProjectParticipant聚合但不建立独立Feature或表。 |
| `CutoverApprovalRoleCandidateFactApi` | CUT-05 | System / CUT审批角色支撑Task | Cutover / F-CUT-005 | 仅支持`CUT_SECOND_LINE_APPROVER/CUT_RND_APPROVER`角色组，返回完整启用成员及成员/用户状态版本；CUT逐人叠加同项目`ACTION_VIEW`后才判断交集唯一。重验按用户ID锁定完整角色成员事实和逐人项目treeVersion并重算交集；全局多人但项目交集唯一必须成功，交集零/多人暂停审批。显式改派目标另行锁定成员资格；不读取SYSTEM表、不注册生产fallback。 |
...
| `AssetProductTypeApi.getAuthorizedDeviceProductType` | EQP-01、CUT-03 | AST / F-AST-002 | CUT / F-CUT-002、F-CUT-003 | 按受信租户、服务端解析的委托用户和稳定设备ID集合返回当前`deviceId/productTypeCode/enabled/sourceVersion/resolutionStatus/syncStatus/lastSuccessfulSyncTime/fromLastSuccessfulCopy`；CUT不扩展该Owner合同、不读取AST表，不从产品编码、型号或CONP推导。F-CUT-002只冻结公开结果可证明的产品类型编码和来源版本，F-CUT-003只消费冻结编码。当前合同缺少写事务内期望版本锁定重验，生产接线须经独立集成Gate；CUT `src/test`受控替身不构成生产事实。 |
...
| `CutoverSpareCallbackApi.bindExternalReference` | CUT-08、INT-06 | CUT | INT-06 Integration ACL | 仅用于首次发起只返回跳转地址后的外部申请号绑定；按受信租户、CUT平台请求ID、INT外部请求ID和外部系统锁定原意图，首次绑定原子执行`REQUEST_PENDING -> EXTERNAL_REFERENCED`，同值重放返回既有`EXTERNAL_REFERENCED`，身份错配、不同申请号或跨任务绑定失败关闭 |
