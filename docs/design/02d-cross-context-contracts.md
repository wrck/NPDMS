...
| `CutoverApprovalRoleCandidateFactApi` | CUT-05 | System / CUT审批角色支撑Task | Cutover / F-CUT-005 | 仅支持`CUT_SECOND_LINE_APPROVER/CUT_RND_APPROVER`角色组，提供唯一候选inspect/重验及显式目标成员锁定；CUT再叠加同项目`ACTION_VIEW`。候选不唯一或失效暂停审批，不读取SYSTEM表、不注册生产fallback。 |
