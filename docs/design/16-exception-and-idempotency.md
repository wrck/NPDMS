...
| PM-06范围追加 | 同一ACTIVE项目，冻结批准申请、COM scopeVersion与订单行事实 | 并发追加同水位仅一方生效；失败全事务回滚，不生成新项目或群组；新增范围不继承旧验收通过 |
| PM-06范围追加 | 同一ACTIVE项目，冻结批准申请、COM scopeVersion与订单行事实 | 并发追加同水位仅一方生效；失败全事务回滚，不生成新项目或群组；新增范围不继承旧验收通过 |
...
## PM-03 当前阶段图推进异常契约（Feature仍待重验证）

| 分类 | 稳定语义 | 写入结果 |
|---|---|---|
| `VALIDATION` | 命令缺字段、客户端指定目标或冻结图无法唯一解析目标 | 零业务写入 |
| `AUTHORIZATION` | 缺功能权限、`PROJECT_MANAGE`范围或当前项目经理关系 | 零业务写入 |
| `STATE_CONFLICT` | 生命周期非ACTIVE、当前不是冻结图真实活动节点或Stage形状冲突 | 零业务写入 |
...
| 场景 | 错误及状态 | 合法恢复 |
|---|---|---|
| 图无目标、多义、不可达或引用未发布 | 保持草稿/原Stage，返回冲突边和规则；Owner未知使用DEPENDENCY_UNAVAILABLE | 修订配置并发布新版本；既有项目必须受控换图，不补造阶段 |
| Stage绑定无权或业务视图不可用 | 保留业务和节点状态，不能降级为通用完成 | 恢复Provider或补授权后重新鉴权，不缓存成功授权 |
| PM-06同键同载荷 | 返回原申请及应用结果，不重复占量或生成任务 | 原幂等结果查询 |
| PM-06同键异载荷/超配/旧水位 | 拒绝并保留原有效范围，失败事务不发布成功事件 | 重新读取COM当前scopeVersion并生成合法修订；不能用群组或新项目绕过 |
| 范围A追加B、仅有旧报告A | 保留A报告历史但当前总体覆盖未通过 | 形成当前整体范围明确通过的新报告版本；不覆盖旧结论 |
| 回调伪造/旧版本/错误租户 | 不生成可用文件和业务成功结果，保留脱敏拒绝证据 | 按来源契约重试/查询/人工核验，不以幂等键替代身份 |
| 预检后临时密码缺失或授权撤销 | 正式执行不下发，原预检结果仍可追溯 | 新输入和当次任务授权同时成立，不能缓存临时密码 |
...
## 修订018：触发、实体办理与门禁的完成点

Requirement：PM-03、PM-11、ACC-03、COM-01。读条件不写实体，触发成功不等于业务完成。

1. 未配置终验依赖：不产生隐式终验缺失项；已配置但事实未满足：返回对应业务缺口；引用失效、Owner未知、无权或版本变化：按既有异常类别拒绝，不能当作未配置放行。
2. 同一来源事实和业务意图的重复触发使用稳定幂等键并重放目标；同键异载荷冲突。不同业务意图的身份与范围应显式区分，不能靠随机重试键重复创建或靠projectId误合并。
3. 只读readiness、视图加载及条件重算不创建实体。独立创建命令已合法成功后，下游门禁尚未通过不应抹去创建事实；阶段推进本身仍须原子完成，不能提交半个阶段转换。
4. 项目创建时的节点、冻结配置及该事务声明必须初始化的对象继续全有或全无；配置明确延迟解析的实体不伪造实例。独立触发与项目创建的具体事务边界由Q-TPLACC-001锁定，不在实现中静默改成最终一致性。
5. 范围变化使明确依赖旧范围的当前判定失效，历史报告/快照不变；旧验收范围退出/解锁未裁决部分不因本次解耦自动开放。
