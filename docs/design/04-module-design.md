...
|---|---|---|---|---|---|
| 项目治理 | 创建、指派、模板、项目树、项目—站点关系、模板前属性判定、模板匹配决策历史和创建后影响识别、Stage→ProjectTask工作台投影、TASK_NATIVE通用任务执行、其他业务绑定与完成判定、项目状态 | Project、ProjectStageSnapshot、ProjectTemplateMatchHistory、ProjectSite、ProjectTask、ProjectTemplate | CRM/ERP属性输入、组织主数据、AssetLocationApi、AcceptanceScopeBindingApi、非TASK_NATIVE绑定业务查询/API | ProjectCreated、ProjectStageChanged、TaskCompleted、ProjectAcceptanceStageFactApi、内部ProjectAttributeResolutionService | 直接改设备/地点/合同库、ACC绑定或报告、反写CRM来源、建设第二套模板匹配器或CHG流程、复制非TASK_NATIVE绑定业务正文、维护第二套导航树 |
...
| 验收与闭环 | 培训、满意度问卷模板/任务/受控链接/答卷/判定/整改与满意度来源归档、初验/终验活动与不可变报告版本、既有项目应交实例的来源索引/归档补偿、齐套校验和闭环交接；按项目阶段快照记录进入验收范围的DeliveryScope分配版本 | Acceptance、AcceptanceScopeBinding、SatisfactionCollection、ProjectClosure、`acc_project_deliverable`应交实例 | ProjectTask/WorkBinding身份、冻结满意度时点与完成命令、ProjectStageSnapshot、ProjectScopeApi、DeliveryEvidence、DeliveryScopeAcceptanceLockApi、PLT `FileArtifactApi`稳定公共事实、`PlatformCommandExecutionApi/PlatformOutboxDeliveryApi` | SatisfactionQuestionnaireTemplateApi、SatisfactionTaskInitializationApi、SatisfactionResultFactApi、AcceptanceActivityInitializationApi、AcceptanceActivityCompletionFactApi、ACC `FileBusinessObjectPolicyProvider`、AcceptanceScopeBindingApi、AcceptanceScopeGuardApi、SatisfactionResultVersionChanged、AcceptanceReportVersionChanged、专用Outbox投递Job、ClosureGateRecheckRequested、ProjectClosureCompleted、ServiceHandoverCreated | 直接改PROJ阶段/任务、COM范围或财务事实，建立第二套项目任务或应交清单真值，从Todo/送达/旧回访状态推断客户提交或通过，直接修改现场实施原始证据，持有COM/PROJ/PLT的DO/Mapper/Repository或PLT内部FileVersion/FileReference ID，使用报告状态触发范围绑定 |
...
| 合同履约 | ERP合同、销售订单、订单行同步副本及项目交付范围主记录与明细；冻结分配发生时目标项目所属SYSTEM办事处快照 | Contract、SalesOrder、OrderLine、DeliveryScope、DeliveryScopeDetail | ERP、OrganizationScopeApi、ProjectOfficeFactApi、ProjectAcceptanceStageFactApi、AcceptanceScopeBindingApi、AcceptanceScopeGuardApi | DeliveryScopeAcceptanceLockApi、ScopeAllocated、ScopeReleased | 直接写ERP、读取SYSTEM/ACC/PROJ的DO/Mapper/Repository、从部门或项目关系推断合同公司范围、把AST站点当作订单范围地点、以报告状态判定验收范围；不创建COM-02履约回写/对账业务聚合 |
...
| 基础平台能力 | 公司、部门、用户公司—部门范围、待办、文件、授权、变更、字典、审计、统一异步导出及共享动态表单模板/实例通用能力 | Company、Department、UserCompanyDepartmentScope、Todo、FileArtifact、ExportTask、Grant、ChangeRequest、DynamicFormTemplate、DynamicFormInstance | LDAP/AD及所有模块 | CompanyApi、DeptApi、OrganizationScopeApi、ExportTaskApi、TodoCreated、AuditRecorded；业务Context以`ExportBusinessDataProvider`保留范围/字段语义；F-SOL-003形成首个真实调用方后提供窄`DynamicFormBusinessInstanceApi`及Owner策略Provider | 由部门推导公司、拥有业务域状态、复制消费方范围算法、把导出或通用表单保存解释为业务完成 |
