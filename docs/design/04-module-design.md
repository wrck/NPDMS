...
| 验收与闭环 | 培训、满意度收集、初验/终验活动与不可变报告版本、既有项目应交实例的来源索引/归档补偿、齐套校验和闭环交接；按项目阶段快照记录进入验收范围的DeliveryScope分配版本 | Acceptance、AcceptanceScopeBinding、SatisfactionCollection、ProjectClosure、`acc_project_deliverable`应交实例 | ProjectTask/WorkBinding身份与完成命令、ProjectStageSnapshot、DeliveryEvidence、DeliveryScopeAcceptanceLockApi、FileArtifact/FileVersion、Questionnaire | AcceptanceActivityCompletionFactApi、AcceptanceScopeBindingApi、AcceptanceScopeGuardApi、AcceptanceReportVersionEffective、ClosureGateRecheckRequested、ProjectClosureCompleted、ServiceHandoverCreated | 直接改PROJ阶段/任务、COM范围或财务事实，建立第二套项目任务或应交清单真值，直接修改现场实施原始证据，持有COM/PROJ的DO/Mapper/Repository，使用报告状态触发范围绑定 |
