...
| 合同履约 | ERP合同、销售订单、订单行同步副本及项目交付范围主记录与明细；冻结分配发生时目标项目所属SYSTEM办事处快照 | Contract、SalesOrder、OrderLine、DeliveryScope、DeliveryScopeDetail | ERP、ProjectOfficeFactApi、ProjectAcceptanceStageFactApi、AcceptanceScopeBindingApi、AcceptanceScopeGuardApi | DeliveryScopeAcceptanceLockApi、ScopeAllocated、ScopeReleased | 直接写ERP、持有ACC或PROJ的DO/Mapper/Repository、把AST站点当作订单范围地点、以报告状态判定验收范围；不创建COM-02履约回写/对账业务聚合 |
