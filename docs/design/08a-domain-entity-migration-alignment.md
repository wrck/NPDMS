...
| `SatisfactionCollection` | PROJ现有`satisfaction_timing`任务快照；旧`pm_cl_quesnaire_*`、`pm_cl_callback*`、`pm_subcontract_project_callback`及售前/维保/督导/日报中的问卷引用或缓存分数 | NONE_NEW+FEATURE_FORWARD_MIGRATION；旧源PRESERVE_RAW/AI-MIG-000 | F-ACC-002只为新平台命令创建模板修订、Task、Questionnaire、Response和Result；PROJ时点可直接复用但ACC模板Fact必须精确解析。旧字段映射、必答规则、签字、客户身份和通过值域未确认，当前不迁为有效业务事实或交付件；不得从回访/审批状态推断 |
| `DeliveryArtifact` | F-PROJ-001正式`acc_project_deliverable`；F-ACC-001来源历史表；V17旧交付清单/归档/完工证明 | CURRENT_FORWARD+RELATION | 直接复用唯一应交根、来源版本与附件集合；F-ACC-001处理初验/终验，F-ACC-002只增加`SatisfactionResult`来源且仅有效达标结果可CURRENT；不得由名称、任务名或`D-ACCEPT-REPORT`推断类型；旧栈不迁成新来源索引 |
