...
| `DynamicFormSchema` | 当前`pms_eng_form_template`、字段关系配置 | CURRENT_FORWARD | 发布模板转不可变revision；无法证明字段含义的配置进入问题 |
| `DynamicFormInstance` | 当前`pms_eng_form_instance` | CURRENT_FORWARD+STRUCTURED | 冻结schema版本和原值；核心查询字段结构化，附件只引用 |
...
1. `AI-MIG-000`关闭，DDL、目标字段目录、映射和manifest引用同一`approvedDdlSha256`；
