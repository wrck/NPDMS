...
| 数据对象 | 来源证据/现有实体 | 策略 | 迁移落位与禁止推断 |
|---|---|---|---|
| `Acceptance` | V17 `pms_acc_acceptance`；V63为存量ProjectTask初始化的当前`TASK_NATIVE`执行契约；锁定旧来源中无符合ACC-03四项完备和不可变版本要求的正式活动根/报告版本 | COMPATIBILITY_ONLY+NONE_NEW+FEATURE_FORWARD_MIGRATION | V17行缺验收人、稳定PLT公共文件事实和可证明活动绑定，保持旧表/旧接口只读兼容，不迁成当前有效报告、不推断缺失事实；新项目由`AcceptanceActivityInitializationApi`在项目创建事务内生成精确活动与ACC执行契约；存量精确任务对两项均非终态且当前契约均为V63 `TASK_NATIVE`时原子切换，两项均`DONE/CLOSED`保持不变，终态/非终态混合及部分/重复整批失败 |
| `SatisfactionCollection` | PROJ现有`satisfaction_timing`任务快照；旧`pm_cl_quesnaire_*`、`pm_cl_callback*`、`pm_subcontract_project_callback`及售前/维保/督导/日报中的问卷引用或缓存分数 | NONE_NEW+FEATURE_FORWARD_MIGRATION；旧源PRESERVE_RAW/AI-MIG-000 | F-ACC-002只为新平台命令创建模板修订、Task、Questionnaire、Response、Result和不可变RemediationFact；首次任务冻结原始source Fact，整改保持source并以RemediationFact触发下一taskRevision。PROJ时点可复用但ACC模板Fact必须精确解析。旧字段映射、必答规则、签字、客户身份和通过值域未确认，当前不迁为有效业务事实或交付件；不得从回访/审批状态推断 |
| `DeliveryArtifact` | F-PROJ-001正式`acc_project_deliverable`；F-ACC-001来源历史表；V55稳定`T-SAT-SURVEY→D-SAT-REPORT`模板关系；V17旧交付清单/归档/完工证明 | CURRENT_FORWARD+RELATION | 直接复用唯一应交根、来源版本与附件集合；F-ACC-001处理初验/终验，F-ACC-002只允许同租户同项目精确`D-SAT-REPORT/T-SAT-SURVEY`根接收有效达标`SatisfactionResult`来源，根缺失/重复/错配失败关闭；不得由名称、任务名或`D-ACCEPT-REPORT`推断类型，也不得按中文名称、其他任务或其他交付件降级；旧栈不迁成新来源索引 |
