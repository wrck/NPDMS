...
- 分配确认在锁定订单行后通过`ProjectOfficeFactApi`重验目标项目版本及办事处部门事实，并把部门ID/编码/名称/版本一次性冻结到新范围版本；项目或部门随后变化不更新旧快照。
- 减量先锁`DeliveryScope`当前行，再调用ACC `AcceptanceScopeGuardApi`读取当前绑定；ACC把范围纳入验收时也先经COM公开锁接口锁同一行，再追加绑定。统一锁顺序为COM范围→ACC绑定，守卫未知、超时或版本变化均整体回滚。
