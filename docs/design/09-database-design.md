...
|---|---|---|---|
| ArrivalAcceptance | `imp_arrival_acceptance` | `imp_arrival_line`、`imp_arrival_difference` | 根冻结DeliveryScope版本、结构化范围水位、项目级单调事实版本、独立迁移核对状态和ACC引用状态；批次编码是整条后继链稳定共享的业务身份，`batch_root_marker=1`只约束一个初始根，successor marker为NULL且每个前驱最多一个直接后继；后续根同时保存`predecessor_acceptance_id/successor_reason`，原因封闭为补签/更正/差异关闭/豁免失效且由服务端写入；明细按设备或订单型号数量追加版本且数量非负/不得超签；差异、拒收处置和具体豁免通过`imp_arrival_difference`不可变版本表达，仅事实影响revision在插入时同时持有非空`fact_impact_type/project_fact_version`，非事实影响revision两者永久为NULL，已提交历史不可覆盖 |

`imp_arrival_acceptance`一经插入，其`tenant_id/project_id/batch_code/predecessor_acceptance_id`为后继链身份，不得更新。跨代证据重验可沿该不可变前驱链只读证明`imp_delivery_evidence_revision.source_record_id`是当前节点或严格祖先；链上任一删除、断链、环、tenant/project/batch不一致均视为数据损坏并失败关闭，不能把任意历史批次当作证据来源。
...
Q-FIMP002-002的后继身份修正使用下一实际空闲Flyway版本：新增服务端字段`batch_root_marker tinyint NULL`且无默认值；初始根必须为`predecessor_acceptance_id/successor_reason=NULL + batch_root_marker=1`，successor必须为两项非NULL且marker=NULL。迁移仅在所有现存根均无predecessor/successor语义时确定性写marker=1，否则在任何ALTER前SIGNAL；随后以`uk(tenant_id, project_id, batch_code, batch_root_marker)`替换旧批次唯一键，并增加`uk(tenant_id, predecessor_acceptance_id)`保证线性后继。失败过程可按Flyway repair原样重跑并清理临时过程，不从编码、状态或时间猜测历史。
...
F-CUT-002新平台路径使用`cut_task`、`cut_task_device_scope`、`cut_task_stage_history`、`cut_assessment`四表。`cut_task`保存SELF_CREATED/ITR/PROJECT_EVENT来源联合、不可变同项目线性`previous_task_id`、背景、P2～P4当前阶段、IMP快照、PROJ/AST/CUS/IMP上下文快照，以及任务创建时冻结的`configuration_revision_id/configuration_code/configuration_revision_no`；三项配置身份对`NEW_PLATFORM`必填且不可改，对`LEGACY_FORWARD`为空。`cut_task_device_scope`按`uk(tenant_id,project_id,device_id,active_marker)`保证活动设备唯一，并冻结AST返回的`device_type_code_snapshot/device_type_source_key_snapshot/device_type_source_version_snapshot/device_type_assignment_version`；新任务要求四项产品主数据分类事实完整。`cut_task_stage_history`只追加P1接入和P2提交迁移；`cut_assessment`按任务+assessmentVersion保存固定CUT模板、DRAFT可空/SUBMITTED非空答案JSON、项目/办事处/设备/IMP/客户等级上下文、人工等级及DRAFT/SUBMITTED/INVALIDATED历史。已提交评估失效必须同事务插入下一版DRAFT、切换唯一current marker和任务当前引用。精确列、可空性、唯一键、联合约束与跨表原子不变量由`specs/features/F-CUT-002-physical-contract.json`锁定；不得用测试模拟、附件或旧风险行补造Owner事实。

ADR-0037的前向差量在`ast_device`增加可空`device_type_code/device_type_source_key/device_type_source_version`及非负`device_type_assignment_version`，在`cut_task_device_scope`增加对应快照列。已执行迁移不可修改；既有AST设备和既有NEW_PLATFORM任务没有精确产品主数据来源时保持空并不得进入完整CUT-03设备类型匹配，禁止按`product_code/product_model/conp_type`、字典默认值或测试种子回填。新写入路径必须完整非空；LEGACY_FORWARD继续不生成设备范围。

任务创建以服务端捕获并同时写入`cut_task.create_time`的单一`taskCreatedAt`为生效时点。SELF_CREATED由用户从服务端返回的当前适用配置列表明确选择`configurationCode`；受信ITR/PROJECT_EVENT命令必须显式携带该代码。CUT只把代码视为选择意图，新任务按`tenant_id + configuration_code + status_code=PUBLISHED + effective_from <= taskCreatedAt < effective_to或无结束时间`精确解析唯一当前发布修订，并冻结三项身份；调用方不得提交revision ID或revisionNo。V146后、配置字段加入前已存在的`NEW_PLATFORM`任务，按任务原`create_time`从具有正式发布事实且`status_code IN (PUBLISHED, DISABLED)`的历史修订中解析生效区间；`DRAFT`永不参与。仅当全租户全部配置代码中恰好一个修订满足`effective_from <= original create_time < effective_to`（无结束时间则开放）时才允许前向补齐；零个或多个候选在任何写入前整批失败。不得使用`CUTOVER_DEFAULT`、当前时间、割接类型、名称或任选一条进行回填。`LEGACY_FORWARD`不补配置身份，也不得进入CUT-03。
...
APPROVAL/PROCESS不新建PMS映射表：`ref_code`冻结Flowable processDefinitionKey，必填`ref_version`冻结`v<正整数>`定义版本；`pms-module-integration`的PMS版本化流程Owner Provider通过Flowable RepositoryService按key+version唯一解析活动定义ID，再以RuntimeService按该ID启动，固定`businessKey=PROJECT_STAGE_GATE:{gateReferenceId}`并冻结tenantId、projectId、stageCode、gateId、gateReferenceId、refType、refCode、refVersion变量。启动时由服务端设置`PROCESS_START_USER_ID=actorUserId`、`PROCESS_STATUS=RUNNING(1)`及`_FLOWABLE_SKIP_EXPRESSION_ENABLED=true`，并通过Flowable `Authentication.setAuthenticatedUserId`在try/finally中设置、清除发起人；命令不接收可覆盖这些字段的客户端变量或自选审批人。事实Provider只按该businessKey及冻结变量读取Flowable运行/历史事实；没有实例、整数状态1运行中、3驳回、4撤回、2批准完成和未知分别按10/16分册判定。既有受管模板的`PROCESS/PROC-PMS-DELIVERY-STD/v1|v2`可直接使用该契约。禁止新增第二门禁结果表、阶段历史表或修改旧Flyway；若实现期证明既有Flowable事实无法唯一承载上述关联，必须回到本Gate复审必要的加性事实，不得在Technical Plan静默补表。
