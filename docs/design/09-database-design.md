...
任务创建以服务端捕获并同时写入`cut_task.create_time`的单一`taskCreatedAt`为生效时点。SELF_CREATED由用户从服务端返回的当前适用配置列表明确选择`configurationCode`；受信ITR/PROJECT_EVENT命令必须显式携带该代码。CUT只把代码视为选择意图，按`tenant_id + configuration_code + effective_from <= taskCreatedAt < effective_to或无结束时间`精确解析唯一已发布修订，并冻结三项身份；调用方不得提交revision ID或revisionNo。V146后、配置字段加入前已存在的`NEW_PLATFORM`任务，仅当按该任务原`create_time`在全租户全部配置代码中能够证明恰有一个适用已发布修订时才允许前向补齐；零个或多个候选在任何写入前整批失败。不得使用`CUTOVER_DEFAULT`、当前时间、割接类型、名称或任选一条进行回填。`LEGACY_FORWARD`不补配置身份，也不得进入CUT-03。
