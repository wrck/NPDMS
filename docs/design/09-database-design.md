...
配置包中`questions[]`公共字段固定为`code/title/type/required`。单选、多选与评分量表增加有序`options[{code,label,score}]`；多选增加`minSelections/maxSelections`；文本增加`minLength/maxLength`；参与`WEIGHTED_AVERAGE_V1`的计分题增加正`weight`，`SUM_V1`禁止weight。`scoring`固定为`ruleVersion/strategy/scoreMin/scoreMax/precision/roundingMode/threshold`，V1的scoreMin必须为0；SUM的scoreMax等于各计分题最大option score之和，加权平均的scoreMax等于各计分题最大option score的加权平均，threshold必须位于0..scoreMax。decimal以JSON字符串表达并必须可无损转换为`DECIMAL(7,2)`；precision仅0、1、2。发布校验失败不得写PUBLISHED状态或根current指针。
