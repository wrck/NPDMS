...
5. `fb_contract`映射发货合同归属，不生成合同主档；回款来源、ERP合同关系和发货归属分别保留。2026-09-17需求方再次确认：ERP生成唯一主档，回款、发货补充信息并去重。`com_contract_summary`作为只读汇总视图，以`com_contract`为根，按同租户、公司、合同号关联来源事实；不生成或覆盖ERP主档，不跨公司归并。相同业务内容只展示一次但保留全部来源引用，冲突不任选、不累加重复金额；字段补缺及明细契约见F-COM-001第6章。
