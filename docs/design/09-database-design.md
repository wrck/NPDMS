...
APPROVAL/PROCESS不新建PMS映射表：`ref_code`冻结Flowable processDefinitionKey，必填`ref_version`冻结`v<正整数>`定义版本；流程启动时固定`businessKey=PROJECT_STAGE_GATE:{gateReferenceId}`，并冻结tenantId、projectId、stageCode、gateId、gateReferenceId、refType、refCode、refVersion变量。BPM Provider只按该businessKey及冻结变量读取Flowable运行/历史事实；没有实例、运行中、驳回/撤回、批准完成和未知分别按10/16分册判定。既有受管模板的`PROCESS/PROC-PMS-DELIVERY-STD/v1|v2`可直接使用该契约。禁止新增第二门禁结果表、阶段历史表或修改旧Flyway；若实现期证明既有Flowable事实无法唯一承载上述关联，必须回到本Gate复审必要的加性事实，不得在Technical Plan静默补表。
