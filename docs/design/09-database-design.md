...
F-CUT-004仅新增`cut_plan_revision`、`cut_step`、`cut_cutover_support_arrangement`三张CUT业务表；联系人变更复用平台操作审计，不增加第四张业务表。NEW_PLATFORM的A/B/C/D方案均冻结任务、评估、项目、设备、配置revision和适用方案模板章节，D仅清单身份为空；在线正文、文件版本和旧来源快照分别遵循`F-CUT-004-api-contract.json`/physical contract的精确JSON联合。`SUBMITTED`正文只读，来源失效只追加失效元数据并转`INVALIDATED`，不得覆盖正文或把审批驳回改名为失效。`pms_cut_plan`只经PLT迁移证据形成`LEGACY_FORWARD`只读revision及四类步骤映射，原审核字段不产生CUT-05事实。
...
F-CUT-002新平台路径使用`cut_task`、`cut_task_device_scope`、`cut_task_stage_history`、`cut_assessment`四表。`cut_task`保存SELF_CREATED/ITR/PROJECT_EVENT来源联合、不可变同项目线性`previous_task_id`、背景、P2～P6当前阶段、IMP快照、PROJ/AST/CUS/IMP上下文快照，以及任务创建时冻结的`configuration_revision_id/configuration_code/configuration_revision_no`；三项配置身份对`NEW_PLATFORM`必填且不可改，对`LEGACY_FORWARD`为空。新平台物理阶段/状态对为P2/GRADE_CONFIRMING、P3/SURVEYING、P4/PLAN_DRAFTING、P5/APPROVING、P6/CLOSURE_IN_PROGRESS。`cut_task_device_scope`按`uk(tenant_id,project_id,device_id,active_marker)`保证活动设备唯一，并为CUT-03冻结F-AST-002公开查询可证明的`device_type_code_snapshot/device_type_source_version_snapshot`；不得补造公开合同不存在的来源键、赋值版本或水位。`cut_task_stage_history`只追加Owner命令实际发生的P1接入、P2提交/失效、P4提交、P5驳回/通过迁移；`cut_assessment`按任务+assessmentVersion保存固定CUT模板、DRAFT可空/SUBMITTED非空答案JSON、项目/办事处/设备/IMP/客户等级上下文、人工等级及DRAFT/SUBMITTED/INVALIDATED历史。V146/V149既有约束不得回改；F-CUT-004/005生产装配前的后续Flyway须先校验旧行，再前向替换检查约束且不改写历史。精确列、可空性、唯一键、联合约束与跨表原子不变量由`specs/features/F-CUT-002-physical-contract.json`锁定；不得用测试模拟、附件或旧风险行补造Owner事实。

ADR-0037只在CUT任务设备范围预留产品类型快照，不改变AST表。已执行迁移不可修改；既有NEW_PLATFORM任务没有精确F-AST-002公开事实时保持空并不得进入完整CUT-03设备类型匹配，禁止按`product_code/product_model/conp_type`、字典默认值或测试种子回填。生产写入的原子重验方案须先通过集成Gate；LEGACY_FORWARD继续不生成设备范围。
