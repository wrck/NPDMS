...
> Requirement ID：NFR-01、NFR-02、NFR-03及全部115项需求的构建、配置、迁移、发布和恢复保障
...
- 历史数据迁移必须先关闭`AI-MIG-000`：生成`approvedDdlSha256`，重建目标字段目录/映射/校验与release manifest；旧`migration-validation.json.passed=true`因DDL哈希漂移不具备当前放行效力。
...
【方向已确认：P3-E03】业务Owner先批准RPO/RTO，再由DBA和运维设计备份与恢复并完成演练。保护对象、备份方式/频率、在线/离线介质、加密、保留、恢复顺序、RPO/RTO数值、演练频率和责任人仍待证据；缺失时Phase 3不能批准。
...
逻辑部署设计可进入20测试；P3-E01～E05关闭且演练通过前，本分册不得转为`BASELINE`。
