...
| `SatisfactionTaskCreated` | ACC | Todo/Project | taskId/projectTaskId、projectId、collectionKey/taskRevisionNo/priorTaskId、sourceOwnerContext/sourceObjectType/sourceObjectId/sourceObjectVersion、triggerOwnerContext/triggerObjectType/triggerFactId/triggerFactVersion、questionnaireId/revision、template/rule/threshold版本、assignee | 首次source=trigger；整改保持source不变且trigger必须为`ACC/SatisfactionRemediationFact`；创建待办投影不表示客户已提交或满意度通过，Todo完成不反向推进ACC |
| `SatisfactionResultVersionChanged` | ACC | ACC满意度来源投影；未来CLO/SUB | changeType(`RECORDED/INVALIDATED`)、projectId/projectTaskId/taskCode、collectionKey/taskRevisionNo、task/questionnaire/response/result及版本、template/rule/threshold、sourceOwnerContext/sourceObjectType/sourceObjectId/sourceObjectVersion、passed/resultStatus/archiveActorUserId、`invalidationReasonCode/invalidatedByUserId/invalidatedAt`（仅INVALIDATED）、files[{role,sequence,artifactId,versionNo,referenceKey,fileFactVersion,scopeVersion,sha256}] | 与Result事务同提交Outbox；满意度来源只允许taskCode=`T-SAT-SURVEY`并精确投影到同租户同项目`D-SAT-REPORT/T-SAT-SURVEY`唯一根；根缺失/重复/错配待补偿。RECORDED置CURRENT前按Result ID/version重验Owner仍EFFECTIVE且passed且无更新结果；否则只保留非当前历史。INVALIDATED仅在根仍指向该版本时清空；两向乱序均不得恢复失效版本或覆盖新来源，历史ACTIVE文件与归档事实保留 |
