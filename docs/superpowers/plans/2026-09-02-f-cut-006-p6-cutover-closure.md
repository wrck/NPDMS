...
- 每个Task先实现已批准的最小正常链，再运行实现后的聚焦单元测试和正常正向闭环验证；本计划不安排测试先行、RED阶段或负向异常矩阵。业务守卫仍按规格实现，共享Feature/Task状态文件串行回写。
...
- [ ] `CollectionIntentIdentity`固定由受信tenantId、taskId、closureId、deviceId、collectionStage和命令`Idempotency-Key`组成；`request`与`inspectByIntent`均使用该完整元组，不拼接、截断或生成第二身份。
- [ ] 在`src/test`实现受控ProjectScope、PLT和INT-12端口：正常成功、下发失败、回调成功和人工文件均由测试显式选择；同意图重复request/inspect始终返回同一collectionTaskId，不加Spring注解。
- [ ] 实现完成后补合同测试，固定单`deviceId`、五种stage、两种认证联合、同意图同任务重放、临时Secret不进入返回对象，以及PLT事实的artifact/reference/version三轴和scopeVersion；运行：`mvn -pl pms-module-cutover -am -Dtest=CutoverClosurePortContractTest -Dsurefire.failIfNoSpecifiedTests=false test`。
...
- [ ] 实现完成后用`XMLMapperBuilder + BoundSql + MetaObject`固定正常链所用动态集合与CAS属性名，避免文本测试漏掉MyBatis运行绑定错误。
- [ ] 用独立MySQL 8.4空卷执行全量Flyway，验证三表信息架构，并插入一条合法DRAFT、终态采集证据和SUCCESS/FAILED归档样例。
...
- [ ] 实现完成后运行“P6负责人首次保存→DRAFT→再次保存CAS→详情刷新”聚焦测试；夹具使用真实CUT任务/批准方案投影和Task 1受控ProjectScope/PLT端口。
- [ ] 运行真实MySQL“根+附件+平台幂等/审计同事务”正常用例；提交并申请Task 3 Application/MySQL Gate。
...
- [ ] 实现请求：闭环和任务版本CAS、设备必须属于冻结范围、一次调用一个deviceId、Secret只传同步端口；调用INT-12时携带完整`CollectionIntentIdentity`，下发事实与闭环版本保存于CUT本地事务。
- [ ] 实现跨Context恢复：INT-12调用超时或返回后CUT本地CAS/事务失败时，重试必须先以原`CollectionIntentIdentity`调用`inspectByIntent`；已存在则用同一collectionTaskId补齐本地投影，权威NOT_FOUND才允许以同一身份再次request，UNKNOWN/不可用保持原Idempotency-Key待重试，不创建新身份或第二任务。
- [ ] 实现回调：平台先认领callbackEventId；同载荷重放不访问CUT行，新事件只允许匹配DRAFT内的DISPATCH_ACCEPTED并追加唯一终态。
- [ ] 实现人工结果：锁原`DISPATCH_FAILED`或`CALLBACK_FAILED`及PLT事实，原子插入`MANUAL_COLLECTION_RESULT`附件与MANUAL_UPLOAD引用，不覆盖原失败行。
- [ ] 实现完成后运行两条正向聚焦链：DRAFT单设备请求→CALLBACK_SUCCEEDED；DISPATCH_FAILED或CALLBACK_FAILED→人工文件；两条链均验证closureVersion按写入次数递增。
- [ ] 真实MySQL验证“INT-12已返回任务、CUT本地写回失败、同意图重试找回同一collectionTaskId并补齐投影”的恢复链，以及正常dispatch/callback/manual持久结果；提交并申请Task 4 Collection/MySQL Gate。
...
- [ ] 实现完成后运行两条正常聚焦链：完整SUCCESS提交产生归档、释放设备和一个事件；完整FAILED提交同样归档、释放设备但事件为零。
- [ ] 真实MySQL验证两条正常链中闭环/任务/历史/设备/幂等/审计/Outbox的原子结果。
...
- [ ] 真实MySQL跑一个正式暂存批次的正常分类与完成链，验证`sourceCount=retainedCount+issueCount`、mappedCount=0；测试fixture不作为生产迁移完成证据。
...
- [ ] 同一环境跑FAILED归档、下发失败+人工结果、CALLBACK_FAILED+人工结果、同意图采集恢复和同键已完成重放；跨模块事实由test-only受控端口提供。
...
- [ ] 核对三类Owner公开合同与本计划端口逐字段一致，特别确认INT-12生产合同支持完整`CollectionIntentIdentity`的幂等request与inspectByIntent；任何差异先回规格Gate，禁止Adapter猜测或降级。
- [ ] 加入正式Adapter、唯一ApplicationService/QueryService/Controller Bean，确保test substitutes不在生产classpath装配。
- [ ] 用真实Spring上下文跑正常生产链：INT-12首次request返回任务、同意图inspect找回同一任务、回调成功、PLT锁定重验和CUT提交归档。
