...
6. 来源投影不得调用生产者的取当前方法；它以事件`projectTaskVersion`调用原`lockAndRevalidateSatisfactionTask`精确重验同项目、任务、版本和`T-SAT-SURVEY`，再锁同项目唯一`D-SAT-REPORT/T-SAT-SURVEY`根。事件`resultVersion`仅写来源`source_version`，`resultFactVersion`仅传给改名后的`SatisfactionResultFactQuery.expectedFactVersion`。RECORDED只在Fact同时命中resultId、业务版本、factVersion且EFFECTIVE+passed时置CURRENT；失效后旧RECORDED只保留非当前历史，不恢复根指针。INVALIDATED只清空精确指向自身业务版本的根指针。
