...
**Goal:** 在现有资产模块内建立以租户内唯一 SN 为业务身份的 AST 设备主档，将旧设备按原 ID 前向迁移到新 Owner，交付高性能设备列表、六类详情、来源降级、软件版本匹配、时态归属、装配、位置、维保和安全配置 Log 下载，并原子退役旧设备写入口。

**Architecture:** `pms-module-asset` 继续作为 AST 实现模块，`pms-module-asset-api` 提供稳定跨模块设备契约。`ast_device` 保存稳定身份和高频当前事实投影，完整发货、版本、归属、装配、位置、维保和核对事实独立保存；跨模块状态解释由各 Owner 的 `-api` 完成，AST 不访问其他模块业务表。管理端实际路径使用 `/pms/asset/devices`，正式 `/api/v1/pms/devices` 语义由当前网关统一增加 `/api/v1` 前缀，不创建重复 Controller。
...
断言新客户端使用 `/pms/asset/devices`，归属请求发送 `If-Match` 和 `Idempotency-Key`；固定六 Tab；CONP 四字段；归属历史、客户关系、装配树、维保记录和待核对标识；下载接口不接收持久 URL。
...
列表先加载轻量投影；详情和抽屉按需加载；来源失败保留最近成功值并显示状态；冲突后刷新服务端真值；无权限不显示操作按钮，但后端仍独立鉴权。
...
### Task 13: 原子退役旧设备写入口和权限

**Files:**
- Modify: `pms-module-asset/src/main/java/cn/iocoder/yudao/module/pms/asset/controller/admin/equipment/EquipmentController.java`
- Modify: `pms-module-asset/src/main/java/cn/iocoder/yudao/module/pms/asset/service/equipment/EquipmentService.java`
- Modify: `pms-module-asset/src/main/java/cn/iocoder/yudao/module/pms/asset/service/equipment/EquipmentServiceImpl.java`
...
- Create: `sql/migrations/V96__fast001_device_menu_permissions_and_legacy_retirement.sql`
- Create: `scripts/tests/test_fast001_no_legacy_writes.py`
- Create: `pms-module-asset/src/test/java/cn/iocoder/yudao/module/pms/asset/controller/admin/equipment/LegacyEquipmentReadOnlyContractTest.java`

- [ ] **Step 1: 编写旧入口失败测试**

断言旧后端只保留 page/get，旧前端只保留读取，旧页面显示“设备历史（只读）”；V96 新增设备工作台和高风险权限，解除旧 create/update/delete/status-change 角色权限。
...
```powershell
python -m unittest scripts.tests.test_fast001_no_legacy_writes
mvn.cmd -pl pms-module-asset -am -DskipITs=false -Dtest=LegacyEquipmentReadOnlyContractTest test
```
...
在新主档、新写服务和新前端全部可用后，同一任务删除旧 HTTP 写映射、旧前端写调用和旧写权限；保留旧列表与详情读取，不转发新写服务。
...
```powershell
python -m unittest scripts.tests.test_fast001_no_legacy_writes scripts.tests.test_fast001_frontend_contract scripts.tests.test_fast001_migration_contract
mvn.cmd -pl pms-module-asset -am -DskipITs=false -Dtest=LegacyEquipmentReadOnlyContractTest test
```
...
浏览器脚本覆盖：SN 精确查询、轻量列表、六 Tab、来源 `STALE/FAILED/NOT_AVAILABLE`、CONP 组合、项目/客户归属、旧版本冲突、装配树、维保记录、配置 Log 无权限与一次性下载、刷新持久化，以及 320/768/1024/1440 四档响应式；收集控制台和失败网络请求。

Expected：全部 PASS，无未解释控制台或网络错误。
...
| `AC-FAST001-002` 六类详情、来源元数据、失败隔离和最近成功值 | Task 7、8、11、12；Controller、来源降级和浏览器测试 |
...
| `AC-FAST001-007` 客户唯一当前归属、不一致核对及租用/共管历史 | Task 4、9、12；客户归属和关系抽屉验证 |
...
| `AC-FAST001-018` 原 ID 前向迁移和旧入口只读退役 | Task 5、13；迁移测试和旧入口契约测试 |
