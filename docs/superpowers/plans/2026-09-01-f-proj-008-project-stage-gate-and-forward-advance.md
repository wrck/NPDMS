...
修订019衔接：F-PROJ-001 Task 10承接项目级成员指派/调整，支持多名项目经理及一个当前主责；不新增固定T-ASSIGN-PM任务或强制绑定。Task 3B须在候选合同及成员功能形成后复核有效角色成员，主责与其他经理按同角色同权限操作，不能用manager_id单人比较排除其他经理，也不能用预置PM或改任务DONE通过浏览器。本计划旧Task与GO保留历史，当前阶段图/ENTRY等实际差量仍以Spec为准。
