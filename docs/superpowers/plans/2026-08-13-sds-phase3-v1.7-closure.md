...
**Architecture:** 继续将 `project-order-physical-schema.mysql.sql` 定位为历史迁移所需的核心物理子集，不扩张为平台全量模型。V1.7 仅新增已进入正式需求、领域模型和迁移契约的 13 张目标表；所有派生目录、漂移寄存器、MySQL 执行证据和 Phase 3 证据包均由同一 DDL 生成。生产环境、KMS、Telemetry 和性能环境实例继续作为部署或专项验收门禁，不前置阻断当前 SDS 逻辑基线。
...
**Interfaces:**
- Consumes: Task 1 的 63 表 DDL、ADR-0019～ADR-0025、独立数据模型复审结论和需求方对新差量字段/约束清单的确认。
