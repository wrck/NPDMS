...
- `allowlist.json`：允许同步的121个正式文件，必须使用排序后的精确相对路径；其中纳入PRD V1.8增量、已批准ADR和正式Feature Spec，仅纳入Phase 1、Phase 2和Phase 3当前门禁的`README.md`与`gate-status.md`，不纳入独立复审、自审、evidence等过程证据或历史材料；
