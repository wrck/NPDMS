...
> 适用基线：`docs/baseline/prd-v1.8.md`及已批准增量`docs/baseline/prd-v1.8-amendment-001-no-manual-project-draft.md`、`docs/baseline/prd-v1.8-amendment-002-organization-and-asset-location.md`、`docs/baseline/prd-v1.8-amendment-003-pm07-template-match-decision-history.md`、`docs/baseline/prd-v1.8-amendment-004-optional-file-security-scan.md`<br>
...
主链描述平台软件从设计到发布的顺序。历史数据迁移不是所有版本的固定阶段：只有发布包含历史迁移或数据切换时，`AI-MIG-000`才作为Release前置门禁并在批准窗口内执行。平台业务中的生产网络割接不是软件发布阶段，其任务、方案、审批和闭环遵循CUT领域业务流程；实际设备配置变更另行执行高影响操作授权。
...
|Feature Ready|Feature Spec、Requirement追溯、适用设计契约、无相关阻断问题|`READY`或`NOT_READY`|Feature Spec或其任务记录|Implementation|
|Implementation Done|代码、适用测试、Code Review和追溯更新|`PASS`或`BLOCKED`|Git/CI及任务记录|Deployment|
...
Gate Owner不在本基线中虚构固定角色；必须在对应`gate-status.md`、Feature Spec、迁移计划或发布记录中登记。关闭证据优先引用Git、CI、测试和运行系统自然产生的记录，不要求复制成统一证据包。
...
- 涉及的Domain、State、Permission、API、Data Change和Integration Contract；
...
DoD只证明Feature工程实现完整，不等于UAT、生产发布、历史迁移或网络割接已经批准。
...
状态只表达真实业务生命周期。能够从事实派生的工程状态应由机器派生，不得通过直接编辑状态字段晋级，也不得为证据、Git确认或尚无真实批次的迁移准备建立状态机。
...
优先自动化Requirement追溯、Schema一致性、DDL漂移、API兼容性、状态迁移、权限负向测试、Secret扫描、静态检查、单元/集成/迁移测试和审计字段检查。人工评审集中在业务语义、架构取舍、权限边界、高风险数据模型变化和不可逆操作判断。
