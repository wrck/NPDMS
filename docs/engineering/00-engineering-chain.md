...
- 初始化数据已补充：按SDS落字典、菜单、配置等有明确定义的种子；无明确定义内容的以示例数据迁移补充（前向版本、幂等、creator标识、高段ID或专用前缀），且必须覆盖关键维度的组合情况（含精确命中、部分限定、优先级让位、无匹配与停用不参与等场景）；受权威来源映射约束的值域（如CRM属性映射）不得臆造取值；
