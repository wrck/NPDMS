...
- 业务规则、权限正向路径，以及当前真实风险对应的权限拒绝、异常、幂等和并发场景得到验证；低收益异常分支按8.3后置；
...
1. 权限实施和验收的最小确定单元是权限键及其服务端控制点。功能、动作、字段、文件、导出等适用权限键必须具有足够颗粒度，并准确落到服务端查询、command或数据范围校验；权限键或控制点本身不清晰时必须先纠正。
2. 当前不要求穷举或永久固定角色—权限映射。只要权限键、服务端控制点和数据边界已经明确，后续新增角色及权限组合由正式授权配置承载，不因角色—权限分配表未穷举而阻断实施。
3. 当尚未明确的是角色归属或权限组合，而不是权限键、控制点或数据边界时，为实施或验收身份通过正式授权配置授予本闭环全部相关权限键，优先完成正向业务闭环；后续调整权限分配不得改写业务流程、Owner事实或历史记录。
4. 低收益异常分支不阻断当前实施。仅理论可构造、当前支持路径不可达，或不会改变正向业务结果、权限/数据边界及历史事实的异常分支，可以后置到真实需求出现或其适用Gate再处理。
5. 只有会造成业务流程错误、Owner或数据边界错误、服务端鉴权/租户隔离失效、不可变历史破坏，或其他会改变锁定业务结果的实质问题才能阻断。不得以角色枚举不完整、低收益异常未覆盖或形式性检查不足制造阻断。
6. “具备全部权限”必须通过正式授权配置实现，只表示该身份获得当前业务闭环全部相关权限键；不得删除、短路或放宽服务端鉴权、租户隔离、Owner约束和数据范围，也不得以测试账号特例或前端隐藏替代正式控制点。
...
优先自动化Requirement版本切片覆盖、Feature与Task引用、Schema一致性、DDL漂移、Flyway版本唯一性、API兼容性、状态迁移、服务端鉴权与租户/数据边界等实质性权限负向测试、Secret扫描、静态检查、单元/集成/迁移测试和审计字段检查。角色组合穷举和低收益异常分支不作为固定自动化门禁；人工评审集中在业务语义、Feature闭环、架构取舍、权限边界、高风险数据模型变化和不可逆操作判断。
