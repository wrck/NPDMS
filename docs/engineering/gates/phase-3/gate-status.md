...
本次回落只反映PRD基线变化，不否定已验证的历史运行事实，也不提前要求部署时才存在的参数。Phase 1/2已完成V1.8差量GO；新增六表DDL差量、MySQL隔离执行和当前哈希独立复审已完成，但Phase 3正式分册与100项测试追溯尚未关闭，因此本阶段保持`REVALIDATION_REQUIRED / NOT_READY_FOR_SDS_BASELINE_V1.8`。
