...
| 数据模型影响 | REVALIDATION_REQUIRED | ADR-0030的WorkBinding执行契约、完成判定与CUT-03清单六表已完成Phase 2物理设计；Phase 3须形成前向DDL并重新执行P3-E09，旧哈希不自动覆盖新增事实 |
...
本次回落只反映PRD基线变化，不否定已验证的历史运行事实，也不提前要求部署时才存在的参数。Phase 1/2已完成V1.8差量GO；Phase 3正式分册、100项测试追溯和新增六表DDL差量完成并通过独立复审前，本阶段保持`REVALIDATION_REQUIRED / NOT_READY_FOR_SDS_BASELINE_V1.8`。
