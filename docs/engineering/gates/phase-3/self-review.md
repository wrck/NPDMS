...
| 脚本单测 | PASS，77/77；含Phase 2/3、领域实体迁移、领域编码数据库命名、目标字段目录、P3-E09具体裁决清单、逐Owner证据包、DDL逐项裁决、恢复目标、分层备份、温备/冷备及演练授权、永久业务审计、统一导出审计与24小时文件生命周期、Telemetry分层期限与采样、门禁作用域防漂移和本地开发证据误升级拦截正反用例 |
| 业务命名 | PASS |
| `git diff --check` | PASS |
