...
| P3-E09 | 当前候选DDL、字段目录、迁移映射、MySQL 8.4执行证据和证据包已统一到`5EB9742F…4249`；1,883项中994项基线继承、197项已有明确逐项依据、692项`DEFER`。Q07/Q08因哈希变化须重新确认，V1.7仍为评审提案；`reviewOwner`与`approvedDdlSha256`为空 | SDS数据模型基线、任何历史数据迁移、数据库切换和迁移发布声明 | 先由需求方确认当前哈希下Q07/Q08与V1.7候选，再由数据架构+业务Owner+迁移Owner完成逐项复核、独立Reviewer签署和批准哈希；随后重建发布manifest |
