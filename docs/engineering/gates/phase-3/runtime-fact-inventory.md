...
| P3-E09 | 当前候选DDL、字段目录、迁移映射、MySQL 8.4执行证据和证据包已统一到`5EB9742F…4249`；1,883项逐项寄存器已全部形成基线继承或需求/ADR决策，无`DEFER`项，但`reviewOwner`与`approvedDdlSha256`仍为空 | SDS数据模型基线、任何历史数据迁移、数据库切换和迁移发布声明 | 数据架构+业务Owner+迁移Owner复核逐项决策，完成独立Reviewer签署并生成`approvedDdlSha256`；随后以批准哈希重建发布manifest |
