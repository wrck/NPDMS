...
| P3-E09 | 当前候选DDL、字段目录、迁移映射、MySQL 8.4执行证据和证据包已统一到`5EB9742F…4249`；1,883项中994项基线继承、197项已有明确逐项依据、692项`DEFER`。Q07/Q08因哈希变化须重新确认，V1.7仍为评审提案；三组仅覆盖584项`DEFER`，另108项已分类为Q09～Q14；`reviewOwner`与`approvedDdlSha256`为空 | SDS数据模型基线、任何历史数据迁移、数据库切换和迁移发布声明 | 先由需求方按`p3-e09-confirmation-packet.md`确认当前哈希九组完整清单，再由数据架构+业务Owner+迁移Owner完成逐项复核、独立Reviewer签署和批准哈希；随后重建发布manifest |

P3-E01～E06是部署、专项验收或生产发布证据，不阻断逻辑SDS基线；缺失时仍严格阻断其登记的下游门禁。P3-E07按具体Feature阻塞联调/上线；P3-E08阻塞前端Feature验收或发布；P3-E09会改变数据模型，因此在`AI-MIG-000`关闭前阻塞SDS数据模型基线、历史数据迁移实施与切换，但不阻塞与该候选模型无关且无迁移的独立Feature准备工作。
