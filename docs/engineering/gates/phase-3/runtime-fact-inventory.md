...
| P3-E09 | 当前DDL、字段目录、迁移映射、MySQL 8.4执行证据和证据包已统一到`5EB9742F…4249`；ADR-0028九组清单覆盖1,883项，`DEFER=0`；独立复审`GO`，P3-E09不定义迁移批准哈希 | 仅包含历史数据迁移、数据库切换的Release | `MODEL_BASELINE_READY`已解除SDS/Feature数据模型输入阻断；普通功能Release为`NOT_APPLICABLE`。适用Release的`AI-MIG-000`须先验证范围、水位、程序、对账、回退和执行窗口，达到`VERIFIED`后只在批准窗口内执行 |
