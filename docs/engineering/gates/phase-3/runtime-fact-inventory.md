...
| P3-E09 | 当前DDL、字段目录、迁移映射、MySQL 8.4执行证据和证据包已统一到`5EB9742F…4249`；ADR-0028九组清单覆盖1,883项，`DEFER=0`；独立复审`GO`，P3-E09不定义迁移批准哈希 | 历史数据迁移、数据库切换和迁移发布声明 | `MODEL_BASELINE_READY`已解除SDS/Feature数据模型输入阻断；`AI-MIG-000`在真实批次验证范围、水位、程序、对账和回退前，禁止执行迁移或切换 |
