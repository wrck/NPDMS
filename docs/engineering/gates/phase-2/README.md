...
> 迁移边界：93对象/104来源绑定/1排除源；Feature前向对象不进入当前核心DDL。仅当发布包含历史迁移或数据切换时，才由`AI-MIG-000`作为Release前置门禁并绑定批准窗口，普通功能发布不适用
...
> 2026-09-02 F-CUT-010候选同步：新增CUT-08外部申请引用、原始状态revision和人工证据三个`NONE_NEW / FEATURE_FORWARD_MIGRATION`对象后，当前契约为96对象/107来源绑定/1排除源；仅同步Feature前向机器合同，不代表Feature Ready或实现通过。
