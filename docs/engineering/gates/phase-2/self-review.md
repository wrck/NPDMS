...
| S2-V18-13 | PRD修订008更正COM-01订单范围地点并触发F-COM-001聚焦物理复核 | 当前候选契约为94对象/107来源绑定/1排除源；办事处发生时快照、PROJ/ACC窄Owner契约、精确物理字段，以及V70的10项必填目标映射（含明细序号唯一键规则）和逐项缺失负向门禁已形成整改候选；P3-E09差量待整改复审，Q-FCOM-001继续BLOCKED_BY_SPEC，不改变Phase 2总体历史结论 |
| S2-V18-14 | PRD修订009补充项目阶段、验收范围与验收报告有序事实 | ADR-0037及02/04/05/06/08/08a/09/10/11/15/16已同步并通过独立复审：ProjectStageSnapshot成为绑定来源，报告与绑定解耦，进入/新范围两条路径同事务、统一PROJ→COM→ACC锁序；`acceptance_id`从聚焦物理差量删除，Q-FCOM-002继续窄阻断 |
...
| S2-V18-16 | F-ACC-001形成ACC-03完整报告闭环与ACC-04初验/终验来源切片 | 当前候选为94对象/109来源绑定/1排除源；同一Gate整改已补DRAFT/EFFECTIVE/SUPERSEDED/REVOKED状态、current_marker生成条件、发布/替换/撤销原子转换、变更类型事件及完整附件集合，并以既有`acc_project_deliverable`根和只追加来源版本/附件从表保留历史；其余PROJ任务Owner、范围绑定回归、V17不复用和权限边界不变，待复审 |
...
| 核心迁移Schema契约 | PASS |
| 领域实体迁移对齐 | PASS，当前候选94对象/109来源绑定/1排除源；不代表独立复审通过 |
