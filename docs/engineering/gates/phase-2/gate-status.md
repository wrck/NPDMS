...
| F-COM-001修订009状态SDS差量 | IN_REVIEW | 候选按项目阶段快照驱动ACC范围绑定，移除绑定对初验/终验报告及`acceptance_id`的依赖；阶段进入和验收阶段内新范围生效分别与绑定原子提交，统一锁序为PROJ→COM→ACC；Q-FCOM-002仍只阻断退出/回退关闭或解锁。本行不表示ADR-0037、物理差量、接口或事务已获批准 |
