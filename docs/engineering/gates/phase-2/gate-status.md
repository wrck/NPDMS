...
| F-PLT-002聚焦动态表单候选 | IN_REVIEW | 候选形成时为90对象/101来源绑定/1排除源，修订007当前总体为98对象/116来源绑定/1排除源；PLT模板、修订、通用实例与Preparation专用实例分离，旧`pms_eng_form_*`仅作复用审计证据且不迁移、不双写；不代表Feature Ready通过 |
