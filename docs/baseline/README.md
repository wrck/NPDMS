...
| 文件 | 用途 |
|---|---|
| `prd-v1.8.md` | PRD V1.8修订019的当前内容快照，供SDS、Feature和测试读取；先改已批准源PRD再同步，不只改单侧 |
...
5. 基线生效后的业务变化必须通过CHG-01记录影响、批准和新版本；当前修订以源PRD修订记录与元数据为准，增量文件只作为批准依据和审计追溯，不得在读取快照后重复叠加。旧签署/校验结果仅证明原范围，不因当前身份更新晋级实现。
