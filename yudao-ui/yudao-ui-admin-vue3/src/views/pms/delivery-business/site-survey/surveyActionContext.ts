...
export const loadSurveyActionContext = async (
  ...
  if (kind !== 'exchange') {
    if (survey.businessValues?.railTrayRequired !== true)
      throw new Error('工勘尚未选择需要导轨、托盘')
    return base
  }
  const selected = (survey.businessValues?.selectedMaterials ||
    []) as SurveyMaterialSelection[]
  const item = selected.find((row) => row.sn === sn && row.reason?.trim())
  if (survey.businessValues?.materialMatches !== false || !item)
