...
vi.mock('@/api/pms/engineering/site-survey/entity', () => ({ getSiteSurvey: vi.fn() }))
vi.mock('@/api/pms/asset/device', () => ({ getDevice: vi.fn() }))
import { getSiteSurvey } from '@/api/pms/engineering/site-survey/entity'
...
describe('survey shortcuts preserve real source identities', () => {
  ...
  it('uses the saved survey context for procurement without inventing quantities', async () => {
    vi.mocked(getSiteSurvey).mockResolvedValue({ id: 1, projectId: 7, name: '工勘', status: 0, businessValues: { railTrayRequired: true } })
  ...
  it('revalidates a selected device and does not confuse AST deviceId with legacy equipmentId', async () => {
    vi.mocked(getSiteSurvey).mockResolvedValue({ id: 1, projectId: 7, name: '工勘', status: 0, businessValues: { materialMatches: false, selectedMaterials: [{ deviceId: 9, sn: 'SN1', reason: '接口不匹配' }] } })
  ...
  it('does not launch a shortcut from a completed survey or a condition that is false', async () => {
    ...
    vi.mocked(getSiteSurvey).mockResolvedValue({ status: 0, businessValues: { railTrayRequired: false } })
