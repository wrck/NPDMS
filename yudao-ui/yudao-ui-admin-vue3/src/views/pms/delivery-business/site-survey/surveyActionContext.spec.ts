...
describe('survey shortcuts preserve real source identities', () => {
  ...
  it('uses the saved survey context for procurement without inventing quantities', async () => {
    vi.mocked(getSiteSurvey).mockResolvedValue({ id: 1, projectId: 7, code: 'SS', name: '工勘', status: 0, businessValues: { railTrayRequired: true } })
  ...
  it('does not launch a shortcut from a completed survey or a condition that is false', async () => {
    vi.mocked(getSiteSurvey).mockResolvedValue({ projectId: 7, code: 'SS', name: '工勘', status: 3 })
    await expect(loadSurveyActionContext(1, 'material')).rejects.toThrow('可编辑')
    vi.mocked(getSiteSurvey).mockResolvedValue({ projectId: 7, code: 'SS', name: '工勘', status: 0, businessValues: { railTrayRequired: false } })
