...
describe('one outsourcing shortcut for a whole site survey', () => {
  ...
  it('dynamic fields do not overwrite the whole-survey shortcut or association', () => {
    ...
    expect(() => mergeSurveyValues(entity, { outsourceRequired: false, outsourceRequestId: 55 })).toThrow('未绑定')
    expect(entity.outsourceRequired).toBe(true)
    expect(entity.outsourceRequestId).toBe(44)
  })
})
