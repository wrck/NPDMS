...
const render = async () => {
  ...
  const { app }=mount(defineComponent({ setup:()=>()=>h(Panel,{ projectId:8,taskId:'2098269306807468033',taskVersion:2,ref:child,onChanged:changed }) }),{},
  ...
  apps.push(app); await flush(); return { state:child.value.$.setupState,changed }
}
beforeEach(()=>{
  vi.clearAllMocks()
...
it('keeps responsible and executor commands distinct and retries one intent with the same key',async()=>{
  const { state,changed }=await render(); state.openRole('RESPONSIBLE'); state.userId=15; state.reason='调整协调职责'
