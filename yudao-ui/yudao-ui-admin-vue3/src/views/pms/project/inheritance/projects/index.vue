    ...
    <Dialog v-model="editVisible" title="编辑项目（可编辑属性）" width="520px">
      <el-form :model="editForm" label-width="100px">
        ...
        <el-form-item label="客户编码">
          <el-input :model-value="editForm.customerCode" readonly />
        </el-form-item>
        <el-form-item label="客户名称">
          <el-input :model-value="editForm.customerName" readonly />
        </el-form-item>
...
defineOptions({ name: 'PmsProjectInheritanceList' })
...
const submitCreate = async () => {
  const sites: ProjectSiteReqVO[] | undefined =
    createForm.locationMode === 'sites'
      ...
      : undefined
  const payload: CustomerSelectedProjectCreate = {
  ...
  createErrorMessage.value = ''
  try {
    const created = await createCustomerSelectedProject(payload, idempotencyKey)
    message.success(
...
const goDetail = (row: ProjectMasterVO) => {
  router.push({
    path: '/pms-inheritance/project-detail',
