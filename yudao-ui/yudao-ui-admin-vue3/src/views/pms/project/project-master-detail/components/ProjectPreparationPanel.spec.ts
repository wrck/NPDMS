...
describe('F-SOL-002 project preparation panel', () => {
  ...
  it('mounts the project workspace and obeys server allowed actions', () => {
    ...
    expect(panel).toContain("item.allowedActions.includes('CONFIRM_ITEM')")
    expect(panel).toContain("item.allowedActions.includes('CONFIRM_NOT_APPLICABLE_ITEM')")
    expect(panel).toContain("item.allowedActions.includes('RETURN_ITEM')")
