    ...
    <el-form label-position="top" :disabled="readonly || loading">
      <el-form-item label="办理页面 / 表单">
        ...
        >
          ...
          <el-option
            v-if="
              currentView && !views.some((view) => sameBusinessViewId(view.id, currentView?.id))
            "
            :value="currentView.id"
            :label="`${viewLabel(currentView)}（仅保留历史）`"
            disabled
          />
        </el-select>
        <p class="help">{{ currentDescription }}。办理中的提交、审批与文件操作保留在同一任务内。</p>
        <p v-if="!loading && !views.length" class="help"
          >暂无可选的已发布办理界面。{{
            canRegister
              ? '可在下方从真实已接入功能登记页面或已发布表单。'
              : '请联系有业务视图管理权限的人员登记；现有绑定不会改变。'
          }}</p
        >
      </el-form-item>
      <template v-if="selectedMetadata">
        <div class="entity-summary">
          <div
            ><span>业务实体</span
            ><strong>{{ entityLabel(selectedMetadata.entityType) }}</strong></div
          >
          <div
            ><span>业务归属</span
            ><strong>{{ ownerLabel(selectedMetadata.ownerContext) }}</strong></div
          >
        </div>
        <el-form-item label="业务对象关联方式">
          <el-select
            :model-value="source"
            :disabled="!modelValue || !canBind"
          ...
          >
            ...
            <el-option
              value="CREATE_ON_FIRST_ACTION"
              label="首次办理时创建（当前未接入创建服务）"
              disabled
            />
          </el-select>
      ...
      <el-form-item label="完成依据">
        ...
        <el-select
          v-if="modelValue?.completion"
          :model-value="modelValue.completion.quantifier"
          @update:model-value="changeQuantifier"
        >
          <el-option value="ALL" label="全部关联记录满足" /><el-option
            value="ANY"
            label="至少一条关联记录满足"
          />
        </el-select>
        <div class="completion" :class="{ pending: completionPending }">
          ...
          <p class="help">{{
            completionPending
              ? '办理界面可配置并保存；现有完成依据尚不能证明该业务完成，不能发布为可运行模板。'
              : '保留当前已发布完成依据，由服务端结合实际业务事实校验；选择页面不授予业务权限。'
          }}</p>
        </div>
      </el-form-item>
      <el-button v-if="modelValue && !readonly" link @click="emit('update:modelValue', undefined)"
        >撤销本次办理方式调整</el-button
      >
      <el-button
        v-if="canRegister && canBind && !readonly && contract"
        plain
        @click="registerOpen = !registerOpen"
        >{{ registerOpen ? '收起登记' : '使用其他已接入页面或表单' }}</el-button
      >
      <section v-if="registerOpen && canRegister && !readonly" class="registration">
        ...
        <el-form-item label="页面或表单类型">
          <el-select
            ...
            placeholder="选择实际已接入的功能"
            @update:model-value="formId = undefined"
          >
            <el-option
              v-for="component in components"
              :key="catalogKey(component)"
              :value="catalogKey(component)"
        ...
        <el-form-item v-if="selectedComponent?.viewSource === 'DYNAMIC_FORM'" label="已发布表单">
          <el-select v-model="formId" filterable placeholder="按名称选择已发布表单">
            <el-option
              v-for="form in forms"
              :key="form.currentPublishedRevisionId"
              :value="form.currentPublishedRevisionId"
              :label="`${form.templateName} · 第${form.currentPublishedRevisionNo}版`"
            />
          </el-select>
        </el-form-item>
        <p class="help"
          >只登记配置，不复制业务页面或表单内容。保存草稿时登记并发布视图，再保存任务绑定；最终仍由服务端校验权限。</p
        >
        <el-button
          :disabled="
            !selectedComponent || (selectedComponent.viewSource === 'DYNAMIC_FORM' && !formId)
          "
          @click="chooseComponent"
          >用于此任务</el-button
        >
      </section>
      <p v-if="!canBind && !readonly" class="help"
        >调整办理界面需要模板维护及定义发布权限；已有配置仍可查看。</p
      >
    </el-form>
  </section>
</template>
...
const canBind = computed(
  () =>
    hasPermission(['pms:project-template:update']) &&
    hasPermission(['pms:project-template:publish'])
)
...
const entityLabel = (type: string) =>
  ({
    ...
    DYNAMIC_FORM_INSTANCE: '业务表单'
  })[type] ?? '关联业务'
const ownerLabel = (owner: string) =>
  ({ SOL: '方案与准备', PLATFORM: '平台表单' })[owner] ?? '原业务模块'
const viewLabel = (row: BusinessViewRegistrationVO) => {
  const form = forms.value.find((form) =>
    sameBusinessViewId(form.currentPublishedRevisionId, row.dynamicFormRevisionId)
  )
  return `${form?.templateName ?? componentLabel(row)} · ${row.viewKey} · 第${row.revisionNo}版`
}
const selectedComponent = computed(() =>
  components.value.find((row) => catalogKey(row) === componentKey.value)
)
const selectedMetadata = computed(() =>
  props.modelValue
    ? 'view' in props.modelValue
...
const source = computed(
  () => props.modelValue?.strategy ?? contract.value?.binding.payload.instanceResolutionStrategy
)
const currentDescription = computed(() =>
  props.modelValue
    ? 'view' in props.modelValue
      ? `待保存：${viewLabel(props.modelValue.view)}`
      : `待保存：${componentLabel(props.modelValue.component)}`
    : currentView.value
      ? `当前：${viewLabel(currentView.value)}`
      : contract.value?.binding.payload.bindingType === 'TASK_NATIVE'
        ? '当前：项目任务办理'
        : '保留当前办理配置'
)
...
const chooseCompletion = (factCode: string) => {
  if (
    props.modelValue &&
    !props.readonly &&
    completionOptions.value.some((option) => option.code === factCode)
  )
...
const changeQuantifier = (quantifier: 'ALL' | 'ANY') => {
  if (props.modelValue?.completion && !props.readonly)
    emit('update:modelValue', {
      ...props.modelValue,
      completion: { ...props.modelValue.completion, quantifier }
    })
}
const completionPending = computed(
  () =>
    !props.modelValue?.completion &&
    Boolean(selectedMetadata.value) &&
    (!contract.value || containsNativeCompletion(contract.value.completion.payload))
)
  ...
  return (
    {
      ...
      TASK: '关联任务结果',
      STATE: '业务阶段结果'
    }[rule.operator ?? rule.predicate] ?? '已配置业务事实规则'
  )
})
let generation = 0
const readContract = async () => {
  ...
  try {
    ...
    const view = result.binding.payload.businessViewRevisionId
      ? await Views.getBusinessView(result.binding.payload.businessViewRevisionId)
      : undefined
    if (current !== generation) return
    contract.value = result
    currentView.value = view
  } catch (error) {
    if (current === generation) failure.value = errorText(error)
  }
}
const load = async () => {
  loading.value = true
...
const chooseComponent = () => {
  ...
  try {
    ...
    emit('update:modelValue', {
      ...
      dynamicFormRevisionId: formId.value,
...
watch(
  () => [
    props.task.definitionRevisionId,
    props.task.workBindingRevisionId,
    props.task.permissionPolicyRevisionId,
    props.task.completionRuleRevisionId
  ],
  readContract,
  { immediate: true }
)
