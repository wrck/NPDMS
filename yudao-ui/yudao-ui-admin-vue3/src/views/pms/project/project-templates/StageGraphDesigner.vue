  ...
  <section class="stage-graph-designer" aria-label="阶段关系可视化设计器">
    <div class="graph-toolbar">
      <div>
        ...
        <span v-else>点击关系线可配置分支；排序不会生成关系。</span>
      </div>
      ...
      <div class="graph-canvas" role="application" aria-label="模板阶段关系图">
        <svg :width="layout.width" :height="layout.height" :viewBox="`0 0 ${layout.width} ${layout.height}`">
          ...
          <g v-for="edge in layout.edges" :key="edge.key" class="edge-group" @click.stop="selectEdge(edge.source)">
            ...
            <path
              :d="edge.path"
              :class="['graph-edge', { selected: selectedEdge === edge.source, default: edge.source.default }]"
              marker-end="url(#stage-designer-arrow)"
            />
          ...
          <g
            ...
            :class="['node-group', { linking: linking, source: linkSource === node.code }]"
            role="button"
            tabindex="0"
          ...
          >
            <rect
              :class="['graph-node', { start: node.start, terminal: node.terminal, cyclic: node.cyclic }]"
              :width="nodeWidth"
              :height="nodeHeight"
              rx="8"
            />
            ...
            <text :x="nodeWidth - 12" :y="20" class="graph-node-badge" text-anchor="end">
              {{ node.start ? 'START' : node.terminal ? 'END' : '' }}
            </text>
          </g>
        </svg>
      </div>
      ...
      <aside class="edge-inspector" aria-label="关系属性">
        <template v-if="selectedEdge">
          <div class="inspector-heading">
            <div><strong>{{ selectedEdge.fromStageCode }} → {{ selectedEdge.toStageCode }}</strong><span>{{ selectedEdge.transitionCode }}</span></div>
            <el-button v-if="!readonly" link type="danger" @click="removeSelected">删除</el-button>
          </div>
          <el-form label-position="top" :disabled="readonly">
            <el-form-item label="条件规则">
              <DefinitionSelect
                :model-value="selectedEdge.conditionRuleRevisionId"
                kind="COMPLETION_RULE"
                :disabled="readonly || !!selectedEdge.default"
                @update:model-value="selectedEdge!.conditionRuleRevisionId = $event"
              />
              <span v-if="selectedEdge.default" class="field-hint">默认分支不能同时配置条件规则。</span>
            </el-form-item>
            <el-form-item label="优先级（小者优先）">
              <el-input-number v-model="selectedEdge.priority" :precision="0" controls-position="right" class="!w-full" />
            </el-form-item>
            <el-form-item label="默认分支">
              <el-switch :model-value="!!selectedEdge.default" @update:model-value="setDefault" />
            </el-form-item>
          </el-form>
        </template>
        <template v-else>
    ...
    <p class="graph-legend">
      ...
      <span>虚线＝默认分支</span><span v-if="cyclic.length" class="legend-cyclic">循环受阻：{{ cyclic.join('、') }}</span>
...
const failure = ref('')
const selectedEdge = ref<StageTransition>()
...
const graph = computed(() => {
  ...
  let frontier = stages
    .filter((stage) => (indegree.get(stage.stageCode) ?? 0) === 0)
...
const layout = computed(() => {
  ...
  const edges = graph.value.edges.map((edge, index) => {
    ...
    return {
      ...
      labelX: (x1 + x2) / 2,
      labelY: (y1 + y2) / 2 - 5,
      label: edge.default ? '默认' : edge.conditionRuleRevisionId ? `规则 ${edge.conditionRuleRevisionId}` : `P${edge.priority ?? 0}`
    }
...
const selectNode = (code: string) => {
  ...
  failure.value = ''
  if (!linkSource.value) {
    linkSource.value = code
    return
  }
  if (linkSource.value === code) {
    failure.value = '来源阶段和目标阶段不能相同。'
    return
  }
  const transitions = props.content.transitions ?? (props.content.transitions = [])
  if (transitions.some((edge) => edge.fromStageCode === linkSource.value && edge.toStageCode === code)) {
    failure.value = `${linkSource.value} → ${code} 已存在关系，不重复创建。`
    return
  }
  ...
  let transitionCode = baseCode
  let suffix = 2
  while (transitions.some((edge) => edge.transitionCode === transitionCode)) transitionCode = `${baseCode}_${suffix++}`
...
const selectEdge = (edge: StageTransition) => {
  selectedEdge.value = edge
  failure.value = ''
}
const setDefault = (value: boolean) => {
  ...
  if (value) {
    const conflict = (props.content.transitions ?? []).find((edge) =>
      edge !== selectedEdge.value && edge.fromStageCode === selectedEdge.value!.fromStageCode && edge.default
    )
    if (conflict) {
      failure.value = `来源阶段 ${selectedEdge.value.fromStageCode} 已有默认分支 ${conflict.transitionCode}，请先明确取消原默认分支。`
      return
    }
    selectedEdge.value.conditionRuleRevisionId = undefined
  }
  selectedEdge.value.default = value
}
const removeSelected = () => {
  if (!selectedEdge.value || props.readonly) return
  const index = props.content.transitions?.indexOf(selectedEdge.value) ?? -1
...
.graph-layout { display: grid; grid-template-columns: minmax(0, 1fr) 260px; border: 1px solid var(--el-border-color-lighter); border-radius: 8px; overflow: hidden; }
...
.edge-inspector { padding: 16px; border-left: 1px solid var(--el-border-color-lighter); background: var(--el-bg-color); }
.inspector-heading { display: flex; align-items: flex-start; justify-content: space-between; gap: 8px; margin-bottom: 16px; }.inspector-heading strong { display: block; font-size: 14px; }.inspector-heading span { display: block; margin-top: 5px; color: var(--el-text-color-secondary); font-size: 11px; }
.field-hint { display: block; margin-top: 6px; color: var(--el-text-color-secondary); font-size: 11px; }
...
.node-group { cursor: default; outline: none; }.node-group.linking { cursor: crosshair; }.node-group.linking:hover .graph-node, .node-group.source .graph-node, .node-group:focus-visible .graph-node { stroke: var(--el-color-primary); stroke-width: 2; }
.graph-node-code { font-size: 12px; font-weight: 600; fill: var(--el-color-primary); }.graph-node-name { font-size: 12px; fill: var(--el-text-color-primary); }.graph-node-badge { font-size: 9px; fill: var(--el-text-color-secondary); }
.graph-edge { fill: none; stroke: var(--el-border-color-darker); stroke-width: 1.5; pointer-events: none; }.graph-edge.default { stroke-dasharray: 6 4; }.graph-edge.selected { stroke: var(--el-color-primary); stroke-width: 2.2; }.edge-hit { fill: none; stroke: transparent; stroke-width: 14; cursor: pointer; }.graph-arrow { fill: var(--el-border-color-darker); }.edge-label-bg { fill: var(--el-bg-color); stroke: var(--el-border-color-extra-light); }.graph-edge-label { font-size: 10px; text-anchor: middle; fill: var(--el-text-color-secondary); pointer-events: none; }
...
@media (max-width: 900px) { .graph-layout { grid-template-columns: 1fr; }.edge-inspector { border-left: 0; border-top: 1px solid var(--el-border-color-lighter); }.graph-toolbar { align-items: flex-start; flex-direction: column; } }
</style>
