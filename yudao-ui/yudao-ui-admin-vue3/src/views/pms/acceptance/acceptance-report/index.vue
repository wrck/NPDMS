<template>
  <ContentWrap>
    <el-alert v-if="embedded" :title="executionClient ? '当前报告操作由项目执行中心校验；附件仍使用原业务文件权限。' : '报告发布/附件办理尚未接入当前任务上下文，请从原业务入口处理'" type="info" :closable="false" show-icon />
...
const load = async () => {
  const token = ++listSequence, project = activeProject
  errorText.value = ''
  ...
  try {
    ...
    if (token === listSequence) { activities.value = []; errorText.value = '验收活动加载失败，请检查访问权限或重试；这不代表验收已完成。' }
...
const switchContext = async () => {
  ...
  if (sameBusinessViewId(project, activeProject) && String(object ?? '') === String(activeObject ?? '')) { contextBlocked.value = false; return }
  listSequence++
...
watch(() => route.query.projectId, value => { if (!scoped.value) query.projectId = isBusinessViewId(value) ? value : undefined })
