...
public class SatisfactionTaskManagementService {
    ...
    protected AssignmentResult assignOnce(Long tenantId, Long actorUserId, Long taskId, Long targetUserId,
        ...
        // 通过 PROJ 参与者事实接口判断指派对象是否为项目当前参与人，不直接访问 PROJ 数据层。
        var participant = participantFactApi.inspect(new ProjectParticipantFactQuery(
                task.getProjectId(), targetUserId, null, LocalDateTime.now()));
        if (participant == null) throw new IllegalStateException("SATISFACTION_TASK_ASSIGNEE_NOT_AUTHORIZED");
