...
public class TaskBindingHostRegistry {
    ...
    public TaskBindingHostRegistry(List<TaskBindingHostProvider> providers) {
        this.providers = providers.stream()
                .flatMap(provider -> provider.bindingTypes().stream().map(type -> Map.entry(type, provider)))
    ...
    public TaskBindingInspection inspect(String bindingType, TaskBindingInspectionQuery query) {
        ...
        try {
            ...
            return result == null
                    ? TaskBindingInspection.failed(bindingType, "BINDING_FACT_UNKNOWN")
                    : new TaskBindingInspection(bindingType, result.allowedActions(), result.factVersion(), result.recoverableError());
