...
public class SatisfactionResultOutboxDeliveryJob implements JobHandler {
    private static final int BATCH_SIZE = 50;
    private final PlatformOutboxDeliveryApi outboxDeliveryApi;
    private final SatisfactionResultSourceProjectionService projectionService;
    ...
    private String deliver() {
        ...
        List<PlatformOutboxMessageDTO> messages = outboxDeliveryApi.claimDue(new PlatformOutboxClaimQuery(
                now, BATCH_SIZE, Set.of("SatisfactionResultVersionChanged")));
        int delivered = 0;
        int retried = 0;
        for (PlatformOutboxMessageDTO message : messages) {
            try {
                ...
                retried++;
                log.warn("[execute][满意度Result事件({})投影失败]", message.eventId(), failure);
            }
        }
        return String.format("满意度Result事件投影成功 %d 条，待重试 %d 条", delivered, retried);
    }

    private SatisfactionResultVersionChangedMessage parse(PlatformOutboxMessageDTO message) {
        if (message == null || !"SatisfactionResultVersionChanged".equals(message.eventType())
                || !Objects.equals(message.tenantId(), TenantContextHolder.getRequiredTenantId())) {
            throw new IllegalArgumentException("invalid satisfaction result outbox metadata");
        }
        SatisfactionResultVersionChangedMessage event = JsonUtils.parseObject(
                message.payload(), SatisfactionResultVersionChangedMessage.class);
        if (event == null || !Objects.equals(event.eventId(), message.eventId())
                || !Objects.equals(event.tenantId(), message.tenantId())) {
            throw new IllegalArgumentException("invalid satisfaction result outbox payload");
        }
        return event;
    }
}
