 ...
 * reading/locking dedicated sol_eng_site_survey fixture rows, NOT the PRE production provider.
...
class ProjectTaskBusinessMySqlIntegrationTest {
    ...
    void cleanup() {
        ...
        try {
            if (committedFixtures) {
                ...
                deleteIds("sol_eng_site_survey", ownerIds);
        ...
        try (var executor = Executors.newSingleThreadExecutor()) {
            probe.afterInspect = () -> {
                Future<Integer> changed = executor.submit(() -> jdbc.update(
                        "UPDATE sol_eng_site_survey SET version=version+1 WHERE id=? AND creator=?", ownerId, prefix));
    ...
    private void concurrentIntent(boolean sameKey) throws Exception {
        ...
        try {
            ...
            try (var connection = dataSource.getConnection(); var statement = connection.createStatement()) {
                ...
                try {
                    SQLException conflict = assertThrows(SQLException.class, () -> statement.executeQuery(
                            "SELECT id FROM sol_eng_site_survey WHERE id=" + ownerId + " FOR UPDATE NOWAIT"));
            ...
            try (var connection = dataSource.getConnection(); var statement = connection.createStatement()) {
                connection.setAutoCommit(false);
                try (var row = statement.executeQuery("SELECT id FROM sol_eng_site_survey WHERE id=" + ownerId + " FOR UPDATE NOWAIT")) {
    ...
    private void insertOwner(long id, long tenantId, long project) {
        jdbc.update("INSERT INTO sol_eng_site_survey (id,project_id,code,name,status,version,tenant_id,creator,updater) "
    ...
    private int ownerVersion() { return jdbc.queryForObject("SELECT version FROM sol_eng_site_survey WHERE id=?", Integer.class, ownerId); }
    ...
    public static class TestSourceOwner implements TaskBusinessObjectProvider {
        ...
        private BusinessObjectFact read(Context context, String objectId, boolean lock) {
            ...
            String sql = "SELECT id,name,version,status FROM sol_eng_site_survey WHERE id=? AND tenant_id=? "
