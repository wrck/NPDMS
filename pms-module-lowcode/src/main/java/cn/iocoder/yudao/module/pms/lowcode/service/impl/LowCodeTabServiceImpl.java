...
public class LowCodeTabServiceImpl
    ...
    public LowCodeTab update(LowCodeTab tab) {
        if (tab == null || tab.getId() == null) {
            throw new LowCodeException("标签页配置或ID不能为空");
        }
        LowCodeTab existing = baseMapper.selectById(tab.getId());
        if (existing == null) {
            throw new LowCodeException("标签页配置不存在");
        }
    ...
    public void publish(Long id) {
        ...
        if (!STATUS_DRAFT.equals(tab.getStatus())) {
            throw new LowCodeException("当前状态不允许发布，当前状态: " + tab.getStatus());
        }
        if (!StringUtils.hasText(tab.getTabConfig())) {
            throw new LowCodeException("标签页配置内容不能为空，无法发布");
        }
    ...
    public void archive(Long id) {
        ...
        if (!STATUS_PUBLISHED.equals(tab.getStatus())) {
            throw new LowCodeException("当前状态不允许归档，当前状态: " + tab.getStatus());
        }
    ...
    public byte[] exportConfig(String code) {
        ...
        LowCodeTab tab = this.getOne(new LambdaQueryWrapper<LowCodeTab>()
                ...
                .last("LIMIT 1"));
        if (tab == null) {
            throw new LowCodeException("标签页配置不存在: " + code);
        }
        try {
            ...
            throw new LowCodeException("导出标签页配置失败: " + e.getMessage());
        }
    }
    ...
    public LowCodeTab importConfig(String json) {
        ...
        try {
            ...
            throw new LowCodeException("解析标签页配置 JSON 失败: " + e.getMessage());
        }
        if (!StringUtils.hasText(tab.getCode())) {
            throw new LowCodeException("导入的标签页配置缺少编码 code");
        }
