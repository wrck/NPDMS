...
public class LowCodeListServiceImpl
    ...
    public LowCodeList update(LowCodeList list) {
        if (list == null || list.getId() == null) {
            throw new LowCodeException("列表配置或ID不能为空");
        }
        LowCodeList existing = baseMapper.selectById(list.getId());
        if (existing == null) {
            throw new LowCodeException("列表配置不存在");
        }
    ...
    public void publish(Long id) {
        ...
        if (!STATUS_DRAFT.equals(list.getStatus())) {
            throw new LowCodeException("当前状态不允许发布，当前状态: " + list.getStatus());
        }
        if (!StringUtils.hasText(list.getListConfig())) {
            throw new LowCodeException("列表配置内容不能为空，无法发布");
        }
    ...
    public void archive(Long id) {
        ...
        if (!STATUS_PUBLISHED.equals(list.getStatus())) {
            throw new LowCodeException("当前状态不允许归档，当前状态: " + list.getStatus());
        }
    ...
    public byte[] exportConfig(String code) {
        ...
        LowCodeList list = this.getOne(new LambdaQueryWrapper<LowCodeList>()
                ...
                .last("LIMIT 1"));
        if (list == null) {
            throw new LowCodeException("列表配置不存在: " + code);
        }
        try {
            ...
            throw new LowCodeException("导出列表配置失败: " + e.getMessage());
        }
    }
    ...
    public LowCodeList importConfig(String json) {
        ...
        try {
            ...
            throw new LowCodeException("解析列表配置 JSON 失败: " + e.getMessage());
        }
        if (!StringUtils.hasText(list.getCode())) {
            throw new LowCodeException("导入的列表配置缺少编码 code");
        }
