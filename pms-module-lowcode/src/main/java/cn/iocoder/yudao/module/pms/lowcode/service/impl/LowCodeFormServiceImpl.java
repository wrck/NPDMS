 ...
 * 状态机：DRAFT → PUBLISHED → ARCHIVED，非法状态流转抛出 {@link LowCodeException}。</p>
 */
@Slf4j
@Service
...
public class LowCodeFormServiceImpl
    ...
    public LowCodeForm create(LowCodeForm form) {
        if (form == null) {
            throw new LowCodeException("表单配置不能为空");
        }
        if (!StringUtils.hasText(form.getCode())) {
            throw new LowCodeException("表单编码不能为空");
        }
        if (!StringUtils.hasText(form.getName())) {
            throw new LowCodeException("表单名称不能为空");
        }
        // 校验 code 唯一性
        long exists = this.count(new LambdaQueryWrapper<LowCodeForm>()
                .eq(LowCodeForm::getCode, form.getCode()));
        if (exists > 0) {
            throw new LowCodeException("表单编码已存在: " + form.getCode());
        }
        form.setId(null);
    ...
    public LowCodeForm update(LowCodeForm form) {
        if (form == null || form.getId() == null) {
            throw new LowCodeException("表单配置或ID不能为空");
        }
        LowCodeForm existing = baseMapper.selectById(form.getId());
        if (existing == null) {
            throw new LowCodeException("表单配置不存在");
        }
    ...
    public void publish(Long id) {
        ...
        if (!STATUS_DRAFT.equals(form.getStatus())) {
            throw new LowCodeException("当前状态不允许发布，当前状态: " + form.getStatus());
        }
        if (!StringUtils.hasText(form.getFormConfig())) {
            throw new LowCodeException("表单配置内容不能为空，无法发布");
        }
    ...
    public void archive(Long id) {
        ...
        if (!STATUS_PUBLISHED.equals(form.getStatus())) {
            throw new LowCodeException("当前状态不允许归档，当前状态: " + form.getStatus());
        }
    ...
    public byte[] exportConfig(String code) {
        ...
        LowCodeForm form = baseMapper.selectOne(new LambdaQueryWrapper<LowCodeForm>()
                ...
                .last("LIMIT 1"));
        if (form == null) {
            throw new LowCodeException("表单配置不存在: " + code);
        }
        try {
            ...
            throw new LowCodeException("导出表单配置失败: " + e.getMessage());
        }
    }
    ...
    public LowCodeForm importConfig(String json) {
        if (!StringUtils.hasText(json)) {
            throw new LowCodeException("导入的 JSON 内容不能为空");
        }
        LowCodeForm form;
        try {
            ...
            throw new LowCodeException("解析表单配置 JSON 失败: " + e.getMessage());
        }
        if (!StringUtils.hasText(form.getCode())) {
            throw new LowCodeException("导入的表单配置缺少编码 code");
        }
        // code 冲突时自动追加数字后缀
