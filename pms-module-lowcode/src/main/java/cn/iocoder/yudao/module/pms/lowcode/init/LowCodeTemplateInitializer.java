package cn.iocoder.yudao.module.pms.lowcode.init;
 ...
 * 低代码模板初始化器。
 *
 * <p>应用启动时分别检查低代码表（{@code low_form}、{@code low_list}、
 * {@code low_tab}、{@code low_related_page}）是否为空，
...
public class LowCodeTemplateInitializer implements ApplicationRunner {
    ...
    /**
     * 初始化表单模板：当 {@code low_form} 表为空时加载预置模板。
     */
    ...
    /**
     * 初始化列表模板：当 {@code low_list} 表为空时加载预置模板。
     */
    ...
    /**
     * 初始化标签页模板：当 {@code low_tab} 表为空时加载预置模板。
     */
    ...
    /**
     * 初始化关联页模板：当 {@code low_related_page} 表为空时加载预置模板。
     */
