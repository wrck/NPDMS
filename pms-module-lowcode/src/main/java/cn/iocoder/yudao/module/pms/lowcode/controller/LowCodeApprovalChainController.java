...
import cn.iocoder.yudao.module.pms.lowcode.service.LowCodeApprovalChainService;
...
public class LowCodeApprovalChainController {
    ...
    public CommonResult<List<LowCodeApprovalChain>> listByConfigType(@PathVariable String configType) {
        return CommonResult.success(approvalChainService.listByConfigType(configType));
    }

    @Operation(summary = "审批链列表（全部）")
    @GetMapping
    @PreAuthorize("@ss.hasPermission('lowcode:approval-chain:list')")
    public CommonResult<List<LowCodeApprovalChain>> list() {
        return CommonResult.success(approvalChainService.list());
    }

    @Operation(summary = "审批链详情")
    @GetMapping("/{id}")
    @PreAuthorize("@ss.hasPermission('lowcode:approval-chain:list')")
    public CommonResult<LowCodeApprovalChain> get(@PathVariable Long id) {
        return CommonResult.success(approvalChainService.getById(id));
    }

    @Operation(summary = "新建审批链")
    @PostMapping
    @PreAuthorize("@ss.hasPermission('lowcode:approval-chain:edit')")
    public CommonResult<LowCodeApprovalChain> create(@RequestBody LowCodeApprovalChain chain) {
    ...
    @PutMapping("/{id}")
    @PreAuthorize("@ss.hasPermission('lowcode:approval-chain:edit')")
    public CommonResult<Void> update(@PathVariable Long id, @RequestBody LowCodeApprovalChain chain) {
        chain.setId(id);
