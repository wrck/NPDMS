...
def render(
    ...
    coverage_document = {
        "schemaVersion": 1,
        "baseline": "PRD V1.8 / CHG-PRD-2026-09-01-011",
        "sources": {
    ...
    lines = [
        ...
        "> 源基线：`需求/PRD-项目实施交付管理平台.md` V1.8修订011候选；领域决策：`docs/design/phase-1-domain-ownership.md`；结构化同源投影：`docs/traceability/requirement-version-coverage.json`。",
        ...
        "> 候选修订：`CHG-PRD-2026-09-01-011`（收敛两条修订007后并行基线，保留COM/ACC/问卷与CUT双机97项语义，并统一BPM定义身份；待独立Baseline Gate）。",
        ...
        "- 当前状态：PRD V1.8修订011候选待独立Baseline Gate；既有Feature实施结论只关闭其机器可读声明覆盖的切片或子闭环，不因Feature完成自动关闭整个Requirement",
