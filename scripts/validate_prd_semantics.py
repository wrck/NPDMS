...
FAILURE_TRIGGER_PATTERN = re.compile(
    r"失败|异常|缺失|不足|不等于|不一致|无权|不可用|超时|重复|冲突|驳回|拒绝|未满足|未通过|不允许|不能"
)
...
OBSERVABLE_RESULT_PATTERN = re.compile(
    r"状态|记录|生成|保存|新增|不改变|保持|阻止|数量|版本|归档|通知|标记|计入|不计入|关联|返回|展示"
)
BOUNDARY_PATTERN = re.compile(
    r"失败|异常|缺失|不足|不等于|不一致|无权|不可|超时|重复|冲突|驳回|拒绝|未满足|未通过|不允许|不能|超出"
)
