...
class DdlModelDecisionCatalogTest(unittest.TestCase):
    def test_catalog_has_complete_concrete_scope(self) -> None:
        ...
        self.assertEqual(61, rendered.count("|T-"))
        self.assertEqual(61, rendered.count("|O-"))
        self.assertEqual(61, rendered.count("|PK-"))
        self.assertEqual(48, rendered.count("|FK-"))
        self.assertEqual(124, rendered.count("|IX-"))
        self.assertEqual(127, rendered.count("|UK-"))
        self.assertEqual(92, rendered.count("|CK-"))
        self.assertIn("1,250", rendered)
        self.assertIn("来源幂等键（15项）", rendered)
        self.assertIn("租户行引用键（61项）", rendered)
