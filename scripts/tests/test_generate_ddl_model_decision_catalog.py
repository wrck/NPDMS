...
class DdlModelDecisionCatalogTest(unittest.TestCase):
    def test_catalog_has_complete_concrete_scope(self) -> None:
        ...
        self.assertEqual(66, rendered.count("|T-"))
        self.assertEqual(66, rendered.count("|O-"))
        self.assertEqual(66, rendered.count("|PK-"))
        self.assertEqual(52, rendered.count("|FK-"))
        self.assertEqual(130, rendered.count("|IX-"))
        self.assertEqual(138, rendered.count("|UK-"))
        self.assertEqual(103, rendered.count("|CK-"))
        self.assertIn("1,382", rendered)
        self.assertIn("来源幂等键（15项）", rendered)
        self.assertIn("租户行引用键（66项）", rendered)
        self.assertIn("业务身份键（27项）", rendered)
        ...
        self.assertIn("待独立整体一致性复审", rendered)
        self.assertNotIn("独立整体一致性复审已GO", rendered)
