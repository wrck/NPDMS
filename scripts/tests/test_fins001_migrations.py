...
V148 = MIGRATIONS / "V148__fins001_inspection_rule_revision.sql"
V149 = MIGRATIONS / "V149__fins001_inspection_rule_seed_and_menu.sql"
V150 = MIGRATIONS / "V150__fins001_legacy_rule_forward_migration.sql"
...
class Fins001MigrationContractTest(unittest.TestCase):
    ...
    def test_v148_is_unique_current_migration(self):
        versions = {}
        ...
        self.assertEqual(V148.name, versions[148][0])

    def test_v148_creates_only_five_inspection_rule_tables(self):
        sql = self._sql()
        for table in (
            "srv_inspection_rule",
    ...
    def test_v148_contains_tenant_audit_and_optimistic_lock_fields(self):
    ...
    def test_v148_places_permanent_name_identity_on_rule(self):
        sql = self._sql()
    ...
    def test_v148_contains_frozen_unique_and_foreign_key_contracts(self):
        sql = self._sql()
        for token in (
    ...
    def test_v148_allows_incomplete_draft_revision_fields_and_checks_published_main_fields(self):
        sql = self._sql()
        revision = self._table(sql, "srv_inspection_rule_revision")
        self.assertIn("`rule_name_snapshot` varchar(128) not null", revision)
        for column in (
    ...
    def test_v148_allows_incomplete_draft_child_rows(self):
        sql = self._sql()
    ...
    def test_v148_contains_state_fact_and_security_review_checks(self):
        sql = self._sql()
        for token in (
    ...
    def test_v148_has_no_destructive_or_cross_owner_writes(self):
        sql = self._sql()
        for token in (
    ...
    def test_v149_contains_two_formal_dictionaries(self):
        sql = self._v149()
    ...
    def test_v149_contains_independent_menu_and_six_permissions(self):
        sql = self._v149()
        for permission in (
    ...
    def test_v149_does_not_seed_rules_or_product_types(self):
        sql = self._v149()
        for token in (
    ...
    def test_v149_fails_closed_on_seed_identity_conflicts(self):
        sql = self._v149()
        self.assertIn(
    ...
    def test_v150_preserves_legacy_and_performs_zero_target_writes(self):
        sql = self._v150()
    ...
    def test_v150_is_assertion_only_and_leaves_no_procedure(self):
        sql = self._v150()
    ...
    @staticmethod
    def _sql():
        return V148.read_text(encoding="utf-8-sig").lower()

    @staticmethod
    def _v149():
        return V149.read_text(encoding="utf-8-sig").lower()

    @staticmethod
    def _v150():
        return V150.read_text(encoding="utf-8-sig").lower()
