...
OVERRIDES: dict[str, list[dict[str, str]]] = {
    ...
    "ArrivalAcceptance": [source("CURRENT_TABLE", "pms_eng_arrival", "CURRENT_FORWARD", "map only provable batch identity, project, occurred time, actor reference and source notes; equipment must resolve to AST stable device and URL must become valid FileReference; legacy status/quantity/shipment/seed never directly produces ACCEPTED; incomplete rows remain legacy plus PENDING_RECONCILIATION", "FEATURE_MAPPING_DEFINED", "F-IMP-002_FEATURE_READY_REVIEW")],
    ...
    "CutoverPlan": [source("CURRENT_TABLE", "pms_cut_plan", "CURRENT_FORWARD", "through PLT migration evidence, resolve the qualified same-tenant legacy task mapping and create one read-only LEGACY_FORWARD revision; map pre_check/procedure/verification/rollback to PRE_OPERATION/OPERATION/POST_BUSINESS_TEST/ROLLBACK step 1, retain raw status/version/mapping identity, and never infer edit mode, assessment, checklist, device, configuration, template, file, risk mitigation, support arrangement, task stage or CUT-05 approval facts; keep unqualified rows with stable issues/retained classification", "FEATURE_MAPPING_DEFINED", "F-CUT-004_FEATURE_READY_REVIEW")],
    ...
    "CutoverSpareApplicationReference": [source("NONE_NEW", "CutoverSpareApplicationReference", "NEW_ONLY", "create CUT-08 integration intent and external reference only from authorized new-platform commands or trusted INT-06 first-reference binding; never infer an external application from legacy CUT/material text, URL or status", "NEW_ONLY", "F-CUT-010_FEATURE_RELEASE")],
    "CutoverSpareStatusRevision": [source("NONE_NEW", "CutoverSpareStatusRevision", "NEW_ONLY", "append raw external status revisions only from a matched INT-06 initiate response, callback or explicit refresh; never derive inventory, approval, arrival, issue or completion facts", "NEW_ONLY", "F-CUT-010_FEATURE_RELEASE")],
    "CutoverSpareManualEvidence": [source("NONE_NEW", "CutoverSpareManualEvidence", "NEW_ONLY", "append only after PLT locks an immutable file fact in CUTOVER_TASK/SPARE_MANUAL_EVIDENCE scope; never copy file bytes or treat manual evidence as external success", "NEW_ONLY", "F-CUT-010_FEATURE_RELEASE")],
    ...
    "CutoverSupportArrangement": [source("NONE_NEW", "CutoverSupportArrangement", "NEW_ONLY", "create support role, person, duty, phone and arrival facts only from new-platform F-CUT-004 commands; pms_cut_plan has no proven source fields and must not be used to infer arrangements", "NEW_ONLY", "FEATURE_RELEASE")],
    "CutoverClosure": [source("CURRENT_TABLE", "pms_cut_execution", "CURRENT_FORWARD", "after a controlled Release importer freezes legacy rows through PlatformMigrationEvidenceApi batch CUT/CUTOVER_CLOSURE_CURRENT_FORWARD/NPDMS_LEGACY/pms_cut_execution with decimal legacy id source keys and FCUT006_LEGACY_V1 rules, require one source unit to prove a complete closure identity, three result flags, rollback union, final result and immutable PLT file facts; CUT production never reads the legacy table or datasource; the legacy step-row schema proves no closure, so classify structurally valid rows as RETAINED with no target, append FCUT006_SOURCE_RECORD_INVALID only for corrupted frozen source identity or payload, and insert no closure; claim/classify/complete counts share the CUT caller transaction and temporary provider failure rolls the batch back; exclude pms_cut_observation and task status/text/time inference", "FEATURE_MAPPING_DEFINED", "F-CUT-006_FEATURE_READY_REVIEW")],
