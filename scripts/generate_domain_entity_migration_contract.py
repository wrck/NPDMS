...
OVERRIDES: dict[str, list[dict[str, str]]] = {
    ...
    "CutoverClosure": [source("CURRENT_TABLE", "pms_cut_execution", "CURRENT_FORWARD", "F-CUT-006 qualification requires one source unit to prove a complete closure identity, three result flags, rollback union, final result and immutable PLT file facts; the legacy step-row schema proves none, so retain every current row with an explicit migration issue and insert no closure; exclude pms_cut_observation and task status/text/time inference", "FEATURE_MAPPING_DEFINED", "F-CUT-006_FEATURE_READY_REVIEW")],
