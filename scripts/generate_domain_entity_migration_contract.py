...
OVERRIDES: dict[str, list[dict[str, str]]] = {
    ...
    "CutoverPlan": [source("CURRENT_TABLE", "pms_cut_plan", "CURRENT_FORWARD", "map only qualified legacy task/code/name and provable pre-check/operation/verification/rollback/level/remark/audit content into read-only LEGACY_FORWARD revisions; never infer edit mode, assessment, checklist, device, configuration, file, risk mitigation, support arrangement or CUT-05 approval facts; keep unqualified rows with migration issues", "FEATURE_MAPPING_DEFINED", "F-CUT-004_FEATURE_READY_REVIEW")],
