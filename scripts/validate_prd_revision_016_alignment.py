...
def inspect_prd(text: str) -> dict[str, bool]:
    ...
    return {
      "R01": any(prefix + "配置`S0→S4`" in r("PM-03") for prefix in ("售前测试模板", "售前预置模板")) and "S0→S4→S6" not in r("PM-03") and "售前测试模板未配置EXE-02" in r("EXE-03"),
      ...
      "R04": "不通过、需整改" in r("ACC-03") and any(marker in r("ACC-03") for marker in ("验收通过事实为否", "不能满足通过谓词")),
      ...
      "R12": any(marker in r("ACC-03") for marker in ("新总体验收版本", "明确引用A/B证据的新版本")) and "旧范围报告和结论" in r("PM-06") and any(marker in r("ACC-03") for marker in ("同一项目、适用报告类型仍只有一个当前报告版本", "不新建第二个当前报告")),
      "R13": "CUT-03的P3采集项入口" in r("INT-12") and "清单版本、采集项和设备" in r("INT-12"),
      "R14": "经审批更长" not in r("NFR-02") and re.search(r"30秒为(?:当前V2)?硬上限", r("NFR-02")) is not None,
