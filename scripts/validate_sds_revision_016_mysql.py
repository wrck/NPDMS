...
def main() -> int:
    ...
    def insert(table: str, **values: object) -> str:
        ...
        for name,definition in model['tables'][table]['columns']:
            ...
            elif name in {'creator','updater'}:value='schema_test'
            elif name in {'version','deleted'}:value=0
            elif name in {'create_time','update_time'}:value='2026-09-07 00:00:00.000'
    ...
    def case(name: str, statement: str, expected_error: int|None=None):
        ...
        cases.append({'case':name,'expectedError':expected_error,'actualError':error,'passed':passed})
    ...
    report_json={
        ...
        'executionEnvironment':'GITHUB_ACTIONS' if os.environ.get('GITHUB_RUN_ID') else 'LOCAL_WORKTREE',
        'sourceTreeDirty':bool(subprocess.check_output(['git','status','--porcelain','--',str(CONTRACT),str(DDL),'docs/baseline/prd-v1.8.md'],cwd=args.root,text=True).strip()),
