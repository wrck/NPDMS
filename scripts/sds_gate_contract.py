...
MYSQL_EVIDENCE = 'docs/engineering/gates/phase-2/revision-016-mysql-schema.json'
...
def validate_design(root: Path) -> list[str]:
    ...
    for table,shape in tables.items():
        ...
        if ['tenant_id','BIGINT UNSIGNED NOT NULL'] not in columns:
...
def validate_mysql_evidence(root: Path) -> list[str]:
    ...
    if evidence.get('prdGitBlob')!=blob(root/PRD) or not evidence.get('sourceCommit') or not evidence.get('actionsRunId'):
