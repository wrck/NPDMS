...
def render_decision_analysis(
    ...
    lines.extend([
        ...
        f"|L2 数据架构不变量|主键、租户引用、同域外键、稳定技术CHECK|{'MODEL_BASELINE_READY' if model_baseline_ready else ('REQUIREMENT_OWNER_ACCEPTED / REVIEW_PENDING' if requirement_owner_accepted else 'RECONFIRMATION_REQUIRED')}|{q07_status}|",
        f"|L3 业务唯一性与状态守卫|业务身份、来源幂等、当前唯一、关系粒度、状态耦合CHECK|需求Owner+数据架构Owner|{'九组已接受；独立整体一致性复审已GO' if model_baseline_ready else ('九组已接受；待独立整体一致性复审' if requirement_owner_accepted else '逐组批准；有空洞的先修模')}|",
        f"|L4 性能候选|{len(constraints['INDEX'])}个普通索引|{'MODEL_BASELINE_READY' if model_baseline_ready else ('REQUIREMENT_OWNER_ACCEPTED / REVIEW_PENDING' if requirement_owner_accepted else 'RECONFIRMATION_REQUIRED')}；Feature Owner+性能Owner后续验证|{q08_status}；仍须由P3-E06压测定稿|",
...
def render(root: Path) -> str:
    ...
    lines.extend([
        ...
        "- 当前哈希九组决策已由需求方按ADR-0028接受，1,883项均已有逐项决策；独立复审已给出`GO`，仅复核制品、哈希和整体一致性，不重新逐项签署。`approvedDdlSha256`保持显式`null`，仅由未来历史迁移门禁另行管理。" if model_baseline_ready else ("- 当前哈希九组决策已由需求方按ADR-0028接受，1,883项均已有逐项决策；独立复审只复核候选制品、哈希和整体一致性，不重新逐项签署。`approvedDdlSha256`保持显式`null`，仅由未来历史迁移门禁另行管理。" if requirement_owner_accepted else "- 旧哈希下Q07/Q08决策已失效；当前哈希状态为RECONFIRMATION_REQUIRED，不得据此生成批准哈希。"),
