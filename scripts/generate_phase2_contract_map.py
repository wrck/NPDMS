...
GROUPS: list[tuple[tuple[str, ...], Contract]] = [
    ...
    (("COM-01",), contract("Contract、SalesOrder、OrderLine、DeliveryScope、DeliveryScopeDetail、AcceptanceScopeBinding、ProjectStageSnapshot", "com_contract、com_sales_order、com_sales_order_line、com_delivery_scope、com_delivery_scope_detail、acc_acceptance_scope_binding、proj_project_stage_snapshot", "/contracts、/sales-orders、/order-lines、/delivery-scopes；内部ProjectOfficeFactApi、ProjectAcceptanceStageFactApi、AcceptanceScopeBindingApi、AcceptanceScopeGuardApi、DeliveryScopeAcceptanceLockApi", events="DeliveryScopeAssigned/Released；ProjectStageChanged仅作提交后通知，不触发绑定", integration="ERP（合同订单权威）；CRM仅提供项目/客户上下文", workflow="ERP来源版本与单位精度守卫、项目办事处发生时快照、范围追加版本、V70必填快照与明细序号确定性转换、PROJ→COM→ACC统一锁序、阶段进入/新范围生效与绑定原子提交、报告与绑定解耦、超分配及验收减量守卫", authorization="ContractProjectScope（Q-FCOM-001关闭前BLOCKED_BY_SPEC）；ERP核心字段只读")),
