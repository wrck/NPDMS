...
GROUPS: list[tuple[tuple[str, ...], Contract]] = [
    ...
    (("PM-03",), contract("ProjectTemplate、ProjectStageSnapshot", "proj_project_template_revision、proj_project_stage_snapshot、BLOCKED_BY_DESIGN(ADR-0029任务绑定与完成规则物理承载待定)", "/project-templates", workflow="模板内StageDefinition/TaskDefinition发布、实例冻结、必填WorkBinding（默认TASK_NATIVE）/PermissionPolicy/CompletionRule/GateRef校验和阶段门禁", authorization="项目模板维护权限；项目阶段范围；非TASK_NATIVE绑定目标权限不得越权")),
    ...
    (("PM-11",), contract("ProjectTask、TaskAncestorProjection、TaskDependency", "proj_project_task、proj_task_tree_path、proj_task_dependency、BLOCKED_BY_DESIGN(ADR-0029任务绑定与完成规则物理承载待定)", "/projects/{id}/workspace、/projects/{id}/tasks、/project-tasks/{id}/workbench、/project-tasks/{id}/actions/move、/project-tasks/{id}/actions/{submit|start|complete|cancel}", events="TaskAssigned、TaskCompleted", workflow="ProjectTask内必填WorkBinding/CompletionRule与Stage→ProjectTask工作台投影、任务任意层级移动；TASK_NATIVE按任务自身事实执行，其他类型按绑定事实完成并受依赖守卫", authorization="ProjectTreeScope；TASK_NATIVE任务范围；其他类型由服务端合并目标业务对象权限")),
    ...
    (("CUT-03",), contract("CutoverTask、CollectionTask", "cut_task、plt_collection_task、BLOCKED_BY_DESIGN(ADR-0029割接清单结果承载待定)", "/cutover-tasks/{id}/checklist、/cutover-tasks/{id}/checklist/actions/rematch、/cutover-tasks/{id}/checklist/items/{itemId}/actions/request-collection", events="CollectionTaskRequested、CollectionResultAvailable、CutoverChecklistItemResultLinked", integration="现有采集平台子应用", files="FileArtifact", workflow="CutoverTask内P3清单在同一工作台动态匹配、人工填写/上传、采集任务下发、回调结果回填和配置缺口留痕", authorization="CutoverTaskScope、BusinessObjectDeviceCredentialScope；服务端按清单项和设备范围裁剪")),
