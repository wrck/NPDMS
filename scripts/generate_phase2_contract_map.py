...
GROUPS: list[tuple[tuple[str, ...], Contract]] = [
    ...
    (('PM-03',), contract('ProjectTemplateVersion、StageTransitionDefinition、Stage/TaskWorkBinding、ProjectStageSnapshot', '既有proj_project_template_revision、proj_project_template_task_definition、proj_project_stage_snapshot；图与Stage绑定的复用或新增物理差量见09分册及受影响Feature，未重验证不得声称NO_PHYSICAL_DELTA', "/project-templates", workflow='冻结图发布校验；当前CompletionRule/准出→唯一后置→目标准入→原子推进；售前仅S0/S4，无虚构阶段', authorization="项目模板维护权限；项目阶段范围；非TASK_NATIVE绑定目标权限不得越权")),
    ...
    (('PM-06',), contract('ContractScopeAppendRequest、COM ProjectScopeVersion引用、范围差异与逐阶段影响', 'PROJ追加申请及COM唯一范围水位/分配；物理复用/新增/兼容处置见09分册，旧多期群组不作为当前输入', '同一projectId范围追加应用命令；COM范围写入、PROJ任务/绑定及ACC范围绑定公开契约', events='范围版本和受影响业务事实事件；不产生ProjectPhaseGroupChanged', workflow='ACTIVE项目审批后同事务追加范围/任务/交付件；重算旧事实覆盖，闭环项目拒绝', authorization='项目、合同/订单范围与审批权限；不授予其他项目权限')),
    ...
    (('PM-11',), contract('ProjectStage/ProjectTask、Stage/TaskWorkBinding、CompletionRule、任务树及依赖', "proj_project_task、proj_project_task_execution_contract、proj_project_task_completion_evaluation、proj_task_tree_path、proj_task_dependency", "/projects/{id}/workspace、/projects/{id}/gantt、/projects/{id}/tasks、/project-tasks/{id}/workbench、/project-tasks/{id}/dependencies、/project-tasks/{id}/actions/move、/project-tasks/{id}/actions/{submit|start|complete|cancel}", events="TaskAssigned、TaskCompleted", workflow='V1阶段/任务工作台及冻结绑定运行；V2只增加甘特展示和受控依赖新增、更新、删除，不建立第二套任务事实；非原生完成须重验Owner事实，图只由PROJ推进', authorization="ProjectTreeScope；TASK_NATIVE任务范围；其他类型由服务端合并目标业务对象权限")),
    ...
    (('ACC-03',), contract('AcceptanceReportRevision、验收结论及精确范围/来源证据引用', "acc_acceptance、acc_acceptance_item、acc_confirmation", "/acceptances", files="FileArtifact", workflow='字段/附件完整只形成证据；明确通过且顺序合法、当前范围完整覆盖才满足门禁；同类型一个当前报告，A+B须新总体明确通过', authorization="ProjectStageScope、FileBusinessScope")),
