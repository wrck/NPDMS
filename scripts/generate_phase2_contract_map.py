...
GROUPS: list[tuple[tuple[str, ...], Contract]] = [
    ...
    (("PRE-02", "PRE-04"), contract("Preparation、PreparationDynamicFormInstance", "sol_preparation、sol_dynamic_form_instance", "/preparations、/{id}/actions/{submit|confirm|return}", files="FileArtifact", workflow="工勘/需求填写、提交、确认和退回；PRE-04在共享基础完成后前向重规划", authorization="ProjectStageScope；字段与文件权限")),
    ...
    (("SOL-01",), contract("DynamicFormTemplate、DynamicFormTemplateRevision、DynamicFormInstance", "plt_dynamic_form_template、plt_dynamic_form_template_revision、plt_dynamic_form_instance", "/dynamic-form-templates、/dynamic-form-template-revisions、/dynamic-form-instances", files="FileArtifact", workflow="模板唯一草稿、发布修订不可变、启停独立、手工实例冻结修订并按CAS保存；消费方业务状态另行拥有", authorization="PLT模板/实例功能权限；发布者为高信任配置主体；目标API继续独立鉴权")),
