...
GROUPS: list[tuple[tuple[str, ...], Contract]] = [
    ...
    (("ACC-03",), contract("Acceptance、AcceptanceScopeBinding、ProjectStageSnapshot", "acc_acceptance、acc_acceptance_item、acc_confirmation、acc_acceptance_scope_binding、proj_project_stage_snapshot", "/acceptances；内部AcceptanceScopeBindingApi、AcceptanceScopeGuardApi、DeliveryScopeAcceptanceLockApi", files="FileArtifact", workflow="项目阶段进入先同步绑定全部当前范围且不创建报告；验收阶段内新范围生效同步绑定；初验/终验活动完成才要求对应当前报告四项完备；报告状态不触发或反推绑定，Q-FCOM-002关闭前不自动关闭/解锁", authorization="ProjectStageScope、FileBusinessScope")),
