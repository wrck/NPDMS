...
GROUPS: list[tuple[tuple[str, ...], Contract]] = [
    ...
    (('PM-03',), contract('ProjectTemplate、ProjectTemplateVersion、DeliveryConfigurationRevision、StageTransitionDefinition、ProjectStageTransition、StageWorkBinding、ProjectStageSnapshot', 'proj_project_template_revision、proj_project_template_task_definition、proj_delivery_definition_revision、proj_delivery_definition_reference、proj_stage_transition_definition、proj_project_stage_transition、proj_project_stage_execution_contract、proj_project_stage_snapshot', "/project-templates", workflow='冻结图发布校验；当前CompletionRule/准出→唯一后置→目标准入→原子推进；售前仅S0/S4，无虚构阶段', authorization="项目模板维护权限；项目阶段范围；非TASK_NATIVE绑定目标权限不得越权")),
    ...
    (('PM-06',), contract('ContractScopeAppendRequest、ProjectScopeVersion', 'proj_contract_scope_append_request、com_delivery_scope_project_version、com_project_scope_revision', '/projects/{id}/scope-append-requests、/scope-append-requests/{id}/actions/{submit|apply}；DeliveryScopeApi、AcceptanceScopeBindingApi', events='ProjectScopeAppendApplied（COM单一Producer；批准范围、差量和Owner版本，不改变旧业务事实）', workflow='ACTIVE项目审批后同事务追加范围/任务/交付件；重算旧事实覆盖，闭环项目拒绝', authorization='项目、合同/订单范围与审批权限；不授予其他项目权限')),
    ...
    (('PM-10',), contract("Project、ProjectStageSnapshot", "proj_project、proj_project_stage_snapshot", "/projects/{id}/actions/rollback、/projects/{id}/actions/close", events="ProjectStageChanged、ProjectClosed(lifecycleStatus=EXCEPTION_CLOSED)", workflow='责任回退保持ACTIVE；仅EXCEPTION_CLOSED可重开；CLO-02按类型唯一产生NORMAL_CLOSED/NO_TRACKING_CLOSED，保留真实阶段', authorization="ProjectTreeScope；状态命令权限与门禁")),
    (('PM-11',), contract('ProjectStage、ProjectTask、StageWorkBinding、TaskWorkBinding、TaskCompletionRule、TaskCompletionEvaluation、TaskAncestorProjection、TaskDependency', "proj_project_stage、proj_project_stage_execution_contract、proj_project_task、proj_project_task_execution_contract、proj_project_task_completion_evaluation、proj_task_tree_path、proj_task_dependency", "/projects/{id}/workspace、/projects/{id}/gantt、/projects/{id}/tasks、/project-tasks/{id}/workbench、/project-tasks/{id}/dependencies、/project-tasks/{id}/actions/move、/project-tasks/{id}/actions/{submit|start|complete|cancel}", events="TaskAssigned、TaskCompleted", workflow='V1阶段/任务工作台及冻结绑定运行；V2只增加甘特展示和受控依赖新增、更新、删除，不建立第二套任务事实；非原生完成须重验Owner事实，图只由PROJ推进', authorization="ProjectTreeScope；TASK_NATIVE任务范围；其他类型由服务端合并目标业务对象权限")),
    ...
    (('PLN-01',), contract("ConstructionPlan", "sol_construction_plan、sol_construction_plan_revision", "/schedules、/{id}/actions/{calculate|apply}", workflow='冻结图唯一计划路径及实际参与阶段占比100%；无S5/S6不生成日期，路径多义不生效；计算草稿经PLN-04批准', authorization="ProjectStageScope；计划只读/维护分离")),
    (('PLN-02',), contract("ConstructionPlan", "sol_construction_plan、sol_construction_plan_revision", "/schedules、/{id}/actions/{calculate|apply}", workflow="计划计算候选、预警和显式应用", authorization="ProjectStageScope；计划只读/维护分离")),
    (('PLN-03',), contract("ConstructionPlan", "sol_construction_plan、sol_construction_plan_revision", "/schedules、/{id}/actions/{calculate|apply}", workflow="计划计算候选、预警和显式应用", authorization="ProjectStageScope；计划只读/维护分离")),
    (('PLN-04',), contract("ConstructionPlan", "sol_construction_plan、sol_construction_plan_revision", "/construction-plans、/{id}/actions/{submit|approve|reject}", files="FileArtifact", workflow='首次批准提供Owner事实，由PROJ重验图推进；后期换版不隐式退回/推进，钉钉仅通知入口', authorization="ProjectStageScope；审批人按PRD")),
    (('SCH-01',), contract("Solution", "sol_solution、sol_solution_revision、sol_solution_review", "/solutions、/{id}/revisions、/{id}/actions/{submit|approve|reject|publish}", files="FileArtifact", workflow="方案导入/编审、重大复审和发布版本", authorization="ProjectStageScope；方案审批与文件权限")),
    (('SCH-02',), contract("Solution", "sol_solution、sol_solution_revision、sol_solution_review", "/solutions、/{id}/revisions、/{id}/actions/{submit|approve|reject|publish}", files="FileArtifact", workflow="方案导入/编审、重大复审和发布版本", authorization="ProjectStageScope；方案审批与文件权限")),
    (('SCH-04',), contract("Solution", "sol_solution、sol_solution_revision、sol_solution_review", "/solutions、/{id}/revisions、/{id}/actions/{submit|approve|reject|publish}", files="FileArtifact", workflow="方案导入/编审、重大复审和发布版本", authorization="ProjectStageScope；方案审批与文件权限")),
    (('SCH-05',), contract("Solution", "sol_solution、sol_solution_revision、sol_solution_review", "/solutions、/{id}/revisions、/{id}/actions/{submit|approve|reject|publish}", files="FileArtifact", workflow='实际BPM全部强制节点通过形成批准版本；只提供Owner事实，PROJ按冻结图校验推进', authorization="ProjectStageScope；方案审批与文件权限")),
    ...
    (('EXE-06',), contract("ImplementationReadinessSnapshot", "proj_project_stage_snapshot", "/implementation-readiness/{projectId}", events="ImplementationReadinessSnapshotPublished", workflow='当前需割接范围被有效成功结果并集及合法不适用依据完整覆盖后形成完成事实；PROJ按图推进，失效重算而不篡改历史', authorization="ImplementationProjectCutoverScope")),
    ...
    (("ACC-03",), contract("AcceptanceReportRevision、Acceptance、AcceptanceScopeBinding、ProjectStageSnapshot、DeliveryArtifact", "acc_acceptance_report、acc_acceptance_report_revision、acc_acceptance、acc_acceptance_report_version、acc_acceptance_report_attachment、acc_project_deliverable、acc_project_deliverable_source_version、acc_project_deliverable_source_attachment、acc_acceptance_scope_binding、proj_project_stage_snapshot", "/acceptances、/acceptances/{id}/report-versions；内部AcceptanceActivityCompletionFactApi、AcceptanceScopeBindingApi、AcceptanceScopeGuardApi、DeliveryScopeAcceptanceLockApi", events="AcceptanceReportVersionChanged(EFFECTIVE/REPLACED/REVOKED)、ClosureGateRecheckRequested；报告事件不触发或反推AcceptanceScopeBinding", files="FileArtifact/FileVersion完整有序集合", workflow="字段/附件完整只形成证据；明确通过且顺序合法、当前范围完整覆盖才满足门禁；同类型一个当前报告，A+B须新总体明确通过；项目阶段进入/验收阶段内新范围生效继续直接复用F-COM-001范围绑定原子路径；PROJ拥有ProjectTask/WorkBinding并以MANDATORY调用ACC完成活动；报告DRAFT与旧EFFECTIVE可并存，发布替换原子关闭旧版，撤销关闭当前且不恢复旧版；终验受当前有效初验守卫；精确D-INITIAL-REPORT/D-FINAL-REPORT更新既有应交根及只追加来源版本/附件集合，归档失败保留报告并待补偿；Q-FCOM-002边界不变", authorization="pms:acceptance:report:query/write/complete/download；ProjectTreeScope、ProjectTaskScope、FileBusinessScope、TenantScope")),
    (("ACC-02",), contract("SatisfactionCollection", "acc_satisfaction_questionnaire_template、acc_satisfaction_questionnaire_template_revision、acc_satisfaction_collection_task、acc_satisfaction_questionnaire、acc_satisfaction_access_grant、acc_satisfaction_response、acc_satisfaction_response_file、acc_satisfaction_result、acc_satisfaction_result_file、acc_satisfaction_remediation_fact、acc_project_deliverable_source_version、acc_project_deliverable_source_attachment", "/satisfaction-questionnaire-templates、/satisfaction-questionnaire-templates/{id}/revisions及publish、/satisfaction-tasks、/satisfaction-tasks/{id}/actions/{assign|recollect}、/satisfaction-tasks/{id}/access-grants、/satisfaction-questionnaires/{token}、/satisfaction-questionnaires/{token}/files、/satisfaction-questionnaires/{token}/responses、/satisfaction-tasks/{id}/assisted-responses、/satisfaction-results、/satisfaction-results/{id}/actions/invalidate；内部SatisfactionQuestionnaireTemplateApi、SatisfactionTaskInitializationApi、SatisfactionResultFactApi", events="SatisfactionTaskCreated、SatisfactionResultVersionChanged、ClosureGateRecheckRequested", integration="V1受控链接/二维码/现场协助；V2仅增加自动触达，INT-10短信/邮件、INT-05钉钉仅保留接口边界", files="PLT公共文件事实；SATISFACTION_SIGNATURE/ATTACHMENT/RESULT_DOCUMENT/ARCHIVE", workflow="发布schemaVersion=1受控题型与SUM_V1/WEIGHTED_AVERAGE_V1计分配置→PROJ冻结模板Fact和业务时点→MANDATORY初始化revision1并冻结完整配置和原始source→客户仅提交答案→服务端按冻结配置最终一次舍入并判定→当前有效达标Result可按版本正式失效且不改写历史→未达标/失效追加RemediationFact并触发下一revision→仅有效达标Result精确进入D-SAT-REPORT/T-SAT-SURVEY来源根→归档失败补偿；Todo或送达不替代提交/通过", authorization="pms:acceptance:satisfaction:query/manage/collect/export/download；ProjectScope/责任人/字段/FileBusinessScope/TenantScope；客户grant仅限唯一问卷")),
    (("ACC-04",), contract("DeliveryArtifact", "acc_project_deliverable、acc_project_deliverable_source_version、acc_project_deliverable_source_attachment、acc_artifact_review、acc_archive_record", "/delivery-artifacts", events="AcceptanceReportVersionChanged、SatisfactionResultVersionChanged、ArtifactAccepted/Archived、ClosureGateRecheckRequested", files="FileArtifact/FileVersion完整有序集合", workflow="索引有效来源/版本/哈希/结论与范围；失败验收证据可归档但不满足通过门禁；下载权限不改变客观齐套；以acc_project_deliverable为唯一应交根；来源版本/附件关系只追加，替换/失效保留旧关系且不恢复旧版；F-ACC-001只实现初验/终验来源切片，F-ACC-002仅允许同租户同项目精确D-SAT-REPORT/T-SAT-SURVEY根接收有效达标SatisfactionResult及归档补偿，缺失/重复/错配失败关闭；其余来源与统一批量下载留待完整Feature", authorization="ProjectStageScope、FileBusinessScope；ACC归档")),
    (('CLO-01',), contract("ProjectClosure、ClosureGateSnapshot、SatisfactionCollection", "acc_project_closure、acc_closure_gate_snapshot、acc_closure_review、acc_satisfaction_result", "/closure-gates/{projectId}、/project-closures", events="ProjectClosureCompleted", files="FileArtifact", workflow='NORMAL/NO_TRACKING先分型，冻结真实阶段/模板/范围/来源；NO_TRACKING不要求正常准出；通过快照变化即失效', authorization="ProjectStageScope；全部后代项目门禁与审批范围")),
    (('CLO-02',), contract("ProjectClosure、ClosureGateSnapshot、SatisfactionCollection", "acc_project_closure、acc_closure_gate_snapshot、acc_closure_review、acc_satisfaction_result", "/closure-gates/{projectId}、/project-closures", events="ProjectClosureCompleted", files="FileArtifact", workflow='实际BPM全部通过且最新快照持续有效后，CLO-02原子形成NORMAL_CLOSED或NO_TRACKING_CLOSED；保留current_stage与closed_from_stage，不造S6', authorization="ProjectStageScope；全部后代项目门禁与审批范围")),
    ...
    (("CUT-03",), contract("CutoverTask、CutoverChecklist、CollectionTask", "cut_task、cut_cutover_checklist、cut_cutover_checklist_item、cut_cutover_checklist_item_result、cut_cutover_configuration_revision、plt_collection_task", "/cutover-tasks/{id}/checklist、/cutover-tasks/{id}/checklist/actions/{rematch|export}、/cutover-tasks/{id}/checklist/items/{itemId}/actions/request-collection、/cutover-config/navigation-rules", events="CollectionTaskRequested、CollectionResultAvailable、CutoverChecklistItemResultLinked", integration="现有采集平台子应用", files="FileArtifact", workflow="V1 P3暂存不推进；有效提交才P4；设备采集绑定清单版本/采集项/设备，技术成功不替代CUT结果；V2增加授权清单导出及受控流程跳转配置优化，不绕过必填、版本和权限门禁；V1在P3同一工作台完成动态匹配、填写、暂存、提交、采集回填和基础跳转；V2增加授权清单导出及受控流程跳转配置优化；不复制DAC技术状态", authorization="CutoverTaskScope、BusinessObjectDeviceCredentialScope；导出和跳转均按清单项、流程状态和设备范围服务端裁剪")),
    (("CUT-04",), contract("CutoverPlan、CutoverSupportArrangement", "cut_plan_revision、cut_step、cut_cutover_support_arrangement", "/cutover-tasks/{id}/plan、/cutover-tasks/{id}/plan/actions/{create-draft|download-draft|submit|revise}", events="N/A（P4同步调用CutoverApprovalFactApi创建/暂停/建立替代CUT-05审批链；CutoverApproved仅由CUT-05发布）", files="FileArtifact", workflow="P4方案编制、不可变revision和与P5审批实例原子交接；来源失效原子暂停审批并将任务P5返回P4，恢复办理创建替代revision/审批；审批状态由CUT-05拥有", authorization="CutoverTaskScope；方案查询/保存/下载/提交权限")),
    ...
    (("CUT-08",), contract("CutoverTask、CutoverSpareApplicationReference、CutoverSpareStatusRevision、CutoverSpareManualEvidence", "cut_task、cut_spare_application_reference、cut_spare_status_revision、cut_spare_manual_evidence", "/api/v1/pms/cutover-tasks/{id}/spare-support、内部CutoverSpareCallbackApi", integration="备件系统（INT-06拥有连接器和第三方事实）", files="FileArtifact", workflow="备件需求识别、外部申请发起/首次引用绑定、只读状态版本、人工证据和对账；不建立本地备件生命周期或P6门禁", authorization="CutoverTaskScope；任务负责人、项目/设备范围及外部回调受信租户")),
    ...
    (('INS-01',), contract("InspectionTask、CollectionTask", "srv_inspection_task、srv_inspection_task_rule_snapshot、plt_collection_task", "/inspection-tasks、/collection-tasks", events="InspectionDispatched、InspectionCompleted、CollectionResultConsumed", integration="现有采集平台子应用", files="FileArtifact", workflow="方式选择、预检/执行、业务消费和归档门禁", authorization="AssignedProjectDeviceScope、BusinessObjectDeviceCredentialScope")),
    (('INS-02',), contract("InspectionTask、CollectionTask", "srv_inspection_task、srv_inspection_task_rule_snapshot、plt_collection_task", "/inspection-tasks、/collection-tasks", events="InspectionDispatched、InspectionCompleted、CollectionResultConsumed", integration="现有采集平台子应用", files="FileArtifact", workflow='在线先独立预检再新任务授权；临时密码重新输入；离线新尝试保留失败；结果进入报告', authorization="AssignedProjectDeviceScope、BusinessObjectDeviceCredentialScope")),
    (('INS-04',), contract("InspectionTask、CollectionTask", "srv_inspection_task、srv_inspection_task_rule_snapshot、plt_collection_task", "/inspection-tasks、/collection-tasks", events="InspectionDispatched、InspectionCompleted、CollectionResultConsumed", integration="现有采集平台子应用", files="FileArtifact", workflow='独立预检任务及有效期；通过不授予正式任务权限，端点/认证变化重检', authorization="AssignedProjectDeviceScope、BusinessObjectDeviceCredentialScope")),
    (('INS-07',), contract("InspectionTask、CollectionTask", "srv_inspection_task、srv_inspection_task_rule_snapshot、plt_collection_task", "/inspection-tasks、/collection-tasks", events="InspectionDispatched、InspectionCompleted、CollectionResultConsumed", integration="现有采集平台子应用", files="FileArtifact", workflow="方式选择、预检/执行、业务消费和归档门禁", authorization='工程师申请归档及授权问题确认；服务经理执行最终归档；三类权限不互相推导')),
    (('INS-03',), contract("InspectionRule", "srv_inspection_rule、srv_inspection_rule_revision", "/inspection-rules、/{id}/revisions", workflow="规则配置、发布版本和任务冻结", authorization="AssignedProjectDeviceScope；规则维护/使用分离")),
    (('INS-09',), contract("InspectionRule", "srv_inspection_rule、srv_inspection_rule_revision", "/inspection-rules、/{id}/revisions", workflow='同一巡检规则版本化发布；单命令1..30秒硬上限，超限无审批例外', authorization="AssignedProjectDeviceScope；规则维护/使用分离")),
    ...
    (('INT-09',), contract("AuthorizationGrant", "plt_authorization_grant、ast_asset_sync_item", "/authorization-grants", integration="LDAP/AD", workflow='目录认证/账号/组织结构Owner与HR属性分离；离职或禁用任一有效拒绝，跨来源不按时间覆盖；应急账号受控', authorization="平台RBAC/DataScope；目录组不直授项目角色")),
    ...
    (("PLT-02",), contract("FileArtifact", "plt_file_artifact、plt_file_version、plt_file_reference", "/files:init-upload、/files/{id}:complete-upload、/file-references", events="FileVersionCommitted、FileReferenceAttached/Detached、FileArchived", files="FileArtifact", workflow="文件身份/版本/权限；外部来源认证+契约验签+对象授权+幂等均须成立；扫描默认关闭真实SKIPPED；初始化上传、内容校验、版本提交、引用和归档", authorization="FileBusinessScope；下载实时回源业务权限")),
    (("COM-01",), contract("Contract、SalesOrder、OrderLine、DeliveryScope、DeliveryScopeDetail、AcceptanceScopeBinding、ProjectStageSnapshot、ProjectScopeVersion", "com_contract、com_sales_order、com_sales_order_line、com_delivery_scope、com_delivery_scope_detail、acc_acceptance_scope_binding、proj_project_stage_snapshot、com_delivery_scope_project_version、com_project_scope_revision", "/contracts、/sales-orders、/order-lines、/delivery-scopes；内部OrganizationScopeApi、ProjectOfficeFactApi、ProjectAcceptanceStageFactApi、AcceptanceScopeBindingApi、AcceptanceScopeGuardApi、DeliveryScopeAcceptanceLockApi", events="DeliveryScopeAssigned/Released；ProjectStageChanged仅作提交后通知，不触发绑定", integration="ERP（合同订单权威）；CRM仅提供项目/客户上下文", workflow="ERP来源版本与单位精度守卫、项目办事处发生时快照、范围追加版本、V70必填快照与明细序号确定性转换、PROJ→COM→ACC统一锁序、阶段进入/新范围生效与绑定原子提交、报告与绑定解耦、超分配及验收减量守卫", authorization="ContractProjectScope：合同管理员按SYSTEM当前有效UserCompanyDepartmentScope.companyCode精确范围，空/不可用失败关闭；敏感字段另需pms:commerce:contract:sensitive-read；ERP核心字段只读")),
...
def phase3_verification(identifier: str, spec: Contract) -> tuple[str, str]:
    ...
    exact: dict[str, tuple[list[str], list[str]]] = {
        ...
        "PM-06": (["同一活动项目范围追加、数量占用、并发版本、全事务回滚、旧终验不覆盖新增范围测试"], ["追加申请、COM范围版本、逐阶段影响、补充验收及原子回滚证据"]),
...
def render(prd: Path) -> str:
    identity_spec = importlib.util.spec_from_file_location("phase2_baseline_identity", Path(__file__).with_name("generate_requirement_traceability.py"))
    ...
    lines = [
        ...
        "",
        f"> 文档状态：`{gates['phase-2']['审查状态']}`",
        f"> 适用基线：PRD V1.8修订{identity['revision']}（`{identity['path']}`；Git Blob `{identity['gitBlob']}`）",
        ...
        f"> Phase 3验证注记状态：`{gates['phase-3']['当前结论']}`；只投影当前Gate，不批准DDL、Feature或Release。",
        "",
