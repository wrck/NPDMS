...
GROUPS: list[tuple[tuple[str, ...], Contract]] = [
    ...
    (("ACC-03",), contract("Acceptance、AcceptanceScopeBinding、ProjectStageSnapshot、DeliveryArtifact", "acc_acceptance、acc_acceptance_report_version、acc_acceptance_report_attachment、acc_project_deliverable、acc_acceptance_scope_binding、proj_project_stage_snapshot", "/acceptances、/acceptances/{id}/report-versions；内部AcceptanceActivityCompletionFactApi、AcceptanceScopeBindingApi、AcceptanceScopeGuardApi、DeliveryScopeAcceptanceLockApi", events="AcceptanceReportVersionEffective、ClosureGateRecheckRequested；报告事件不触发或反推AcceptanceScopeBinding", files="FileArtifact/FileVersion固定引用", workflow="项目阶段进入/验收阶段内新范围生效继续直接复用F-COM-001范围绑定原子路径；PROJ拥有ProjectTask/WorkBinding并以MANDATORY调用ACC完成活动；报告四项完备后形成不可变当前有效版本，终验受当前有效初验守卫；精确D-INITIAL-REPORT/D-FINAL-REPORT更新既有acc_project_deliverable来源索引，索引/归档失败保留报告并待补偿；Q-FCOM-002关闭前不自动关闭/解锁绑定", authorization="pms:acceptance:report:query/write/complete/download；ProjectTreeScope、ProjectTaskScope、FileBusinessScope、TenantScope")),
    ...
    (("ACC-04",), contract("DeliveryArtifact", "acc_project_deliverable、acc_artifact_review、acc_archive_record", "/delivery-artifacts", events="ArtifactAccepted/Archived、ClosureGateRecheckRequested", files="FileArtifact/FileVersion", workflow="以acc_project_deliverable为项目应交实例和唯一来源索引根；来源版本新增、替换、撤销或归档失败保留历史并重校验CLO；F-ACC-001只实现初验/终验报告来源切片，其余来源与统一批量下载留待ACC-04完整Feature", authorization="ProjectStageScope、FileBusinessScope；ACC归档")),
