...
> 状态：`REVIEW_REQUIRED`
> 决策登记SHA-256：`8DE7BB01C5A97FB4216036EFF495565FDE693CDA0CC33C9A1B33838093F05F12`
> 约束清单SHA-256：`B47ECE2D2A4436DE0629262A7EEDCD98DD8D380163218A7D2B68BC624D9C000C`
...
|分组|数量|当前事实|建议裁决方式|
|---|---:|---|---|
|表|61|当前核心迁移子集；新增、修改和移除事实见逐项登记|按ADR及Reviewer证据逐项裁决|
|字段|1,250|当前DDL字段；不包含已移除V3治理表字段|按业务语义、类型和约束分类裁决|
|表选项|61|旧基线未保存|需确认字符比较与存储规则|
|主键|61|旧基线未保存|Q07已按技术不变量确认，Reviewer待签署|
|外键|48|旧基线未保存|Q07已确认同域外键；违规历史数据隔离|
|普通索引|124|旧基线未保存|Q08已接受为候选基线，Feature/P3-E06验证|
|唯一键|127|旧基线未保存|影响重复业务数据，必须业务审查|
|CHECK|92|旧基线未保存|影响异常历史数据，必须业务审查|
...
|`ADDED`|255|当前模型相对历史目标DDL新增|必须有需求或ADR依据|
|`MODIFIED`|3|字段定义或说明发生变化|必须说明是否改变业务含义|
|`REMOVED`|72|历史目标DDL中存在、当前模型已移除|必须确认是范围排除而非数据遗漏|
|`UNVERIFIED_BASELINE_MISSING`|522|历史目录未保存约束和表选项|必须按约束语义分类评审，不能自动接受|

因此，1048项MATCH只保留逐项追溯；真正需要裁决的是新增/修改/移除的模型变化，以及522项缺少历史结构证据的约束、表选项或生成表达式。
...
|---|---|---|---|
|精确键与默认排序规则冲突|61张表默认`utf8mb4_0900_ai_ci`；28个来源键/哈希字段要求原值精确匹配|大小写或重音不同的来源键可能被视为相同|来源键改用二进制排序规则，名称继续使用中文友好排序规则|
...
|普通索引没有查询证据|124个候选索引未绑定查询计划、基数和写入成本|过量索引增加同步写入成本，缺失索引影响树查询和对账|Q08已接受为候选基线；Feature/P3-E06用真实查询和压测定稿|

### 1.5 Q07已按数据架构不变量批量确认的内容

|内容|数量|批量确认的前提|仍未包含的业务判断|
|---|---:|---|---|
|主键结构|61|60张实体/关系表使用单列`id`；分析投影使用`(tenant_id, project_id)`复合主键|不决定业务编码是否可重复|
|租户复合引用键|61|仅支撑同租户复合外键/行引用|不替代业务唯一键|
...
|非负数与计数一致性|13|仅约束物理不变量|不替代数量可分配性检查|
...
|`cut_cutover_support_responsibility_interval`|`uk_cutover_responsibility_current`|`UNIQUE KEY uk_cutover_responsibility_current (tenant_id, current_support_task_id)`|确认租户内业务身份永久唯一|
...
|非负数与计数|13|序号、目标数、树深、成功失败计数|技术规则批量确认|
...
7个当前唯一生成列逐项如下。Q03已确认其业务事实，表达式只依赖稳定有效期、删除标记或主标记，不依赖可扩展业务状态码：
...
|`cut_cutover_support_responsibility_interval.current_support_task_id`|`CASE WHEN effective_to IS NULL THEN support_task_id ELSE NULL END`|当前唯一业务事实|已改为只依赖稳定有效期、删除标记或主标记|
...
|`cut_cutover_support_task`|`source_business_key`|`VARCHAR(128)`|是|`utf8mb4_0900_bin`|
...
|L4 性能候选|124个普通索引|需求方接受候选；Feature Owner+性能Owner验证|Q08进入候选基线，绑定查询后由P3-E06压测定稿|
...
|T-042|`cut_cutover_support_responsibility_interval`|12|`create_time`、`creator`、`current_support_task_id`、`effective_from`、`effective_to`、`evidence_ref`、`handover_reason`、`id`、`interval_no`、`responsible_user_id`、`support_task_id`、`tenant_id`|
|T-043|`cut_cutover_support_task`|21|`create_time`、`creator`、`current_handler_user_id`、`current_responsibility_interval_id`、`current_responsible_user_id`、`cutover_task_id`、`device_scope_snapshot`、`id`、`project_id`、`source_business_key`、`source_system`、`state_machine_version`、`status_code`、`support_scope_hash`、`task_no`、`tenant_id`、`update_time`、`updater`、`version`、`window_end`、`window_start`|
...
|T-045|`imp_configuration_collection_result`|18|`collection_task_id`、`create_time`、`creator`、`device_id`、`device_snapshot`、`id`、`operated_time`、`operator_user_id`、`parser_version`、`project_id`、`project_snapshot`、`raw_log_file_id`、`raw_log_sha256`、`result_type_code`、`result_version_no`、`script_version`、`source_code`、`tenant_id`|
|T-046|`imp_configuration_component_candidate`|16|`candidate_no`、`card_configuration_ref`、`card_model_code`、`card_sn`、`chassis_sn`、`create_time`、`creator`、`evidence_ref`、`id`、`match_status_code`、`matched_device_id`、`parse_attempt_id`、`parse_revision_no`、`parser_version`、`slot_code`、`tenant_id`|
|T-047|`plt_business_document`|13|`create_time`、`creator`、`current_version_id`、`deleted`、`document_code`、`document_name`、`document_type`、`id`、`status`、`tenant_id`、`update_time`、`updater`、`version`|
...
|T-060|`srv_service_incident`|23|`closed_time`、`create_time`、`creator`、`deleted`、`id`、`incident_no`、`incident_title`、`incident_type`、`occurred_time`、`owner_id`、`project_id`、`report_document_id`、`reported_time`、`restored_time`、`root_cause`、`severity`、`solution`、`status`、`symptom`、`tenant_id`、`update_time`、`updater`、`version`|
|T-061|`srv_service_incident_device_relation`|10|`create_time`、`creator`、`deleted`、`device_id`、`id`、`impact_description`、`incident_id`、`tenant_id`、`update_time`、`updater`|
...
|O-045|`imp_configuration_collection_result`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '不可覆盖的配置采集结果、整机Log及项目设备快照'`|待确认字符比较规则后分类接受|
|O-046|`imp_configuration_component_candidate`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '配置Log解析形成的板卡候选及待匹配证据'`|待确认字符比较规则后分类接受|
|O-047|`plt_business_document`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '业务文档元数据'`|待确认字符比较规则后分类接受|
...
|O-060|`srv_service_incident`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '故障及服务事件主档'`|待确认字符比较规则后分类接受|
|O-061|`srv_service_incident_device_relation`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '故障与受影响设备多对多关系'`|待确认字符比较规则后分类接受|
...
|PK-045|`imp_configuration_collection_result`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
|PK-046|`imp_configuration_component_candidate`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
|PK-047|`plt_business_document`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
...
|PK-060|`srv_service_incident`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
|PK-061|`srv_service_incident_device_relation`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
...
|IX-090|`imp_configuration_collection_result`|`KEY idx_configuration_collection_result_device (tenant_id, project_id, device_id, operated_time)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-091|`imp_configuration_collection_result`|`KEY idx_configuration_collection_result_hash (tenant_id, raw_log_sha256)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-092|`imp_configuration_component_candidate`|`KEY idx_configuration_component_candidate_match (tenant_id, match_status_code, create_time)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-093|`imp_configuration_component_candidate`|`KEY idx_configuration_component_candidate_sn (tenant_id, chassis_sn, slot_code, card_sn)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
...
|IX-122|`srv_service_incident`|`KEY idx_incident_owner (tenant_id, owner_id, status)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-123|`srv_service_incident`|`KEY idx_incident_project (tenant_id, project_id, status, occurred_time)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-124|`srv_service_incident_device_relation`|`KEY idx_incident_device_reverse (tenant_id, device_id, incident_id)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
...
|UK-005|`acc_satisfaction_collection_task`|`UNIQUE KEY uk_satisfaction_task_revision (tenant_id, project_id, source_context, source_object_type, source_object_id, source_object_version, business_purpose_code, applicable_timing_code, payment_stage_key, task_revision_no)`|影响重复数据；需逐组业务确认|
...
|UK-083|`cut_cutover_support_responsibility_interval`|`UNIQUE KEY uk_cutover_responsibility_current (tenant_id, current_support_task_id)`|影响重复数据；需逐组业务确认|
|UK-084|`cut_cutover_support_responsibility_interval`|`UNIQUE KEY uk_cutover_responsibility_interval_sequence (tenant_id, support_task_id, interval_no)`|影响重复数据；需逐组业务确认|
|UK-085|`cut_cutover_support_responsibility_interval`|`UNIQUE KEY uk_cutover_support_responsibility_interval_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-086|`cut_cutover_support_task`|`UNIQUE KEY uk_cutover_support_scope_window (tenant_id, cutover_task_id, support_scope_hash, window_start, window_end)`|影响重复数据；需逐组业务确认|
|UK-087|`cut_cutover_support_task`|`UNIQUE KEY uk_cutover_support_task_no (tenant_id, task_no)`|影响重复数据；需逐组业务确认|
|UK-088|`cut_cutover_support_task`|`UNIQUE KEY uk_cutover_support_task_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-089|`imp_configuration_collection_parse_attempt`|`UNIQUE KEY uk_configuration_parse_attempt (tenant_id, collection_result_id, attempt_no)`|影响重复数据；需逐组业务确认|
|UK-090|`imp_configuration_collection_parse_attempt`|`UNIQUE KEY uk_configuration_parse_attempt_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-091|`imp_configuration_collection_result`|`UNIQUE KEY uk_configuration_collection_result (tenant_id, collection_task_id, result_type_code, result_version_no)`|影响重复数据；需逐组业务确认|
|UK-092|`imp_configuration_collection_result`|`UNIQUE KEY uk_configuration_collection_result_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-093|`imp_configuration_component_candidate`|`UNIQUE KEY uk_configuration_component_candidate (tenant_id, parse_attempt_id, candidate_no)`|影响重复数据；需逐组业务确认|
|UK-094|`imp_configuration_component_candidate`|`UNIQUE KEY uk_configuration_component_candidate_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-095|`plt_business_document`|`UNIQUE KEY uk_business_document_code (tenant_id, document_code)`|影响重复数据；需逐组业务确认|
|UK-096|`plt_business_document`|`UNIQUE KEY uk_business_document_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-097|`plt_document_version`|`UNIQUE KEY uk_document_version (tenant_id, document_id, version_no)`|影响重复数据；需逐组业务确认|
|UK-098|`plt_document_version`|`UNIQUE KEY uk_document_version_owner (tenant_id, document_id, id)`|影响重复数据；需逐组业务确认|
|UK-099|`plt_document_version`|`UNIQUE KEY uk_document_version_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-100|`plt_external_key_mapping`|`UNIQUE KEY uk_external_key_map_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-101|`plt_external_key_mapping`|`UNIQUE KEY uk_external_key_source_target ( tenant_id, source_system, source_table, source_pk, target_role, target_sequence, target_table, target_id )`|影响重复数据；需逐组业务确认|
|UK-102|`plt_migration_issue`|`UNIQUE KEY uk_migration_issue_source ( tenant_id, batch_id, source_table, source_pk, issue_type )`|影响重复数据；需逐组业务确认|
|UK-103|`plt_migration_issue`|`UNIQUE KEY uk_migration_issue_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-104|`plt_migration_source_record`|`UNIQUE KEY uk_migration_source_record ( tenant_id, batch_id, source_system, source_table, source_pk )`|影响重复数据；需逐组业务确认|
|UK-105|`plt_migration_source_record`|`UNIQUE KEY uk_migration_source_record_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-106|`plt_sync_batch`|`UNIQUE KEY uk_sync_batch_no (tenant_id, batch_no)`|影响重复数据；需逐组业务确认|
|UK-107|`plt_sync_batch`|`UNIQUE KEY uk_sync_batch_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-108|`proj_project`|`UNIQUE KEY uk_project_code (tenant_id, project_code)`|影响重复数据；需逐组业务确认|
|UK-109|`proj_project`|`UNIQUE KEY uk_project_code_sequence (tenant_id, code_root_id, project_sequence)`|影响重复数据；需逐组业务确认|
|UK-110|`proj_project`|`UNIQUE KEY uk_project_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-111|`proj_project_company_department_relation`|`UNIQUE KEY uk_project_company_department_rel_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-112|`proj_project_company_department_relation`|`UNIQUE KEY uk_project_company_department_role ( tenant_id, project_id, company_code, department_code, relation_role, effective_from )`|影响重复数据；需逐组业务确认|
|UK-113|`proj_project_company_department_relation`|`UNIQUE KEY uk_project_primary_company_department ( tenant_id, primary_project_id, relation_role )`|影响重复数据；需逐组业务确认|
|UK-114|`proj_project_member_assignment`|`UNIQUE KEY uk_project_member_role ( tenant_id, project_id, user_id, member_role, effective_from )`|影响重复数据；需逐组业务确认|
|UK-115|`proj_project_member_assignment`|`UNIQUE KEY uk_project_member_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-116|`proj_project_party`|`UNIQUE KEY uk_project_party_source ( tenant_id, source_system, source_table, source_record_key, party_role )`|影响重复数据；需逐组业务确认|
|UK-117|`proj_project_party`|`UNIQUE KEY uk_project_party_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-118|`proj_project_portfolio`|`UNIQUE KEY uk_portfolio_code (tenant_id, portfolio_code)`|影响重复数据；需逐组业务确认|
|UK-119|`proj_project_portfolio`|`UNIQUE KEY uk_portfolio_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-120|`proj_project_portfolio_member`|`UNIQUE KEY uk_portfolio_project ( tenant_id, portfolio_id, project_id, member_source )`|影响重复数据；需逐组业务确认|
|UK-121|`proj_project_portfolio_member`|`UNIQUE KEY uk_portfolio_project_rel_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-122|`proj_project_relation`|`UNIQUE KEY uk_project_relation ( tenant_id, source_project_id, target_project_id, relation_type )`|影响重复数据；需逐组业务确认|
|UK-123|`proj_project_relation`|`UNIQUE KEY uk_project_relation_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-124|`srv_service_incident`|`UNIQUE KEY uk_service_incident_no (tenant_id, incident_no)`|影响重复数据；需逐组业务确认|
|UK-125|`srv_service_incident`|`UNIQUE KEY uk_service_incident_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-126|`srv_service_incident_device_relation`|`UNIQUE KEY uk_incident_device (tenant_id, incident_id, device_id)`|影响重复数据；需逐组业务确认|
|UK-127|`srv_service_incident_device_relation`|`UNIQUE KEY uk_incident_device_rel_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
...
|CK-067|`imp_configuration_collection_result`|`CONSTRAINT chk_configuration_collection_result_version CHECK (result_version_no > 0)`|影响异常历史数据；需逐组业务确认|
|CK-068|`imp_configuration_component_candidate`|`CONSTRAINT chk_configuration_component_candidate_no CHECK (candidate_no > 0)`|影响异常历史数据；需逐组业务确认|
|CK-069|`plt_business_document`|`CONSTRAINT chk_business_document_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
...
|CK-090|`srv_service_incident`|`CONSTRAINT chk_service_incident_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-091|`srv_service_incident`|`CONSTRAINT chk_service_incident_times CHECK ( restored_time IS NULL OR occurred_time IS NULL OR restored_time >= occurred_time )`|影响异常历史数据；需逐组业务确认|
|CK-092|`srv_service_incident_device_relation`|`CONSTRAINT chk_incident_device_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
