...
> 状态：`REVIEW_REQUIRED`
> 决策登记SHA-256：`1137EDE5B4F9E2F717894BD38E8CEA85986919C1A4A40A443F415929BAFD1AEC`
> 约束清单SHA-256：`AC204D5504AA535124D374E88672A1ED560623F16BBFB43D5EE28F5A182BC00B`
...
|分组|数量|当前事实|建议裁决方式|
|---|---:|---|---|
|表|63|当前核心迁移子集；新增、修改和移除事实见逐项登记|按ADR及Reviewer证据逐项裁决|
|字段|1,242|当前DDL字段；不包含已移除V3治理表字段|按业务语义、类型和约束分类裁决|
|表选项|63|旧基线未保存|需确认字符比较与存储规则|
|主键|63|旧基线未保存|Q07已按技术不变量确认，Reviewer待签署|
|外键|48|旧基线未保存|Q07已确认同域外键；违规历史数据隔离|
|普通索引|126|旧基线未保存|Q08已接受为候选基线，Feature/P3-E06验证|
|唯一键|130|旧基线未保存|影响重复业务数据，必须业务审查|
...
|`ADDED`|249|当前模型相对历史目标DDL新增|必须有需求或ADR依据|
|`MODIFIED`|3|字段定义或说明发生变化|必须说明是否改变业务含义|
|`REMOVED`|72|历史目标DDL中存在、当前模型已移除|必须确认是范围排除而非数据遗漏|
|`UNVERIFIED_BASELINE_MISSING`|533|历史目录未保存约束和表选项|必须按约束语义分类评审，不能自动接受|

因此，1048项MATCH只保留逐项追溯；真正需要裁决的是新增/修改/移除的模型变化，以及533项缺少历史结构证据的约束、表选项或生成表达式。
...
|CUT|3|`cut_cutover_support_history`、`cut_cutover_support_responsibility_interval`、`cut_cutover_support_task`|
|IMP|2|`imp_configuration_collection_parse_attempt`、`imp_configuration_component_candidate`|
|PLT|7|`plt_business_document`、`plt_directory_sync_snapshot`、`plt_document_version`、`plt_external_key_mapping`、`plt_migration_issue`、`plt_migration_source_record`、`plt_sync_batch`|
...
|SRV|4|`srv_historical_time_record`、`srv_historical_work_order`、`srv_service_incident`、`srv_service_incident_device_relation`|
...
|---|---|---|---|
|精确键与默认排序规则冲突|63张表默认`utf8mb4_0900_ai_ci`；29个来源键/哈希字段要求原值精确匹配|大小写或重音不同的来源键可能被视为相同|来源键改用二进制排序规则，名称继续使用中文友好排序规则|
...
|普通索引没有查询证据|126个候选索引未绑定查询计划、基数和写入成本|过量索引增加同步写入成本，缺失索引影响树查询和对账|Q08已接受为候选基线；Feature/P3-E06用真实查询和压测定稿|

### 1.5 Q07已按数据架构不变量批量确认的内容

|内容|数量|批量确认的前提|仍未包含的业务判断|
|---|---:|---|---|
|主键结构|63|62张实体/关系表使用单列`id`；分析投影使用`(tenant_id, project_id)`复合主键|不决定业务编码是否可重复|
|租户复合引用键|63|仅支撑同租户复合外键/行引用|不替代业务唯一键|
|同领域物理外键|48|48个外键的父子表均在同一领域；违规旧数据进入迁移问题池|不授权跨Context直接访问Repository|
|软删除检查|45|`deleted`稳定为0/1技术字段|删除不得释放永久业务键|
|时间顺序检查|21|只拒绝结束早于开始，不补造旧数据时间|不决定业务有效期|
|稳定布尔标志|8|字段确为稳定0/1标志|业务状态不能压缩成布尔值|
...
|`cut_cutover_support_task`|`uk_cutover_support_task_no`|`UNIQUE KEY uk_cutover_support_task_no (tenant_id, task_no)`|确认租户内业务身份永久唯一|
...
|`plt_directory_sync_snapshot`|`uk_directory_sync_snapshot_source`|`UNIQUE KEY uk_directory_sync_snapshot_source (tenant_id, source_system, source_key)`|确认来源键精确匹配且重放幂等|
...
|`srv_historical_time_record`|`uk_historical_time_record_source`|`UNIQUE KEY uk_historical_time_record_source (tenant_id, source_system, source_business_key)`|确认来源键精确匹配且重放幂等|
|`srv_historical_work_order`|`uk_historical_work_order_source`|`UNIQUE KEY uk_historical_work_order_source (tenant_id, source_system, source_business_key)`|确认来源键精确匹配且重放幂等|
...
|表|唯一键|当前字段组合|判断重点|
|---|---|---|---|
|`acc_satisfaction_collection_task`|`uk_satisfaction_task_revision`|`UNIQUE KEY uk_satisfaction_task_revision (tenant_id, source_context, source_object_type, source_object_id, task_revision_no)`|确认版本/序号只增不复用|
...
|`cut_cutover_support_history`|`uk_cutover_support_history_sequence`|`UNIQUE KEY uk_cutover_support_history_sequence (tenant_id, support_task_id, history_no)`|确认版本/序号只增不复用|
|`cut_cutover_support_responsibility_interval`|`uk_cutover_responsibility_interval_sequence`|`UNIQUE KEY uk_cutover_responsibility_interval_sequence (tenant_id, support_task_id, interval_no)`|确认版本/序号只增不复用|
...
|`cut_cutover_support_task`|`uk_cutover_support_scope_window`|`UNIQUE KEY uk_cutover_support_scope_window (tenant_id, cutover_task_id, support_scope_hash, window_start, window_end)`|确认字段完整表达关系粒度，重点检查NULL|
...
|`cut_cutover_support_history`|`uk_cutover_support_history_tenant_row`|`UNIQUE KEY uk_cutover_support_history_tenant_row (tenant_id, id)`|技术引用键，可按架构规则确认|
|`cut_cutover_support_responsibility_interval`|`uk_cutover_support_responsibility_interval_tenant_row`|`UNIQUE KEY uk_cutover_support_responsibility_interval_tenant_row (tenant_id, id)`|技术引用键，可按架构规则确认|
|`cut_cutover_support_task`|`uk_cutover_support_task_tenant_row`|`UNIQUE KEY uk_cutover_support_task_tenant_row (tenant_id, id)`|技术引用键，可按架构规则确认|
...
|`plt_directory_sync_snapshot`|`uk_directory_sync_snapshot_tenant_row`|`UNIQUE KEY uk_directory_sync_snapshot_tenant_row (tenant_id, id)`|技术引用键，可按架构规则确认|
...
|`srv_historical_time_record`|`uk_historical_time_record_tenant_row`|`UNIQUE KEY uk_historical_time_record_tenant_row (tenant_id, id)`|技术引用键，可按架构规则确认|
|`srv_historical_work_order`|`uk_historical_work_order_tenant_row`|`UNIQUE KEY uk_historical_work_order_tenant_row (tenant_id, id)`|技术引用键，可按架构规则确认|
...
|时间顺序|21|结束不得早于开始|接受；旧数据缺失保持NULL|
|稳定布尔标志|8|主标记、必需标记等|仅稳定二值字段可接受|
...
5个当前唯一生成列逐项如下。Q03已确认其业务事实，表达式只依赖稳定有效期、删除标记或主标记，不依赖可扩展业务状态码：
...
|`srv_historical_time_record`|`source_business_key`|`VARCHAR(128)`|否|`utf8mb4_0900_bin`|
|`srv_historical_work_order`|`source_business_key`|`VARCHAR(128)`|否|`utf8mb4_0900_bin`|
...
|L4 性能候选|126个普通索引|需求方接受候选；Feature Owner+性能Owner验证|Q08进入候选基线，绑定查询后由P3-E06压测定稿|
...
|T-003|`acc_satisfaction_collection_task`|16|`create_time`、`creator`、`current_responsible_user_id`、`frozen_threshold`、`id`、`source_context`、`source_object_id`、`source_object_type`、`state_machine_version`、`status_code`、`task_revision_no`、`template_version`、`tenant_id`、`update_time`、`updater`、`version`|
|T-004|`acc_satisfaction_questionnaire`|9|`create_time`、`creator`、`frozen_question_json`、`frozen_threshold`、`id`、`questionnaire_revision_no`、`task_id`、`template_version`、`tenant_id`|
|T-005|`acc_satisfaction_response`|11|`answer_json`、`attachment_refs_json`、`create_time`、`creator`、`id`、`questionnaire_id`、`request_id`、`response_no`、`signature_ref`、`submit_time`、`tenant_id`|
|T-006|`acc_satisfaction_result`|12|`create_time`、`creator`、`decision_rule_version`、`decision_time`、`frozen_threshold`、`id`、`passed`、`questionnaire_id`、`response_id`、`result_no`、`score`、`tenant_id`|
...
|T-041|`cut_cutover_support_history`|12|`action_code`、`create_time`、`evidence_ref`、`history_no`、`id`、`occurred_time`、`operator_user_id`、`reason`、`status_after_code`、`status_before_code`、`support_task_id`、`tenant_id`|
|T-042|`cut_cutover_support_responsibility_interval`|11|`create_time`、`creator`、`effective_from`、`effective_to`、`evidence_ref`、`handover_reason`、`id`、`interval_no`、`responsible_user_id`、`support_task_id`、`tenant_id`|
|T-043|`cut_cutover_support_task`|15|`create_time`、`creator`、`current_responsible_user_id`、`cutover_task_id`、`id`、`state_machine_version`、`status_code`、`support_scope_hash`、`task_no`、`tenant_id`、`update_time`、`updater`、`version`、`window_end`、`window_start`|
|T-044|`imp_configuration_collection_parse_attempt`|12|`attempt_no`、`collection_result_id`、`completed_time`、`create_time`、`creator`、`error_summary`、`evidence_ref`、`id`、`parse_status_code`、`parser_version`、`started_time`、`tenant_id`|
|T-045|`imp_configuration_component_candidate`|14|`candidate_no`、`card_model_code`、`card_sn`、`chassis_sn`、`create_time`、`creator`、`evidence_ref`、`id`、`match_status_code`、`matched_device_id`、`parse_attempt_id`、`parser_version`、`slot_code`、`tenant_id`|
...
|T-047|`plt_directory_sync_snapshot`|20|`create_time`、`creator`、`employment_status_code`、`id`、`organization_code`、`person_name`、`person_no`、`position_code`、`source_key`、`source_payload`、`source_payload_sha256`、`source_system`、`source_updated_time`、`source_version`、`sync_batch_no`、`sync_watermark`、`synced_time`、`tenant_id`、`update_time`、`updater`|
|T-048|`plt_document_version`|15|`create_time`、`creator`、`deleted`、`document_id`、`file_checksum`、`file_id`、`file_name`、`id`、`status`、`tenant_id`、`update_time`、`updater`、`uploaded_by`、`uploaded_time`、`version_no`|
|T-049|`plt_external_key_mapping`|17|`batch_id`、`create_time`、`creator`、`id`、`mapping_status`、`source_business_key`、`source_checksum`、`source_pk`、`source_system`、`source_table`、`target_id`、`target_role`、`target_sequence`、`target_table`、`tenant_id`、`update_time`、`updater`|
|T-050|`plt_migration_issue`|18|`batch_id`、`candidate_target_ids`、`create_time`、`creator`、`id`、`issue_type`、`raw_business_key`、`raw_payload`、`resolution_action`、`resolution_status`、`resolved_time`、`resolver`、`source_pk`、`source_system`、`source_table`、`tenant_id`、`update_time`、`updater`|
|T-051|`plt_migration_source_record`|16|`batch_id`、`create_time`、`creator`、`extracted_time`、`id`、`mapped_target_count`、`mapping_status`、`source_business_key`、`source_checksum`、`source_payload`、`source_pk`、`source_system`、`source_table`、`tenant_id`、`update_time`、`updater`|
|T-052|`plt_sync_batch`|20|`batch_no`、`create_time`、`creator`、`error_summary`、`failure_count`、`finished_time`、`id`、`object_type`、`read_count`、`source_cursor`、`source_extract_checksum`、`source_extract_location`、`source_system`、`started_time`、`status`、`success_count`、`sync_mode`、`tenant_id`、`update_time`、`updater`|
|T-053|`proj_project`|53|`business_type`、`code_root_id`、`code_rule_version`、`company_code`、`company_id`、`company_name`、`create_time`、`creator`、`customer_code`、`customer_id`、`customer_name`、`customer_project_name`、`deleted`、`department_code`、`department_id`、`department_name`、`expend_code`、`expend_name`、`id`、`implementation_mode`、`industry_code`、`industry_name`、`lifecycle_template_id`、`major_project_level`、`manager_employee_no`、`manager_id`、`manager_name`、`market_code`、`market_name`、`not_track_reason`、`parent_id`、`project_category`、`project_close_time`、`project_code`、`project_name`、`project_refresh_time`、`project_sequence`、`project_start_time`、`project_type`、`root_id`、`sales_type`、`service_level_code`、`source_type`、`status`、`system_code`、`system_name`、`tenant_id`、`tree_depth`、`tree_path`、`tree_sort`、`update_time`、`updater`、`version`|
|T-054|`proj_project_company_department_relation`|21|`company_code`、`company_id`、`company_name`、`create_time`、`creator`、`deleted`、`department_code`、`department_id`、`department_name`、`effective_from`、`effective_to`、`id`、`is_primary`、`primary_project_id`、`project_id`、`relation_role`、`status`、`tenant_id`、`update_time`、`updater`、`version`|
|T-055|`proj_project_member_assignment`|22|`company_code`、`company_id`、`company_name`、`create_time`、`creator`、`deleted`、`department_code`、`department_name`、`effective_from`、`effective_to`、`employee_no`、`id`、`member_name`、`member_role`、`project_id`、`responsibility`、`status`、`tenant_id`、`update_time`、`updater`、`user_id`、`version`|
|T-056|`proj_project_party`|20|`contact_name`、`create_time`、`creator`、`deleted`、`effective_from`、`effective_to`、`id`、`party_code`、`party_name`、`party_role`、`phone`、`project_id`、`source_record_key`、`source_system`、`source_table`、`status`、`tenant_id`、`update_time`、`updater`、`version`|
|T-057|`proj_project_portfolio`|16|`create_time`、`creator`、`deleted`、`id`、`member_rule`、`member_rule_type`、`owner_id`、`portfolio_code`、`portfolio_name`、`status`、`tenant_id`、`update_time`、`updater`、`valid_from`、`valid_to`、`version`|
|T-058|`proj_project_portfolio_member`|12|`create_time`、`creator`、`deleted`、`effective_from`、`effective_to`、`id`、`member_source`、`portfolio_id`、`project_id`、`tenant_id`、`update_time`、`updater`|
|T-059|`proj_project_relation`|14|`create_time`、`creator`、`deleted`、`effective_time`、`id`、`reason`、`relation_type`、`source_project_id`、`status`、`target_project_id`、`tenant_id`、`update_time`、`updater`、`version`|
|T-060|`srv_historical_time_record`|15|`direction_code`、`duration_hours`、`id`、`imported_time`、`record_end_time`、`record_start_time`、`signed_adjustment_hours`、`source_business_key`、`source_payload`、`source_payload_sha256`、`source_responsible_user_key`、`source_status_code`、`source_system`、`source_type_code`、`tenant_id`|
|T-061|`srv_historical_work_order`|15|`approval_history_json`、`attachment_refs_json`、`audit_evidence_json`、`id`、`imported_time`、`source_business_key`、`source_completed_time`、`source_created_time`、`source_payload`、`source_payload_sha256`、`source_responsible_user_key`、`source_status_code`、`source_system`、`source_type_code`、`tenant_id`|
|T-062|`srv_service_incident`|23|`closed_time`、`create_time`、`creator`、`deleted`、`id`、`incident_no`、`incident_title`、`incident_type`、`occurred_time`、`owner_id`、`project_id`、`report_document_id`、`reported_time`、`restored_time`、`root_cause`、`severity`、`solution`、`status`、`symptom`、`tenant_id`、`update_time`、`updater`、`version`|
|T-063|`srv_service_incident_device_relation`|10|`create_time`、`creator`、`deleted`、`device_id`、`id`、`impact_description`、`incident_id`、`tenant_id`、`update_time`、`updater`|
...
|O-003|`acc_satisfaction_collection_task`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '项目成员承办的满意度收集领域任务'`|待确认字符比较规则后分类接受|
|O-004|`acc_satisfaction_questionnaire`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '不可覆盖的满意度问卷冻结实例'`|待确认字符比较规则后分类接受|
|O-005|`acc_satisfaction_response`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '不可覆盖的客户满意度答卷、签字和附件事实'`|待确认字符比较规则后分类接受|
|O-006|`acc_satisfaction_result`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '不可覆盖的满意度评分、阈值与达标判定事实'`|待确认字符比较规则后分类接受|
|O-007|`ana_project_delivery_summary`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '可重建的项目合同、订单、发货和SN汇总读模型'`|待确认字符比较规则后分类接受|
|O-008|`ast_device_component_relation`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '机框、槽位与板卡的当前及历史关系'`|待确认字符比较规则后分类接受|
|O-009|`ast_device_configuration`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '设备分阶段配置主记录'`|待确认字符比较规则后分类接受|
|O-010|`ast_device_configuration_feature`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '设备配置启用特性明细'`|待确认字符比较规则后分类接受|
|O-011|`ast_device_configuration_service`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '设备配置运行服务明细'`|待确认字符比较规则后分类接受|
|O-012|`ast_device_project_assignment`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '设备SN到项目的归属及转移历史'`|待确认字符比较规则后分类接受|
|O-013|`ast_device_relation`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '合同维度主附加SN、RMA替换等设备关系'`|待确认字符比较规则后分类接受|
|O-014|`ast_device_shipment_event`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '设备发货、退回、返还和再次发放的物流生命周期事件'`|待确认字符比较规则后分类接受|
|O-015|`ast_device_sn`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '设备SN主档，不承载重复发货事件'`|待确认字符比较规则后分类接受|
|O-016|`ast_device_version`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '设备组件版本及阶段历史'`|待确认字符比较规则后分类接受|
|O-017|`ast_network_topology`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '项目网络拓扑版本'`|待确认字符比较规则后分类接受|
|O-018|`ast_network_topology_device_relation`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '拓扑节点与设备关系'`|待确认字符比较规则后分类接受|
|O-019|`ast_product`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '产品主档，安服属性由产品配置判定'`|待确认字符比较规则后分类接受|
|O-020|`ast_product_release`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '产品版本发布与支持周期'`|待确认字符比较规则后分类接受|
|O-021|`com_contract`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '合同主档，以所属公司和合同号为业务唯一键'`|待确认字符比较规则后分类接受|
|O-022|`com_contract_receivable`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = 'SAP合同回款来源记录，保留公司待解析和一号多行证据'`|待确认字符比较规则后分类接受|
|O-023|`com_crm_execution_config`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = 'CRM已获得的执行单产品配置，仅作辅助证据'`|待确认字符比较规则后分类接受|
|O-024|`com_crm_execution_order`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = 'CRM执行单辅助主档，安服仅保存正向证据'`|待确认字符比较规则后分类接受|
|O-025|`com_delivery_scope`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '项目对ERP订单行的权威实施范围'`|待确认字符比较规则后分类接受|
|O-026|`com_delivery_scope_detail`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '交付范围按地点、产品或设备类型及批次拆分的明细'`|待确认字符比较规则后分类接受|
|O-027|`com_execution_order_merge_batch`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '特殊业务合并下单批次'`|待确认字符比较规则后分类接受|
|O-028|`com_execution_order_merge_member`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '特殊合并下单执行单成员，不限制成员数量'`|待确认字符比较规则后分类接受|
|O-029|`com_order_change_relation`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '改单、拆分、替代和退货订单血缘'`|待确认字符比较规则后分类接受|
|O-030|`com_order_contract_relation`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '合同与ERP订单N:N关系'`|待确认字符比较规则后分类接受|
|O-031|`com_order_execution_relation`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = 'ERP订单与CRM执行单辅助关系'`|待确认字符比较规则后分类接受|
|O-032|`com_order_line_execution_relation`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = 'ERP订单行与CRM执行单辅助关系'`|待确认字符比较规则后分类接受|
|O-033|`com_project_contract_relation`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '项目与合同直接N:N关系'`|待确认字符比较规则后分类接受|
|O-034|`com_sales_order`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = 'ERP销售订单主档'`|待确认字符比较规则后分类接受|
|O-035|`com_sales_order_line`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = 'ERP销售订单行及数量快照'`|待确认字符比较规则后分类接受|
|O-036|`com_shipment_contract_reference`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '发货记录的合同归属，不作为合同主档'`|待确认字符比较规则后分类接受|
|O-037|`com_shipment_package`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '发货装箱单主档'`|待确认字符比较规则后分类接受|
|O-038|`cus_customer`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '客户主档'`|待确认字符比较规则后分类接受|
|O-039|`cus_customer_contact`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '客户联系人'`|待确认字符比较规则后分类接受|
|O-040|`cus_market_relation`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = 'CRM同步的客户市场行业划分组合目录'`|待确认字符比较规则后分类接受|
|O-041|`cut_cutover_support_history`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '割接保障状态与动作的不可覆盖历史'`|待确认字符比较规则后分类接受|
|O-042|`cut_cutover_support_responsibility_interval`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '割接保障责任转交的不可覆盖生效区间'`|待确认字符比较规则后分类接受|
|O-043|`cut_cutover_support_task`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = 'CUT领域割接保障任务，不复用通用工单聚合'`|待确认字符比较规则后分类接受|
|O-044|`imp_configuration_collection_parse_attempt`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '配置采集结果解析尝试，不覆盖原始配置Log'`|待确认字符比较规则后分类接受|
|O-045|`imp_configuration_component_candidate`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '配置Log解析形成的板卡候选及待匹配证据'`|待确认字符比较规则后分类接受|
|O-046|`plt_business_document`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '业务文档元数据'`|待确认字符比较规则后分类接受|
|O-047|`plt_directory_sync_snapshot`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = 'HR通讯录权威来源键、版本和水位快照'`|待确认字符比较规则后分类接受|
|O-048|`plt_document_version`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '业务文档不可变版本'`|待确认字符比较规则后分类接受|
|O-049|`plt_external_key_mapping`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '旧主键到新主键的可追溯映射'`|待确认字符比较规则后分类接受|
|O-050|`plt_migration_issue`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '迁移缺失、重复、多义映射和人工解决记录'`|待确认字符比较规则后分类接受|
|O-051|`plt_migration_source_record`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '迁移批次逐源行的完整原值证据，不因目标归并或去重而覆盖'`|待确认字符比较规则后分类接受|
|O-052|`plt_sync_batch`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '一次性迁移及只读同步批次'`|待确认字符比较规则后分类接受|
|O-053|`proj_project`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '项目主档及非固定层级项目树'`|待确认字符比较规则后分类接受|
|O-054|`proj_project_company_department_relation`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '项目业务角色下的公司与部门组合关系，保留配对但不建立全局主数据从属关系'`|待确认字符比较规则后分类接受|
|O-055|`proj_project_member_assignment`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '项目成员、角色及有效期'`|待确认字符比较规则后分类接受|
|O-056|`proj_project_party`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '项目参与方，按合同客户、最终用户、代理商、服务商等角色保存'`|待确认字符比较规则后分类接受|
|O-057|`proj_project_portfolio`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '项目组合，不改变项目父子层级'`|待确认字符比较规则后分类接受|
|O-058|`proj_project_portfolio_member`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '项目组合成员'`|待确认字符比较规则后分类接受|
|O-059|`proj_project_relation`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '扩容、续采、改造等非树项目关系'`|待确认字符比较规则后分类接受|
|O-060|`srv_historical_time_record`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '旧工时不可删除、不可续流的只读历史事实'`|待确认字符比较规则后分类接受|
|O-061|`srv_historical_work_order`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '旧工单不可删除、不可续流的只读历史事实'`|待确认字符比较规则后分类接受|
|O-062|`srv_service_incident`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '故障及服务事件主档'`|待确认字符比较规则后分类接受|
|O-063|`srv_service_incident_device_relation`|`ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci COMMENT = '故障与受影响设备多对多关系'`|待确认字符比较规则后分类接受|
...
|PK-041|`cut_cutover_support_history`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
|PK-042|`cut_cutover_support_responsibility_interval`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
|PK-043|`cut_cutover_support_task`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
|PK-044|`imp_configuration_collection_parse_attempt`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
...
|PK-047|`plt_directory_sync_snapshot`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
|PK-048|`plt_document_version`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
|PK-049|`plt_external_key_mapping`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
|PK-050|`plt_migration_issue`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
|PK-051|`plt_migration_source_record`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
|PK-052|`plt_sync_batch`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
|PK-053|`proj_project`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
|PK-054|`proj_project_company_department_relation`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
|PK-055|`proj_project_member_assignment`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
|PK-056|`proj_project_party`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
|PK-057|`proj_project_portfolio`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
|PK-058|`proj_project_portfolio_member`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
|PK-059|`proj_project_relation`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
|PK-060|`srv_historical_time_record`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
|PK-061|`srv_historical_work_order`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
|PK-062|`srv_service_incident`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
|PK-063|`srv_service_incident_device_relation`|`PRIMARY KEY (id)`|Q07已确认；Reviewer待签署|
...
|IX-085|`cut_cutover_support_history`|`KEY idx_cutover_support_history_time (tenant_id, support_task_id, occurred_time)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-086|`cut_cutover_support_responsibility_interval`|`KEY idx_cutover_responsibility_current (tenant_id, support_task_id, effective_to, effective_from)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-087|`cut_cutover_support_task`|`KEY idx_cutover_support_task_owner (tenant_id, current_responsible_user_id, status_code)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-088|`cut_cutover_support_task`|`KEY idx_cutover_support_task_window (tenant_id, window_start, window_end)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-089|`imp_configuration_collection_parse_attempt`|`KEY idx_configuration_parse_attempt_result (tenant_id, collection_result_id, started_time)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
...
|IX-092|`plt_directory_sync_snapshot`|`KEY idx_directory_sync_snapshot_batch (tenant_id, sync_batch_no, synced_time)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-093|`plt_directory_sync_snapshot`|`KEY idx_directory_sync_snapshot_org (tenant_id, organization_code, employment_status_code)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-094|`plt_document_version`|`KEY idx_document_file (tenant_id, file_id)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-095|`plt_external_key_mapping`|`KEY idx_external_key_batch (tenant_id, batch_id, mapping_status)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-096|`plt_external_key_mapping`|`KEY idx_external_key_source ( tenant_id, source_system, source_table, source_pk )`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-097|`plt_external_key_mapping`|`KEY idx_external_key_target ( tenant_id, target_table, target_id )`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-098|`plt_migration_issue`|`KEY idx_migration_issue_status ( tenant_id, issue_type, resolution_status, create_time )`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-099|`plt_migration_source_record`|`KEY idx_migration_source_business ( tenant_id, source_system, source_table, source_business_key(191) )`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-100|`plt_migration_source_record`|`KEY idx_migration_source_mapping ( tenant_id, batch_id, source_table, mapping_status )`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-101|`plt_sync_batch`|`KEY idx_sync_batch_object ( tenant_id, source_system, object_type, started_time )`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-102|`proj_project`|`KEY idx_project_company_department ( tenant_id, company_code, department_code, status, id )`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-103|`proj_project`|`KEY idx_project_company_department_id ( tenant_id, company_id, department_id, status, id )`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-104|`proj_project`|`KEY idx_project_customer_code (tenant_id, customer_code, status, id)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-105|`proj_project`|`KEY idx_project_department_company ( tenant_id, department_code, company_code, status, id )`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-106|`proj_project`|`KEY idx_project_manager (tenant_id, manager_id, status)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-107|`proj_project`|`KEY idx_project_manager_employee (tenant_id, manager_employee_no, status, id)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-108|`proj_project`|`KEY idx_project_market_relation ( tenant_id, market_code, system_code, expend_code, industry_code, status, id )`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-109|`proj_project`|`KEY idx_project_parent (tenant_id, parent_id, tree_sort, id)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-110|`proj_project`|`KEY idx_project_path (tenant_id, root_id, tree_path(191))`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-111|`proj_project_company_department_relation`|`KEY idx_project_company_department_id ( tenant_id, company_id, department_id, status, project_id )`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-112|`proj_project_company_department_relation`|`KEY idx_project_company_reverse ( tenant_id, company_code, relation_role, status, project_id )`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-113|`proj_project_company_department_relation`|`KEY idx_project_department_reverse ( tenant_id, department_code, company_code, relation_role, status, project_id )`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-114|`proj_project_member_assignment`|`KEY idx_project_member_company_department ( tenant_id, company_code, department_code, status, project_id )`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-115|`proj_project_member_assignment`|`KEY idx_project_member_employee (tenant_id, employee_no, status, project_id)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-116|`proj_project_member_assignment`|`KEY idx_project_member_user (tenant_id, user_id, status, project_id)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-117|`proj_project_party`|`KEY idx_project_party_code ( tenant_id, party_role, party_code, status )`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-118|`proj_project_party`|`KEY idx_project_party_project ( tenant_id, project_id, party_role, status )`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-119|`proj_project_portfolio`|`KEY idx_portfolio_owner (tenant_id, owner_id, status)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-120|`proj_project_portfolio_member`|`KEY idx_portfolio_project_reverse (tenant_id, project_id, portfolio_id)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-121|`proj_project_relation`|`KEY idx_project_relation_target ( tenant_id, target_project_id, relation_type )`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-122|`srv_historical_time_record`|`KEY idx_historical_time_record_time (tenant_id, record_start_time, source_business_key)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-123|`srv_historical_work_order`|`KEY idx_historical_work_order_time (tenant_id, source_created_time, source_business_key)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-124|`srv_service_incident`|`KEY idx_incident_owner (tenant_id, owner_id, status)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-125|`srv_service_incident`|`KEY idx_incident_project (tenant_id, project_id, status, occurred_time)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
|IX-126|`srv_service_incident_device_relation`|`KEY idx_incident_device_reverse (tenant_id, device_id, incident_id)`|Q08候选基线；Feature查询计划与P3-E06压测验证|
...
|UK-004|`acc_satisfaction_collection_task`|`UNIQUE KEY uk_satisfaction_collection_task_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-005|`acc_satisfaction_collection_task`|`UNIQUE KEY uk_satisfaction_task_revision (tenant_id, source_context, source_object_type, source_object_id, task_revision_no)`|影响重复数据；需逐组业务确认|
|UK-006|`acc_satisfaction_questionnaire`|`UNIQUE KEY uk_satisfaction_questionnaire_revision (tenant_id, task_id, questionnaire_revision_no)`|影响重复数据；需逐组业务确认|
|UK-007|`acc_satisfaction_questionnaire`|`UNIQUE KEY uk_satisfaction_questionnaire_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-008|`acc_satisfaction_response`|`UNIQUE KEY uk_satisfaction_response_request (tenant_id, questionnaire_id, request_id)`|影响重复数据；需逐组业务确认|
|UK-009|`acc_satisfaction_response`|`UNIQUE KEY uk_satisfaction_response_sequence (tenant_id, questionnaire_id, response_no)`|影响重复数据；需逐组业务确认|
|UK-010|`acc_satisfaction_response`|`UNIQUE KEY uk_satisfaction_response_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-011|`acc_satisfaction_result`|`UNIQUE KEY uk_satisfaction_result_response (tenant_id, response_id)`|影响重复数据；需逐组业务确认|
|UK-012|`acc_satisfaction_result`|`UNIQUE KEY uk_satisfaction_result_sequence (tenant_id, questionnaire_id, result_no)`|影响重复数据；需逐组业务确认|
|UK-013|`acc_satisfaction_result`|`UNIQUE KEY uk_satisfaction_result_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-014|`ast_device_component_relation`|`UNIQUE KEY uk_device_component_current_slot (tenant_id, chassis_device_id, current_slot_code)`|影响重复数据；需逐组业务确认|
|UK-015|`ast_device_component_relation`|`UNIQUE KEY uk_device_component_relation_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-016|`ast_device_configuration`|`UNIQUE KEY uk_device_configuration_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-017|`ast_device_configuration_feature`|`UNIQUE KEY uk_device_configuration_feature ( tenant_id, configuration_id, feature_code )`|影响重复数据；需逐组业务确认|
|UK-018|`ast_device_configuration_feature`|`UNIQUE KEY uk_device_configuration_feature_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-019|`ast_device_configuration_service`|`UNIQUE KEY uk_device_configuration_service ( tenant_id, configuration_id, service_code )`|影响重复数据；需逐组业务确认|
|UK-020|`ast_device_configuration_service`|`UNIQUE KEY uk_device_configuration_service_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-021|`ast_device_project_assignment`|`UNIQUE KEY uk_device_assignment_source ( tenant_id, source_system, source_record_key )`|影响重复数据；需逐组业务确认|
|UK-022|`ast_device_project_assignment`|`UNIQUE KEY uk_device_current_assignment (tenant_id, current_device_id)`|影响重复数据；需逐组业务确认|
|UK-023|`ast_device_project_assignment`|`UNIQUE KEY uk_project_device_assignment_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-024|`ast_device_relation`|`UNIQUE KEY uk_device_relation_source ( tenant_id, source_system, source_record_key )`|影响重复数据；需逐组业务确认|
|UK-025|`ast_device_relation`|`UNIQUE KEY uk_device_relation_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-026|`ast_device_shipment_event`|`UNIQUE KEY uk_device_shipment_event_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-027|`ast_device_shipment_event`|`UNIQUE KEY uk_shipment_event_source ( tenant_id, source_system, source_record_key )`|影响重复数据；需逐组业务确认|
|UK-028|`ast_device_sn`|`UNIQUE KEY uk_device_sn (tenant_id, sn)`|影响重复数据；需逐组业务确认|
|UK-029|`ast_device_sn`|`UNIQUE KEY uk_device_sn_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-030|`ast_device_version`|`UNIQUE KEY uk_device_version_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-031|`ast_network_topology`|`UNIQUE KEY uk_network_topology_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-032|`ast_network_topology_device_relation`|`UNIQUE KEY uk_topology_device (tenant_id, topology_id, device_id)`|影响重复数据；需逐组业务确认|
|UK-033|`ast_network_topology_device_relation`|`UNIQUE KEY uk_topology_device_rel_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-034|`ast_product`|`UNIQUE KEY uk_product_code (tenant_id, product_code)`|影响重复数据；需逐组业务确认|
|UK-035|`ast_product`|`UNIQUE KEY uk_product_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-036|`ast_product_release`|`UNIQUE KEY uk_product_release ( tenant_id, product_id, release_version, release_type )`|影响重复数据；需逐组业务确认|
|UK-037|`ast_product_release`|`UNIQUE KEY uk_product_release_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-038|`com_contract`|`UNIQUE KEY uk_contract_business ( tenant_id, company_code, contract_no )`|影响重复数据；需逐组业务确认|
|UK-039|`com_contract`|`UNIQUE KEY uk_contract_master_source ( tenant_id, master_source_system, master_source_record_key )`|影响重复数据；需逐组业务确认|
|UK-040|`com_contract`|`UNIQUE KEY uk_contract_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-041|`com_contract_receivable`|`UNIQUE KEY uk_contract_receivable_source ( tenant_id, source_system, source_record_key )`|影响重复数据；需逐组业务确认|
|UK-042|`com_contract_receivable`|`UNIQUE KEY uk_contract_receivable_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-043|`com_crm_execution_config`|`UNIQUE KEY uk_crm_execution_config ( tenant_id, config_source, source_config_key )`|影响重复数据；需逐组业务确认|
|UK-044|`com_crm_execution_config`|`UNIQUE KEY uk_crm_execution_config_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-045|`com_crm_execution_order`|`UNIQUE KEY uk_crm_execution ( tenant_id, source_system, execution_no )`|影响重复数据；需逐组业务确认|
|UK-046|`com_crm_execution_order`|`UNIQUE KEY uk_crm_execution_order_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-047|`com_delivery_scope`|`UNIQUE KEY uk_project_order_line_scope_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-048|`com_delivery_scope`|`UNIQUE KEY uk_scope_current ( tenant_id, project_id, current_order_line_id )`|影响重复数据；需逐组业务确认|
|UK-049|`com_delivery_scope_detail`|`UNIQUE KEY uk_delivery_scope_detail_sequence ( tenant_id, delivery_scope_id, detail_sequence )`|影响重复数据；需逐组业务确认|
|UK-050|`com_delivery_scope_detail`|`UNIQUE KEY uk_delivery_scope_detail_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-051|`com_execution_order_merge_batch`|`UNIQUE KEY uk_execution_merge_batch ( tenant_id, source_system, source_merge_key )`|影响重复数据；需逐组业务确认|
|UK-052|`com_execution_order_merge_batch`|`UNIQUE KEY uk_execution_merge_batch_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-053|`com_execution_order_merge_member`|`UNIQUE KEY uk_execution_merge_member_source ( tenant_id, merge_batch_id, source_record_key )`|影响重复数据；需逐组业务确认|
|UK-054|`com_execution_order_merge_member`|`UNIQUE KEY uk_execution_merge_member_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-055|`com_order_change_relation`|`UNIQUE KEY uk_order_change ( tenant_id, source_order_id, target_order_id, relation_type )`|影响重复数据；需逐组业务确认|
|UK-056|`com_order_change_relation`|`UNIQUE KEY uk_order_change_rel_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-057|`com_order_contract_relation`|`UNIQUE KEY uk_order_contract (tenant_id, order_id, contract_id)`|影响重复数据；需逐组业务确认|
|UK-058|`com_order_contract_relation`|`UNIQUE KEY uk_order_contract_rel_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-059|`com_order_execution_relation`|`UNIQUE KEY uk_order_execution (tenant_id, order_id, execution_id)`|影响重复数据；需逐组业务确认|
|UK-060|`com_order_execution_relation`|`UNIQUE KEY uk_order_execution_rel_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-061|`com_order_line_execution_relation`|`UNIQUE KEY uk_order_line_execution (tenant_id, order_line_id, execution_id)`|影响重复数据；需逐组业务确认|
|UK-062|`com_order_line_execution_relation`|`UNIQUE KEY uk_order_line_execution_rel_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-063|`com_project_contract_relation`|`UNIQUE KEY uk_project_contract ( tenant_id, project_id, contract_id, relation_role )`|影响重复数据；需逐组业务确认|
|UK-064|`com_project_contract_relation`|`UNIQUE KEY uk_project_contract_rel_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-065|`com_sales_order`|`UNIQUE KEY uk_sales_order_business ( tenant_id, source_system, company_code, order_type, order_no )`|影响重复数据；需逐组业务确认|
|UK-066|`com_sales_order`|`UNIQUE KEY uk_sales_order_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-067|`com_sales_order_line`|`UNIQUE KEY uk_sales_order_line (tenant_id, order_id, line_no)`|影响重复数据；需逐组业务确认|
|UK-068|`com_sales_order_line`|`UNIQUE KEY uk_sales_order_line_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-069|`com_shipment_contract_reference`|`UNIQUE KEY uk_shipment_contract_ref_source ( tenant_id, source_system, source_record_key )`|影响重复数据；需逐组业务确认|
|UK-070|`com_shipment_contract_reference`|`UNIQUE KEY uk_shipment_contract_ref_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-071|`com_shipment_package`|`UNIQUE KEY uk_shipment_package_no ( tenant_id, source_system, package_no )`|影响重复数据；需逐组业务确认|
|UK-072|`com_shipment_package`|`UNIQUE KEY uk_shipment_package_source ( tenant_id, source_system, source_record_key )`|影响重复数据；需逐组业务确认|
|UK-073|`com_shipment_package`|`UNIQUE KEY uk_shipment_package_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-074|`cus_customer`|`UNIQUE KEY uk_customer_code (tenant_id, customer_code)`|影响重复数据；需逐组业务确认|
|UK-075|`cus_customer`|`UNIQUE KEY uk_customer_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-076|`cus_customer_contact`|`UNIQUE KEY uk_customer_contact_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-077|`cus_customer_contact`|`UNIQUE KEY uk_customer_primary_contact (tenant_id, primary_customer_id)`|影响重复数据；需逐组业务确认|
|UK-078|`cus_market_relation`|`UNIQUE KEY uk_market_relation_business ( tenant_id, market_code, system_code, expend_code, industry_code )`|影响重复数据；需逐组业务确认|
|UK-079|`cus_market_relation`|`UNIQUE KEY uk_market_relation_source ( tenant_id, source_system, source_record_key )`|影响重复数据；需逐组业务确认|
|UK-080|`cus_market_relation`|`UNIQUE KEY uk_market_relation_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-081|`cut_cutover_support_history`|`UNIQUE KEY uk_cutover_support_history_sequence (tenant_id, support_task_id, history_no)`|影响重复数据；需逐组业务确认|
|UK-082|`cut_cutover_support_history`|`UNIQUE KEY uk_cutover_support_history_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-083|`cut_cutover_support_responsibility_interval`|`UNIQUE KEY uk_cutover_responsibility_interval_sequence (tenant_id, support_task_id, interval_no)`|影响重复数据；需逐组业务确认|
|UK-084|`cut_cutover_support_responsibility_interval`|`UNIQUE KEY uk_cutover_support_responsibility_interval_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-085|`cut_cutover_support_task`|`UNIQUE KEY uk_cutover_support_scope_window (tenant_id, cutover_task_id, support_scope_hash, window_start, window_end)`|影响重复数据；需逐组业务确认|
|UK-086|`cut_cutover_support_task`|`UNIQUE KEY uk_cutover_support_task_no (tenant_id, task_no)`|影响重复数据；需逐组业务确认|
|UK-087|`cut_cutover_support_task`|`UNIQUE KEY uk_cutover_support_task_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-088|`imp_configuration_collection_parse_attempt`|`UNIQUE KEY uk_configuration_parse_attempt (tenant_id, collection_result_id, attempt_no)`|影响重复数据；需逐组业务确认|
|UK-089|`imp_configuration_collection_parse_attempt`|`UNIQUE KEY uk_configuration_parse_attempt_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-090|`imp_configuration_component_candidate`|`UNIQUE KEY uk_configuration_component_candidate (tenant_id, parse_attempt_id, candidate_no)`|影响重复数据；需逐组业务确认|
|UK-091|`imp_configuration_component_candidate`|`UNIQUE KEY uk_configuration_component_candidate_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-092|`plt_business_document`|`UNIQUE KEY uk_business_document_code (tenant_id, document_code)`|影响重复数据；需逐组业务确认|
|UK-093|`plt_business_document`|`UNIQUE KEY uk_business_document_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-094|`plt_directory_sync_snapshot`|`UNIQUE KEY uk_directory_sync_snapshot_source (tenant_id, source_system, source_key)`|影响重复数据；需逐组业务确认|
|UK-095|`plt_directory_sync_snapshot`|`UNIQUE KEY uk_directory_sync_snapshot_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-096|`plt_document_version`|`UNIQUE KEY uk_document_version (tenant_id, document_id, version_no)`|影响重复数据；需逐组业务确认|
|UK-097|`plt_document_version`|`UNIQUE KEY uk_document_version_owner (tenant_id, document_id, id)`|影响重复数据；需逐组业务确认|
|UK-098|`plt_document_version`|`UNIQUE KEY uk_document_version_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-099|`plt_external_key_mapping`|`UNIQUE KEY uk_external_key_map_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-100|`plt_external_key_mapping`|`UNIQUE KEY uk_external_key_source_target ( tenant_id, source_system, source_table, source_pk, target_role, target_sequence, target_table, target_id )`|影响重复数据；需逐组业务确认|
|UK-101|`plt_migration_issue`|`UNIQUE KEY uk_migration_issue_source ( tenant_id, batch_id, source_table, source_pk, issue_type )`|影响重复数据；需逐组业务确认|
|UK-102|`plt_migration_issue`|`UNIQUE KEY uk_migration_issue_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-103|`plt_migration_source_record`|`UNIQUE KEY uk_migration_source_record ( tenant_id, batch_id, source_system, source_table, source_pk )`|影响重复数据；需逐组业务确认|
|UK-104|`plt_migration_source_record`|`UNIQUE KEY uk_migration_source_record_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-105|`plt_sync_batch`|`UNIQUE KEY uk_sync_batch_no (tenant_id, batch_no)`|影响重复数据；需逐组业务确认|
|UK-106|`plt_sync_batch`|`UNIQUE KEY uk_sync_batch_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-107|`proj_project`|`UNIQUE KEY uk_project_code (tenant_id, project_code)`|影响重复数据；需逐组业务确认|
|UK-108|`proj_project`|`UNIQUE KEY uk_project_code_sequence (tenant_id, code_root_id, project_sequence)`|影响重复数据；需逐组业务确认|
|UK-109|`proj_project`|`UNIQUE KEY uk_project_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-110|`proj_project_company_department_relation`|`UNIQUE KEY uk_project_company_department_rel_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-111|`proj_project_company_department_relation`|`UNIQUE KEY uk_project_company_department_role ( tenant_id, project_id, company_code, department_code, relation_role, effective_from )`|影响重复数据；需逐组业务确认|
|UK-112|`proj_project_company_department_relation`|`UNIQUE KEY uk_project_primary_company_department ( tenant_id, primary_project_id, relation_role )`|影响重复数据；需逐组业务确认|
|UK-113|`proj_project_member_assignment`|`UNIQUE KEY uk_project_member_role ( tenant_id, project_id, user_id, member_role, effective_from )`|影响重复数据；需逐组业务确认|
|UK-114|`proj_project_member_assignment`|`UNIQUE KEY uk_project_member_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-115|`proj_project_party`|`UNIQUE KEY uk_project_party_source ( tenant_id, source_system, source_table, source_record_key, party_role )`|影响重复数据；需逐组业务确认|
|UK-116|`proj_project_party`|`UNIQUE KEY uk_project_party_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-117|`proj_project_portfolio`|`UNIQUE KEY uk_portfolio_code (tenant_id, portfolio_code)`|影响重复数据；需逐组业务确认|
|UK-118|`proj_project_portfolio`|`UNIQUE KEY uk_portfolio_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-119|`proj_project_portfolio_member`|`UNIQUE KEY uk_portfolio_project ( tenant_id, portfolio_id, project_id, member_source )`|影响重复数据；需逐组业务确认|
|UK-120|`proj_project_portfolio_member`|`UNIQUE KEY uk_portfolio_project_rel_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-121|`proj_project_relation`|`UNIQUE KEY uk_project_relation ( tenant_id, source_project_id, target_project_id, relation_type )`|影响重复数据；需逐组业务确认|
|UK-122|`proj_project_relation`|`UNIQUE KEY uk_project_relation_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-123|`srv_historical_time_record`|`UNIQUE KEY uk_historical_time_record_source (tenant_id, source_system, source_business_key)`|影响重复数据；需逐组业务确认|
|UK-124|`srv_historical_time_record`|`UNIQUE KEY uk_historical_time_record_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-125|`srv_historical_work_order`|`UNIQUE KEY uk_historical_work_order_source (tenant_id, source_system, source_business_key)`|影响重复数据；需逐组业务确认|
|UK-126|`srv_historical_work_order`|`UNIQUE KEY uk_historical_work_order_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-127|`srv_service_incident`|`UNIQUE KEY uk_service_incident_no (tenant_id, incident_no)`|影响重复数据；需逐组业务确认|
|UK-128|`srv_service_incident`|`UNIQUE KEY uk_service_incident_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
|UK-129|`srv_service_incident_device_relation`|`UNIQUE KEY uk_incident_device (tenant_id, incident_id, device_id)`|影响重复数据；需逐组业务确认|
|UK-130|`srv_service_incident_device_relation`|`UNIQUE KEY uk_incident_device_rel_tenant_row (tenant_id, id)`|影响重复数据；需逐组业务确认|
...
|CK-004|`acc_satisfaction_collection_task`|`CONSTRAINT chk_satisfaction_task_revision CHECK (task_revision_no > 0)`|影响异常历史数据；需逐组业务确认|
|CK-005|`acc_satisfaction_questionnaire`|`CONSTRAINT chk_satisfaction_questionnaire_revision CHECK (questionnaire_revision_no > 0)`|影响异常历史数据；需逐组业务确认|
|CK-006|`acc_satisfaction_response`|`CONSTRAINT chk_satisfaction_response_sequence CHECK (response_no > 0)`|影响异常历史数据；需逐组业务确认|
|CK-007|`acc_satisfaction_result`|`CONSTRAINT chk_satisfaction_result_passed CHECK (passed IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-008|`acc_satisfaction_result`|`CONSTRAINT chk_satisfaction_result_sequence CHECK (result_no > 0)`|影响异常历史数据；需逐组业务确认|
|CK-009|`ast_device_component_relation`|`CONSTRAINT chk_device_component_dates CHECK (effective_to IS NULL OR effective_to >= effective_from)`|影响异常历史数据；需逐组业务确认|
|CK-010|`ast_device_configuration`|`CONSTRAINT chk_device_configuration_dates CHECK (effective_to IS NULL OR effective_to >= effective_from)`|影响异常历史数据；需逐组业务确认|
|CK-011|`ast_device_configuration`|`CONSTRAINT chk_device_configuration_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-012|`ast_device_configuration_feature`|`CONSTRAINT chk_configuration_feature_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-013|`ast_device_configuration_service`|`CONSTRAINT chk_configuration_service_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-014|`ast_device_project_assignment`|`CONSTRAINT chk_device_assignment_dates CHECK (effective_to IS NULL OR effective_from IS NULL OR effective_to >= effective_from)`|影响异常历史数据；需逐组业务确认|
|CK-015|`ast_device_project_assignment`|`CONSTRAINT chk_device_assignment_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-016|`ast_device_relation`|`CONSTRAINT chk_device_relation_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-017|`ast_device_relation`|`CONSTRAINT chk_device_relation_self CHECK (source_device_id <> target_device_id)`|影响异常历史数据；需逐组业务确认|
|CK-018|`ast_device_shipment_event`|`CONSTRAINT chk_shipment_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-019|`ast_device_sn`|`CONSTRAINT chk_device_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-020|`ast_device_sn`|`CONSTRAINT chk_device_secondary_cache CHECK ( secondary_sn IS NOT NULL OR secondary_item IS NULL )`|影响异常历史数据；需逐组业务确认|
|CK-021|`ast_device_sn`|`CONSTRAINT chk_device_secondary_self CHECK ( secondary_sn IS NULL OR secondary_sn <> sn )`|影响异常历史数据；需逐组业务确认|
|CK-022|`ast_device_version`|`CONSTRAINT chk_device_version_dates CHECK (effective_to IS NULL OR effective_to >= effective_from)`|影响异常历史数据；需逐组业务确认|
|CK-023|`ast_device_version`|`CONSTRAINT chk_device_version_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-024|`ast_network_topology`|`CONSTRAINT chk_network_topology_dates CHECK (effective_to IS NULL OR effective_to >= effective_from)`|影响异常历史数据；需逐组业务确认|
|CK-025|`ast_network_topology`|`CONSTRAINT chk_network_topology_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-026|`ast_network_topology_device_relation`|`CONSTRAINT chk_topology_device_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-027|`ast_product`|`CONSTRAINT chk_product_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-028|`ast_product`|`CONSTRAINT chk_product_service CHECK (service_product_flag IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-029|`ast_product_release`|`CONSTRAINT chk_product_release_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-030|`com_contract`|`CONSTRAINT chk_contract_dates CHECK (expiry_date IS NULL OR effective_date IS NULL OR expiry_date >= effective_date)`|影响异常历史数据；需逐组业务确认|
|CK-031|`com_contract`|`CONSTRAINT chk_contract_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-032|`com_contract_receivable`|`CONSTRAINT chk_contract_receivable_dates CHECK ( source_effective_to IS NULL OR source_effective_from IS NULL OR source_effective_to >= source_effective_from )`|影响异常历史数据；需逐组业务确认|
|CK-033|`com_contract_receivable`|`CONSTRAINT chk_contract_receivable_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-034|`com_crm_execution_config`|`CONSTRAINT chk_crm_execution_config_af CHECK (is_af_evidence IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-035|`com_crm_execution_config`|`CONSTRAINT chk_crm_execution_config_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-036|`com_crm_execution_order`|`CONSTRAINT chk_crm_execution_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-037|`com_delivery_scope`|`CONSTRAINT chk_scope_dates CHECK (effective_to IS NULL OR effective_from IS NULL OR effective_to >= effective_from)`|影响异常历史数据；需逐组业务确认|
|CK-038|`com_delivery_scope`|`CONSTRAINT chk_scope_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-039|`com_delivery_scope_detail`|`CONSTRAINT chk_delivery_scope_detail_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-040|`com_delivery_scope_detail`|`CONSTRAINT chk_delivery_scope_detail_subject CHECK (product_code IS NOT NULL OR device_type_code IS NOT NULL)`|影响异常历史数据；需逐组业务确认|
|CK-041|`com_execution_order_merge_batch`|`CONSTRAINT chk_execution_merge_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-042|`com_execution_order_merge_member`|`CONSTRAINT chk_execution_merge_member_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-043|`com_execution_order_merge_member`|`CONSTRAINT chk_execution_merge_member_primary CHECK (is_primary IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-044|`com_order_change_relation`|`CONSTRAINT chk_order_change_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-045|`com_order_change_relation`|`CONSTRAINT chk_order_change_self CHECK (source_order_id <> target_order_id)`|影响异常历史数据；需逐组业务确认|
|CK-046|`com_order_contract_relation`|`CONSTRAINT chk_order_contract_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-047|`com_order_execution_relation`|`CONSTRAINT chk_order_execution_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-048|`com_order_execution_relation`|`CONSTRAINT chk_order_execution_primary CHECK (is_primary IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-049|`com_order_line_execution_relation`|`CONSTRAINT chk_order_line_execution_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-050|`com_project_contract_relation`|`CONSTRAINT chk_project_contract_dates CHECK (effective_to IS NULL OR effective_from IS NULL OR effective_to >= effective_from)`|影响异常历史数据；需逐组业务确认|
|CK-051|`com_project_contract_relation`|`CONSTRAINT chk_project_contract_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-052|`com_sales_order`|`CONSTRAINT chk_sales_order_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-053|`com_sales_order_line`|`CONSTRAINT chk_sales_order_line_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-054|`com_shipment_contract_reference`|`CONSTRAINT chk_shipment_contract_ref_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-055|`com_shipment_package`|`CONSTRAINT chk_shipment_package_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-056|`com_shipment_package`|`CONSTRAINT chk_shipment_package_warranty_dates CHECK ( warranty_end_time IS NULL OR warranty_start_time IS NULL OR warranty_end_time >= warranty_start_time )`|影响异常历史数据；需逐组业务确认|
|CK-057|`cus_customer`|`CONSTRAINT chk_customer_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-058|`cus_customer_contact`|`CONSTRAINT chk_customer_contact_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-059|`cus_customer_contact`|`CONSTRAINT chk_customer_contact_primary CHECK (is_primary IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-060|`cus_market_relation`|`CONSTRAINT chk_market_relation_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-061|`cut_cutover_support_history`|`CONSTRAINT chk_cutover_support_history_sequence CHECK (history_no > 0)`|影响异常历史数据；需逐组业务确认|
|CK-062|`cut_cutover_support_responsibility_interval`|`CONSTRAINT chk_cutover_responsibility_dates CHECK (effective_to IS NULL OR effective_to >= effective_from)`|影响异常历史数据；需逐组业务确认|
|CK-063|`cut_cutover_support_responsibility_interval`|`CONSTRAINT chk_cutover_responsibility_interval_sequence CHECK (interval_no > 0)`|影响异常历史数据；需逐组业务确认|
|CK-064|`cut_cutover_support_task`|`CONSTRAINT chk_cutover_support_window CHECK (window_end >= window_start)`|影响异常历史数据；需逐组业务确认|
|CK-065|`imp_configuration_collection_parse_attempt`|`CONSTRAINT chk_configuration_parse_attempt_no CHECK (attempt_no > 0)`|影响异常历史数据；需逐组业务确认|
|CK-066|`imp_configuration_collection_parse_attempt`|`CONSTRAINT chk_configuration_parse_attempt_time CHECK (completed_time IS NULL OR completed_time >= started_time)`|影响异常历史数据；需逐组业务确认|
|CK-067|`imp_configuration_component_candidate`|`CONSTRAINT chk_configuration_component_candidate_no CHECK (candidate_no > 0)`|影响异常历史数据；需逐组业务确认|
|CK-068|`plt_business_document`|`CONSTRAINT chk_business_document_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-069|`plt_directory_sync_snapshot`|`CONSTRAINT chk_directory_sync_times CHECK (synced_time >= source_updated_time)`|影响异常历史数据；需逐组业务确认|
|CK-070|`plt_document_version`|`CONSTRAINT chk_document_version_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-071|`plt_external_key_mapping`|`CONSTRAINT chk_external_key_target_sequence CHECK (target_sequence >= 0)`|影响异常历史数据；需逐组业务确认|
|CK-072|`plt_migration_source_record`|`CONSTRAINT chk_migration_source_target_count CHECK (mapped_target_count >= 0)`|影响异常历史数据；需逐组业务确认|
|CK-073|`plt_sync_batch`|`CONSTRAINT chk_sync_batch_count CHECK (success_count + failure_count <= read_count)`|影响异常历史数据；需逐组业务确认|
|CK-074|`plt_sync_batch`|`CONSTRAINT chk_sync_batch_time CHECK (finished_time IS NULL OR finished_time >= started_time)`|影响异常历史数据；需逐组业务确认|
|CK-075|`proj_project`|`CONSTRAINT chk_project_code_namespace CHECK ( (project_sequence = 0 AND code_root_id = id) OR project_sequence > 0 )`|影响异常历史数据；需逐组业务确认|
|CK-076|`proj_project`|`CONSTRAINT chk_project_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-077|`proj_project`|`CONSTRAINT chk_project_depth CHECK (tree_depth >= 0)`|影响异常历史数据；需逐组业务确认|
|CK-078|`proj_project_company_department_relation`|`CONSTRAINT chk_project_company_department_dates CHECK (effective_to IS NULL OR effective_from IS NULL OR effective_to >= effective_from)`|影响异常历史数据；需逐组业务确认|
|CK-079|`proj_project_company_department_relation`|`CONSTRAINT chk_project_company_department_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-080|`proj_project_company_department_relation`|`CONSTRAINT chk_project_company_department_pair CHECK (department_id IS NULL OR department_code IS NOT NULL)`|影响异常历史数据；需逐组业务确认|
|CK-081|`proj_project_company_department_relation`|`CONSTRAINT chk_project_company_department_primary CHECK (is_primary IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-082|`proj_project_member_assignment`|`CONSTRAINT chk_project_member_dates CHECK (effective_to IS NULL OR effective_from IS NULL OR effective_to >= effective_from)`|影响异常历史数据；需逐组业务确认|
|CK-083|`proj_project_member_assignment`|`CONSTRAINT chk_project_member_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-084|`proj_project_party`|`CONSTRAINT chk_project_party_dates CHECK (effective_to IS NULL OR effective_from IS NULL OR effective_to >= effective_from)`|影响异常历史数据；需逐组业务确认|
|CK-085|`proj_project_party`|`CONSTRAINT chk_project_party_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-086|`proj_project_portfolio`|`CONSTRAINT chk_portfolio_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-087|`proj_project_portfolio_member`|`CONSTRAINT chk_portfolio_project_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-088|`proj_project_relation`|`CONSTRAINT chk_project_relation_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-089|`proj_project_relation`|`CONSTRAINT chk_project_relation_self CHECK (source_project_id <> target_project_id)`|影响异常历史数据；需逐组业务确认|
|CK-090|`srv_historical_time_record`|`CONSTRAINT chk_historical_time_dates CHECK (record_end_time IS NULL OR record_start_time IS NULL OR record_end_time >= record_start_time)`|影响异常历史数据；需逐组业务确认|
|CK-091|`srv_historical_work_order`|`CONSTRAINT chk_historical_work_order_dates CHECK (source_completed_time IS NULL OR source_created_time IS NULL OR source_completed_time >= source_created_time)`|影响异常历史数据；需逐组业务确认|
|CK-092|`srv_service_incident`|`CONSTRAINT chk_service_incident_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
|CK-093|`srv_service_incident`|`CONSTRAINT chk_service_incident_times CHECK ( restored_time IS NULL OR occurred_time IS NULL OR restored_time >= occurred_time )`|影响异常历史数据；需逐组业务确认|
|CK-094|`srv_service_incident_device_relation`|`CONSTRAINT chk_incident_device_deleted CHECK (deleted IN (0, 1))`|影响异常历史数据；需逐组业务确认|
