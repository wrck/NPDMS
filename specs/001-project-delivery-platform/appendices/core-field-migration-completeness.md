...
> 以上DDL验证是2026-08-05历史快照。当前核心迁移子集已扩展为66表、1,382列、52个同域外键、103个CHECK及130项候选普通索引，哈希为`6B203BF3...275B`；ADR-0028保留历史决策锚点，ADR-0030精确覆盖六表差量，4张V3技术公告治理表及跨领域物理外键仍被排除。当前子集已在隔离MySQL 8.4.10完整执行，但因DDL和item集合变化，旧独立GO已失效，模型基线保持`MODEL_BASELINE_REVIEW_PENDING`。P3-E09不定义迁移批准哈希；只有Release包含历史迁移或数据切换时，才另行启用`AI-MIG-000`真实批次门禁并绑定批准窗口，普通功能Release不适用。
