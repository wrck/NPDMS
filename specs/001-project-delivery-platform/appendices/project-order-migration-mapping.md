...
> 本节记录2026-08-05的历史52表验证。当前核心迁移子集为66表、1,382列、489项DDL约束/索引，已在隔离MySQL 8.4.10完整执行；ADR-0028保留历史决策锚点，ADR-0030精确覆盖六表差量。正式独立复审已GO、模型基线已发布为`MODEL_BASELINE_READY`，可用于SDS/Feature模型输入。只有Release包含历史迁移或数据切换时，才由`AI-MIG-000`以真实批次范围、水位、程序、校验、对账、回退和批准窗口进行阻断；普通功能Release为`NOT_APPLICABLE`。本门禁不要求三类Owner逐项签署或非空批准哈希。其中`cus_market_relation`、`cus_customer`和`proj_project`均禁止`relation_id`，4张V3技术公告治理表不进入本子集；Q08的130项仍只是候选索引。
