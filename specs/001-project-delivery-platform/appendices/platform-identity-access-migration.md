...
本文记录已经确认的身份、组织、部门、账号、菜单、数据范围和外部人员项目授权规则，作为后续物理表设计、迁移脚本、权限接口、前端工作台和验收测试的共同依据。
...
2. 公司直接作为组织，不再创建与公司重复的组织主档。
3. 所属公司和租户是不同边界：租户是技术隔离边界，公司组织是业务归属和授权边界。
4. 部门基础数据全平台共享，部门编码使用同一套编码体系。
5. 不建立长期维护的“组织—部门”主数据关系，也不得从部门反推所属公司。
6. 内部关联使用目标表ID；工号、组织编码、部门编码和岗位编码作为正式业务编码，用于集成、检索、导入导出和问题对账。

### 2.2 系统账号与员工目录

1. 系统账号是平台登录、权限、项目成员、任务、审批和审计的主体，所有正式业务关系使用`system_users.id`。
2. EHR员工是组织人员全集，只作为只读集成人员目录，服务于查找组织内人员和创建内部系统账号时按工号带入信息。
...
- `fnd_user_info`没有公司外键，不能通过`fnd_company`或共享部门编码推导所属组织。
...
7. 内部人员按共享部门获得项目范围时，还必须满足项目所属组织处于用户显式组织关系和当前组织上下文内；部门范围不能跨公司组织单独放行。

## 10. 仍待形成明细的事项

以下内容尚未确认到可直接实施的明细，不得写入迁移脚本的默认推断：

1. 外部账号接入哪个公司组织，以及是否允许一个外部账号服务多个公司组织。
