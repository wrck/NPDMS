...
| 当前目标DDL SHA-256 | `3CDDE2E206EE4AE401ECC398EA01A6F44FFAD37AC2644478DCD42202330D58BC` |
| 现有证据记录的DDL SHA-256 | `2B206992BA5580E776060F9D4ED177A7BD8C34DB614FD65EC9560DAF38F8BF33` |
| 现有证据记录的目标表/字段 | 52表 / 1076字段 |
| 现有证据记录的外键/CHECK | 79 / 81 |
| 核心旧表/字段 | 18表 / 326字段，旧证据记录未映射0 |
| 全部物理证据 | 3931行 / 3908个唯一旧字段（待按当前DDL重建） |
| 语义证据 | 197行 / 108个唯一数据元 |
| 迁移校验 | 现有`migration-validation.json.passed=true`已过期，不代表当前工作树通过 |

`【P0阻断】`当前DDL与目标字段目录、完整矩阵、摘要和校验结果已经失配。任何后续AI必须先完成`AI-MIG-000`，重建全部制品并使当前DDL哈希一致；在此之前不得声称“52表DDL已验证”“字段映射完整”或开始生产迁移。
