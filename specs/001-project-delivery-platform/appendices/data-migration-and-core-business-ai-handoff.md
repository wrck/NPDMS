...
`【P0阻断】`当前DDL、目标字段目录、完整矩阵、摘要和校验结果已使用同一当前哈希，但Q07/Q08尚未裁决，整体模型仍未取得Reviewer签署和非空`approvedDdlSha256`。任何后续AI必须继续完成`AI-MIG-000`剩余逐项裁决；在此之前不得把历史“52表DDL已验证”升级为当前50表核心子集放行证据，也不得把该子集冒充平台全量模型或开始生产迁移。
