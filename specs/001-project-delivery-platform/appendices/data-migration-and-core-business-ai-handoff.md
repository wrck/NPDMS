...
| 当前目标DDL SHA-256 | `AA0344713CBDE21B22C91ED9A1B77631905055C650294A69CC81D302890079A9` |
| 现有证据记录的DDL SHA-256 | `2B206992BA5580E776060F9D4ED177A7BD8C34DB614FD65EC9560DAF38F8BF33` |
| 当前核心迁移表/字段 | 49表 / 1048字段；不是平台全量模型 |
| 当前约束/表选项 | 380 / 49；其中同域外键46、CHECK 79 |
...
`【P0阻断】`当前DDL、目标字段目录、完整矩阵、摘要和校验结果已使用同一当前哈希，但整体模型仍未取得Reviewer签署和非空`approvedDdlSha256`。任何后续AI必须继续完成`AI-MIG-000`剩余逐项裁决；在此之前不得把历史“52表DDL已验证”升级为当前49表核心子集放行证据，也不得把该子集冒充平台全量模型或开始生产迁移。
