...
- 租户是技术隔离边界；公司是业务主体和授权边界，两者不得混用。
- 公司统一使用`company_id/company_code/company_name`，部门统一使用`department_id/department_code/department_name`。
- 部门主数据全平台共享，不建立可从部门反推公司的全局从属关系；项目归属、用户任职和权限范围需要公司—部门配对时，必须在同一业务关系行保存二者。
- 目标表、字段、DTO、OpenAPI Schema、查询参数和导出字段禁止使用`org_*`或`organization*`表示公司；旧来源原字段名和原始载荷除外。
- 高频引用的`*_code/*_name`默认保存业务发生时值，主档变更不自动回写；需要当前值时按`*_id`显式查询，明确执行回归刷新时必须留审计和对账记录。
- 客户单位内部部门使用`customer_department_*`，承运商使用`carrier_*`，不得与平台公司或内部共享部门混淆。
