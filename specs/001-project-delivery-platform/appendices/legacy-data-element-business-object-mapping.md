...
- 正式设计证据：`project-order-physical-schema.mysql.sql`当前包含49张核心迁移评审表和1,048个带中文描述的字段；ADR-0021新增`cus_market_relation`并明确客户/项目不保存`relation_id`，ADR-0022移除4张V3治理表和跨领域物理外键。该清单不是平台全量模型；18张核心旧表326个字段有数据库画像，全部活动结构另有3,931条物理字段证据处置，但仍不是生产迁移。
