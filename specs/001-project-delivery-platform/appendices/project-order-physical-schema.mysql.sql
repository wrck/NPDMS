...
--           on 2026-08-05; 52 tables, 965 commented columns,
--           76 tenant-safe foreign keys and 80 CHECK constraints.
...
CREATE TABLE pms_customer (
    id BIGINT NOT NULL COMMENT '全局唯一主键，由基础平台分布式ID生成器赋值',
    tenant_id BIGINT NOT NULL COMMENT '数据所属租户ID，用于租户级隔离',
    ...
    status VARCHAR(32) NOT NULL DEFAULT 'ENABLED' COMMENT '记录当前业务状态，取值由对应业务状态字典约束',
    version INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '乐观锁版本号，每次业务更新递增',
    creator VARCHAR(64) NOT NULL DEFAULT '' COMMENT '创建该记录的系统用户工号或服务账号编码',
    create_time DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '记录写入本系统的时间，采用系统统一时区',
    updater VARCHAR(64) NOT NULL DEFAULT '' COMMENT '最后更新该记录的系统用户工号或服务账号编码',
    update_time DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '记录最后写入本系统的时间，采用系统统一时区',
    deleted TINYINT NOT NULL DEFAULT 0 COMMENT '逻辑删除标志：0有效，1已删除',
    PRIMARY KEY (id),
...
CREATE TABLE pms_customer_contact (
    ...
    department_name VARCHAR(255) NULL COMMENT '客户联系人的部门名称',
...
CREATE TABLE pms_project (
    ...
    org_id BIGINT NULL COMMENT '项目当前主责公司组织ID；多组织角色以项目组织关系为准',
    ...
    status VARCHAR(32) NOT NULL COMMENT '记录当前业务状态，取值由对应业务状态字典约束',
    version INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '乐观锁版本号，每次业务更新递增',
    creator VARCHAR(64) NOT NULL DEFAULT '' COMMENT '创建该记录的系统用户工号或服务账号编码',
    create_time DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '记录写入本系统的时间，采用系统统一时区',
    updater VARCHAR(64) NOT NULL DEFAULT '' COMMENT '最后更新该记录的系统用户工号或服务账号编码',
...
CREATE TABLE pms_project_relation (
    ...
    status VARCHAR(32) NOT NULL DEFAULT 'ACTIVE' COMMENT '记录当前业务状态，取值由对应业务状态字典约束',
    version INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '乐观锁版本号，每次业务更新递增',
    creator VARCHAR(64) NOT NULL DEFAULT '' COMMENT '创建该记录的系统用户工号或服务账号编码',
    create_time DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '记录写入本系统的时间，采用系统统一时区',
    updater VARCHAR(64) NOT NULL DEFAULT '' COMMENT '最后更新该记录的系统用户工号或服务账号编码',
...
CREATE TABLE pms_project_org_rel (
    id BIGINT NOT NULL COMMENT '全局唯一主键，由基础平台分布式ID生成器赋值',
    tenant_id BIGINT NOT NULL COMMENT '数据所属租户ID，用于租户级隔离',
    project_id BIGINT NOT NULL COMMENT '关联项目记录的全局唯一ID',
    org_code VARCHAR(64) NOT NULL COMMENT '项目组织关系的组织编码',
    org_role VARCHAR(32) NOT NULL COMMENT '组织角色编码，取值由对应业务字典约束',
    is_primary TINYINT NOT NULL DEFAULT 0 COMMENT '同一业务范围内是否为主记录：0否，1是',
    effective_from DATETIME(3) NULL COMMENT '业务关系或事实开始生效的时间，空值表示来源未提供',
    effective_to DATETIME(3) NULL COMMENT '业务关系或事实失效的时间，空值表示当前仍有效',
    status VARCHAR(32) NOT NULL DEFAULT 'ACTIVE' COMMENT '记录当前业务状态，取值由对应业务状态字典约束',
    ...
    PRIMARY KEY (id),
    UNIQUE KEY uk_project_org_rel_tenant_row (tenant_id, id),
    UNIQUE KEY uk_project_org_role (
        tenant_id, project_id, org_code, org_role, effective_from
    ),
    KEY idx_project_org_reverse (tenant_id, org_code, org_role, status, project_id),
    UNIQUE KEY uk_project_primary_org (tenant_id, primary_project_id, org_role),
    CONSTRAINT fk_project_org_project
        FOREIGN KEY (tenant_id, project_id) REFERENCES pms_project (tenant_id, id),
    CONSTRAINT chk_project_org_primary CHECK (is_primary IN (0, 1)),
    CONSTRAINT chk_project_org_dates
        CHECK (effective_to IS NULL OR effective_from IS NULL OR effective_to >= effective_from),
    CONSTRAINT chk_project_org_deleted CHECK (deleted IN (0, 1))
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_0900_ai_ci
  COMMENT = '项目与公司组织的多角色时态关系';
...
CREATE TABLE pms_business_document (
    ...
    status VARCHAR(32) NOT NULL DEFAULT 'DRAFT' COMMENT '记录当前业务状态，取值由对应业务状态字典约束',
    version INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '乐观锁版本号，每次业务更新递增',
    creator VARCHAR(64) NOT NULL DEFAULT '' COMMENT '创建该记录的系统用户工号或服务账号编码',
    create_time DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '记录写入本系统的时间，采用系统统一时区',
    updater VARCHAR(64) NOT NULL DEFAULT '' COMMENT '最后更新该记录的系统用户工号或服务账号编码',
...
CREATE TABLE pms_document_version (
    ...
    status VARCHAR(32) NOT NULL DEFAULT 'VALID' COMMENT '记录当前业务状态，取值由对应业务状态字典约束',
    creator VARCHAR(64) NOT NULL DEFAULT '' COMMENT '创建该记录的系统用户工号或服务账号编码',
    create_time DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '记录写入本系统的时间，采用系统统一时区',
    updater VARCHAR(64) NOT NULL DEFAULT '' COMMENT '最后更新该记录的系统用户工号或服务账号编码',
    update_time DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '记录最后写入本系统的时间，采用系统统一时区',
...
CREATE TABLE pms_project_deliverable (
    ...
    status VARCHAR(32) NOT NULL DEFAULT 'PENDING' COMMENT '记录当前业务状态，取值由对应业务状态字典约束',
    version INT UNSIGNED NOT NULL DEFAULT 0 COMMENT '乐观锁版本号，每次业务更新递增',
    creator VARCHAR(64) NOT NULL DEFAULT '' COMMENT '创建该记录的系统用户工号或服务账号编码',
    create_time DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3) COMMENT '记录写入本系统的时间，采用系统统一时区',
    updater VARCHAR(64) NOT NULL DEFAULT '' COMMENT '最后更新该记录的系统用户工号或服务账号编码',
...
CREATE TABLE pms_contract (
    id BIGINT NOT NULL COMMENT '全局唯一主键，由基础平台分布式ID生成器赋值',
    tenant_id BIGINT NOT NULL COMMENT '数据所属租户ID，用于租户级隔离',
    company_code VARCHAR(32) NOT NULL COMMENT '合同主档的公司编码',
...
CREATE TABLE pms_contract_receivable (
    ...
    company_code VARCHAR(32) NULL COMMENT '合同回款的公司编码',
    ...
    marketing_department_name VARCHAR(128) NULL COMMENT '合同回款的市场部门名称',
    office_code VARCHAR(80) NULL COMMENT '合同回款的办事处编码',
    office_name VARCHAR(128) NULL COMMENT '合同回款的办事处名称',
    ...
    market_code VARCHAR(80) NULL COMMENT '合同回款的市场编码',
    system_org_code VARCHAR(64) NULL COMMENT '合同回款的系统组织编码',
    industry_code VARCHAR(64) NULL COMMENT '合同回款的行业编码',
    expand_org_code VARCHAR(64) NULL COMMENT '合同回款的拓展组织编码',
    ...
    original_system_org_code VARCHAR(64) NULL COMMENT '合同回款的原始系统组织编码',
    original_expand_org_code VARCHAR(64) NULL COMMENT '合同回款的原始拓展组织编码',
...
CREATE TABLE pms_shipment_contract_ref (
    ...
    company_code VARCHAR(32) NULL COMMENT '发货合同归属的公司编码',
    office_code VARCHAR(32) NULL COMMENT '发货合同归属的办事处编码',
    contract_type VARCHAR(32) NULL COMMENT '合同类型编码，取值由对应业务字典约束',
    customer_name VARCHAR(512) NULL COMMENT '发货合同归属的客户名称',
    project_name VARCHAR(512) NULL COMMENT '发货合同归属的项目名称',
    market_code VARCHAR(32) NULL COMMENT '发货合同归属的市场编码',
    market_name VARCHAR(128) NULL COMMENT '发货合同归属的市场名称',
    system_id BIGINT NULL COMMENT '关联系统记录的全局唯一ID',
    system_name VARCHAR(128) NULL COMMENT '发货合同归属的系统名称',
...
CREATE TABLE pms_shipment_package (
    ...
    express_company VARCHAR(256) NULL COMMENT '发货装箱单的快递公司',
    mapping_status VARCHAR(32) NOT NULL DEFAULT 'PENDING_MAPPING' COMMENT '跨系统关联解析状态，如待映射、已映射或存在冲突',
    source_system VARCHAR(32) NOT NULL COMMENT '来源系统编码，用于同步幂等和数据血缘追踪',
    source_record_key VARCHAR(128) NOT NULL COMMENT '来源记录稳定唯一键，用于幂等写入和回溯',
    source_sync_time DATETIME(3) NULL COMMENT '来源记录最近一次成功同步到本系统的时间',
...
CREATE TABLE pms_sales_order (
    ...
    tenant_id BIGINT NOT NULL COMMENT '数据所属租户ID，用于租户级隔离',
    source_system VARCHAR(32) NOT NULL COMMENT '来源系统编码，用于同步幂等和数据血缘追踪',
    company_code VARCHAR(32) NOT NULL COMMENT 'ERP销售订单的公司编码',
    ...
    customer_id BIGINT NULL COMMENT '关联客户记录的全局唯一ID',
    customer_code VARCHAR(64) NULL COMMENT 'ERP销售订单的客户编码',
...
CREATE TABLE pms_device_sn (
    ...
    company_barcode VARCHAR(100) NULL COMMENT '设备SN主档的公司条码',
    ...
    KEY idx_device_company_barcode (tenant_id, company_barcode),
...
CREATE TABLE pms_crm_execution_order (
    ...
    market_code VARCHAR(64) NULL COMMENT 'CRM执行单镜像的市场编码',
    market_name VARCHAR(255) NULL COMMENT 'CRM执行单镜像的市场名称',
    system_org_code VARCHAR(64) NULL COMMENT 'CRM执行单镜像的系统组织编码',
    system_org_name VARCHAR(255) NULL COMMENT 'CRM执行单镜像的系统组织名称',
    expand_org_code VARCHAR(64) NULL COMMENT 'CRM执行单镜像的拓展组织编码',
    expand_org_name VARCHAR(255) NULL COMMENT 'CRM执行单镜像的拓展组织名称',
    industry_code VARCHAR(64) NULL COMMENT 'CRM执行单镜像的行业编码',
    industry_name VARCHAR(255) NULL COMMENT 'CRM执行单镜像的行业名称',
    office_code VARCHAR(1024) NULL COMMENT 'CRM执行单镜像的办事处编码',
    office_name VARCHAR(128) NULL COMMENT 'CRM执行单镜像的办事处名称',
    ...
    company_code VARCHAR(64) NULL COMMENT 'CRM执行单镜像的公司编码',
    ...
    KEY idx_crm_execution_org (
        tenant_id, company_code, office_code(191), status
    ),
...
CREATE TABLE pms_crm_execution_config (
    ...
    company_code VARCHAR(64) NULL COMMENT 'CRM执行单配置的公司编码',
