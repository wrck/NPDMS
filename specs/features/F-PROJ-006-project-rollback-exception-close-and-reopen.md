...
- BPM审批、PM-11项目任务、CUT割接、采集与INS巡检等提供方通过公开只读守卫接口返回活动阻断。任一必需提供方缺失、异常、超时或返回未知状态时按阻断处理。
...
- `AC-FPROJ006-004`：存在未关闭后代、审批、项目任务、割接、采集、巡检或未知/超时守卫时返回最小阻断明细；守卫后树版本或任一提供方事实版本/水位变化时提交返回`VERSION_CONFLICT`，Project、快照、事件和幂等成功事实均不变化。
