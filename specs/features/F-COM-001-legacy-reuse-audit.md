...
| 状态机 | 当前范围状态仅`ACTIVE/RELEASED/CONFLICT`，订单行数量状态仅`CONFIRMED/PENDING_AUTHORITY` | `DIRECT_REUSE_AND_COMPLETE` | 保留封闭值；补来源变化触发与冲突处置规则，不从旧tinyint或文本猜测状态 |
...
- DeliveryScope：必须同时解析项目、订单行和正数分配量；缺任一项保持待核对且不进入指标或当前范围。
