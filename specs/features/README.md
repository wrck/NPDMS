...
| [F-SOL-003](F-SOL-003-requirement-analysis-versioning.md) | 需求分析动态表单与版本冻结 | PRE-04（V1） | IN_REVIEW | PENDING（原Feature/计划已取消；基于已完成F-PLT-002按方案A重新送Feature Ready） | REPLAN_REQUIRED（保留现有代码，待新Feature Ready GO后生成全新中文Technical Plan） |
...
| [F-PLT-002](F-PLT-002-shared-dynamic-form-template-and-instance-foundation.md) | 共享动态表单模板与实例基础能力 | SOL-01（主）、PRE-04/PM-03/PM-11（支撑） | IN_REVIEW（业务实例API聚焦修订） | PENDING（原基础闭环Feature Ready GO保留；本次与F-SOL-003联合复审） | IMPLEMENTATION_COMPLETE（NPDMS `0ce68d04`；`NPDMS-FPLT002-IMPLEMENTATION-20260827-02-R2`） |
