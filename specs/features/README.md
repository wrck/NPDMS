...
| [F-AST-002](F-AST-002-device-product-type-copy-and-public-query.md) | 设备产品类型受控副本与公开查询 | EQP-01（PARTIAL，V1） | BASELINE | READY（`NPDMS-FAST002-FEATURE-READY-20260830-01`） | IMPLEMENTATION_IN_PROGRESS（Technical Plan `NPDMS-FAST002-TECHPLAN-20260830-01`；Task 1复审`NPDMS-FAST002-TASK1-REMEDIATION-REVIEW-20260830-01`；下一Task 2；不含CRM/MES连接器） |
...
| [F-INS-001](F-INS-001-inspection-rule-version-and-field-configuration-foundation.md) | 巡检规则版本与字段配置基础 | INS-03（PARTIAL）、INS-09（FULL）（V2） | BASELINE | READY（`NPDMS-FINS001-FEATURE-READY-20260830-01`） | TECHNICAL_PLAN_READY（`NPDMS-FINS001-TECHPLAN-20260830-01`；F-AST-002实施依赖阻断发布/选择/Done） |
