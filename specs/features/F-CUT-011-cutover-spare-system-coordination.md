# F-CUT-011 割接备件系统协同 Feature Spec
...
> 旧实现复用审计：`specs/features/F-CUT-011-legacy-reuse-audit.md`
...
- 用户REST、回调合同、WireLong/时间、错误Envelope和恢复动作见`F-CUT-011-api-contract.json`。
- 三张CUT Owner表、JSON结构、唯一键、可空联合、版本和迁移边界见`F-CUT-011-physical-contract.json`。
