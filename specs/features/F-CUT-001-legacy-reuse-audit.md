...
| `pms_cut_risk`及cut-risk页面/API | 运行任务下的风险记录 | 不可复用为统一采集项主数据 | 保持不变；不得反推风险定义或绑定规则 |
...
- F-CUT-001必须新建配置领域类、接口、页面和前向表，不能增强旧task/risk/plan类后改变旧行为。
...
现有实现保持不变是正确的。F-CUT-001只复用平台通用技术模式，业务领域对象和UI新增；参考附件不建立第二事实源，方案模板章节使用配置根结构化版本快照。
