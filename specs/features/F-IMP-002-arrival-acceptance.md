...
- COM已分配范围非空但明确SN并集为空时，AST设备事实对该数量范围不适用：IMP消费编排层不调用AST空集合Query，冻结空`devices`与空设备归属水位。混合范围仅解析非空明确SN并集。这不是Provider fallback或空范围放行：COM空范围、范围结构损坏、应有明确SN却跳过AST仍失败关闭。
