...
> 历史创建范围Implementation Done：`PASS`（NPDMS `1c76050`；2026-08-25回归独立GO）；当前新增量状态仅由Feature Task维护
...
> 差量依据：`CHG-PRD-2026-09-08-019`、修订018及尚未关闭的修订017义务；项目级多经理/主责模型已确认，Q-FPROJ-010候选业务裁决已关闭；通用查询及成员写入分别按当前合同实施；旧Task事实保留
...
修订019替代原首次-only及固定任务方案。成员指派与调整通过PROJ项目级命令办理，可同次指派服务经理与多名项目经理，也可先指派服务经理后单独增补项目经理；各业务阶段按对应权限和数据范围操作，不依赖T-ASSIGN-PM或已有经理。每位项目经理保留独立责任区间，并保留一个当前主责，主责引用必须指向有效经理成员；切换主责不自动删除其他成员。同次联合操作全有或全无，历史区间、审批和审计不覆盖；指派不自动完成任务或推进阶段。同一项目角色操作权限相同，主责不附加权限，不使用单manager_id代表角色资格；候选按Q-FPROJ-010采用同公司、可跨部门及上游指定角色资格。
...
| 项目级成员指派/调整（PM-01@V1，候选采用SDS10通用公司/角色查询） | 对应操作权限、项目范围、合法人员及版本/幂等；不限定S0或首次 | 联合/单独维护多经理成员与当前主责，保存责任历史并重算双主责状态 | 不把单个manager_id当完整名单；不自动完成固定任务或删除其他经理；不以前置V2批量功能阻断 |
...
| 指派/调整项目经理成员与主责 | 有对应指派权限的服务经理或工程管理部授权人员 | Project版本、项目范围、SYSTEM有效人员、成员/主责一致性及幂等；同角色同权限；候选公司/角色资格按SDS10通用查询重验 | 不依赖固定任务；不因已有项目经理或不处于S0而拒绝合法调整；不外推未批准的组织限制 |
...
原`AssignInitialProjectManager`、单个`projectManagerUserId`且禁止调整的尚未实施合同已由修订019替代，不作为当前编码依据。新合同必须承载多名PROJECT_MANAGER成员、一个当前主责及联合/单独操作，并定义增补、调整、主责切换的精确意图；同一请求使用If-Match和Idempotency-Key，联合操作同事务完成或回滚。业务范围和Q-FPROJ-010已明确：候选为同公司、允许跨部门的有效在职人员，由SDS10通用公司/角色资格查询提供；候选与提交重验使用同一查询，不能查询现有项目成员替代上游资格。集合写命令仍须补齐类型化请求/响应、锁序及消费者合同，不从查询合同已定稿推定整个成员闭环已实现。
...
无固定指派任务的项目中，可同次指派服务经理和多名项目经理，也可服务经理已指派后单独增补项目经理；各业务阶段按对应权限调整，保留一个当前有效主责和各成员责任历史。主责切换不默认移除其他经理；同一人员同角色不产生重叠区间，重放不重复业务事实，联合指派失败全回滚。只有两类主责齐全才为ASSIGNED；成员操作不自动完成任务或推进阶段，通知失败不回滚已提交责任。主责与其他有效经理的同角色操作权限相同，实际权限测试须覆盖两者；候选正反向用例按已关闭Q-FPROJ-010覆盖同公司/指定角色、跨部门和无效组合；未实际实现和验证不得勾选本AC。
...
| API Contract | `/projects`、模板查询/详情、既有服务经理API及SDS10通用公司/角色用户查询与项目级成员/主责API的错误与版本语义 |
...
当前修订019：`REVALIDATION_REQUIRED`；Q-FPROJ-010业务裁决已关闭。SDS10通用公司/角色查询合同已明确，可在有效DU内先实施该只读增量；完整成员写入及其消费者合同仍须按影响落位，不自动恢复整体Feature Ready。以下表格及BASELINE/READY结论是原创建范围的历史DoR，不授权当前成员管理增量。
