...
- 调整预览须携带当前范围ID与期望分配版本，确认目标归属后排除自身占用计算可用量；新增预览仍拒绝同项目同订单行已有当前范围。减量预览沿用ACC绑定守卫，不能因预览放行跳过实际调整的锁定重验。
...
### 2026-09-17 合同来源事实与唯一主档汇总

需求方批准局部扩展：补齐合同来源事实表，并由ERP主档汇总来源信息。此扩展不包含回款业务操作、财务统计、历史生产迁移或ERP连接器；上文历史排除范围不再排除本次获准的建表及只读汇总。

- 前向创建 `com_contract_receivable`、`com_shipment_contract_reference`、`com_shipment_package`；字段、来源唯一键及同COM外键继承核心迁移目标DDL。`proj_project_party`按PROJ既定模型独立创建，不被COM读取或推断成合同关系。
- `com_contract`仍是唯一合同主档，按租户、所属公司、合同编号唯一，只有ERP生成。来源表没有对应ERP主档时保留原记录，不创建主档，也不因合同号相同跨公司归并。
- `com_contract_summary`为只读数据库视图，一条主档返回一行；主档已有字段优先，回款/发货仅补缺。补充候选存在不同非空值时不任选一条，不用项目名称冒充合同名称、不用币种名称冒充币种编码。
- 同一个补充字段在来源中只有一个不同非空值时才填充；冲突保留来源明细并由 `source_conflict` 标记。公司未知、显式contract_id与公司/合同号指向不一致时不参与汇总。
- 来源金额和装箱单按事实展示，不对回款重复快照或装箱单重复记录求和。汇总明细按完整业务内容去重，来源ID、系统、稳定键和同步时间另外保留为来源引用，不删除来源证据。数组顺序不构成契约。
- 视图既不覆盖ERP主档，也不回写来源表；应用接入仍须使用既有COM服务端租户与公司/项目数据范围。当前增量仅交付数据库层，不宣称API/UI已接入。
