...
- 草稿答案四个精确键均可为`null`，暂存不推进状态；每次暂存只读刷新DRAFT上下文，允许客户等级从`NOT_CONFIGURED`变为`AVAILABLE`，也允许保存IMP结构化`NOT_READY`快照及未满足项。提交必须具备全部非空答案、当前`AVAILABLE`客户等级、权威重验为`READY`和人工最终等级A/B/C/D，并保存评估版本、上下文快照、提交人和时间。
...
CUT通过`ImplementationReadinessApi.inspect/lockAndRevalidate`消费IMP；通过`ProjectScopeApi.ACTION_EDIT`消费“本人参与、负责或明确授权项目”范围；通过`ProjectCutoverContextFactApi.inspect/lockAndRevalidate`消费同一项目主档行的项目、发生时客户及部门（办事处）快照。CUT写命令把前次`FOUND`完整Fact原样作为Expected交给PROJ，Owner锁行后逐字段比较；任一字段变化均返回版本冲突，Expected只作并发守卫，CUT只冻结锁后`currentFact`。项目/客户/部门编码最长64字符，名称最长255字符且不得截断。通过AST物理Owner支撑Task `T-FIMP001-AST-01`的`DeviceScopeFactApi`消费`deviceId/currentProjectId/projectAssignmentVersion`；通过F-AST-002的`AssetProductTypeApi.getAuthorizedDeviceProductType`取得已授权设备的`productTypeCode/enabled/sourceVersion/resolutionStatus/syncStatus`。CUT只预留消费端口，不实现跨模块Owner；新任务冻结公开合同可证明的编码和来源版本，禁止从产品编码、型号、CONP或客户端输入推断。生产原子重验能力须在集成Gate关闭；`src/test`受控替身可推进CUT正向闭环。通过CUS `CustomerServiceLevelFactApi.inspectCurrent/lockAndRevalidate`消费当前有效客户服务等级事实。CUT禁止依赖其他Context的Service、Mapper、DO或业务表。

## 5. 数据与迁移边界

- 新建CUT Owner前向表：`cut_task`、`cut_task_device_scope`、`cut_task_stage_history`、`cut_assessment`。其中聚合根和评估表名严格沿用正式SDS，不另建同义表。
- `cut_task`保存来源判别联合、不可变`previous_task_id`、背景、当前阶段/状态、负责人、IMP快照、PROJ水位、项目/办事处/设备/客户/就绪上下文快照，以及任务创建时冻结且不可改的配置revision三元组；`cut_task_device_scope`保存稳定设备ID、SN、归属版本、产品类型编码/来源版本快照和活动唯一标记；`cut_task_stage_history`只追加P1接入与P2提交产生的阶段迁移；`cut_assessment`保存DRAFT/SUBMITTED/INVALIDATED版本、服务端固定模板、草稿可空/提交非空答案JSON、上下文JSON和人工等级。已提交版本失效时同事务追加下一版DRAFT并原子切换当前评估引用，不留下无current marker状态。
...
- CURRENT_FORWARD批次身份由CUT服务端固定为`ownerContextCode=CUT`、`purposeCode=CUTOVER_TASK_CURRENT_FORWARD`、`sourceSystem=NPDMS_LEGACY`、`sourceTable=pms_cut_task`；受信入口只接收`tenantId/correlationId`，不得由调用方覆盖批次身份。CUT只通过PLT公开API领取最早`STAGED_READY`批次，并在写目标、追加mapping/issue及完成核对的同一外层事务内复核该身份；暂时失败整体回滚后重领，不新增CUT迁移幂等表。
