...
修订019协作边界：服务经理可与项目经理同次指派，也可分次办理，各业务阶段按对应权限调整；本Feature保留PM-08候选、层级、主责/协同及历史规则。项目经理为多成员集合并保留一个当前主责，以下双主责判断中的项目经理指有效当前主责，不代表项目只能有一个经理。完整名单及同角色同权限由F-PROJ-001承接；Q-FPROJ-010只待确认候选范围，原单经理测试或历史GO不证明新联合指派已实现。
