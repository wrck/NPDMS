...
- PM-01@V1项目级多经理成员/主责管理实现（由F-PROJ-001承接）；本Feature只消费其有效责任事实重算双主责状态，不把固定PM-11任务或PM-09@V2作为单项目成员操作前置；
...
修订019协作边界：服务经理可与项目经理同次指派，也可分次办理，各业务阶段按对应权限调整；本Feature保留PM-08候选、层级、主责/协同及历史规则。项目经理为多成员集合并保留一个当前主责，以下双主责判断中的项目经理指有效当前主责，不代表项目只能有一个经理。完整名单及同角色同权限由F-PROJ-001承接；Q-FPROJ-010已关闭候选范围；通用查询不静默改变PM-08原规则，原单经理测试或历史GO不证明新联合指派已实现。
...
- `AC-FPROJ005-010`：不宣称PM-08 V2、由F-PROJ-001承接的项目级多经理/主责管理、PM-05/06/09、Deployment、SIT、UAT或Release完成。
