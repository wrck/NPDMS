...
- WorkBinding自动匹配、项目模板绑定、ProjectTask工作台适配或任务完成判定；
- PRE-04十一项核心内容、项目经理业务权限、需求分析完成版本、历史对比、SCH-01输入引用或既有F-SOL-003代码调整；
...
- 已发布修订不提供UPDATE/DELETE业务入口。实例不建立状态机或历史表；保存通过version CAS，后续领域版本化不得反向覆盖实例冻结修订。
