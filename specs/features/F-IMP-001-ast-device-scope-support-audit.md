...
- `lockAndRevalidate(DeviceScopeRevalidationQuery)`输入完整期望设备项和调用方专用`ExpectedScopeWatermark`，按稳定`deviceId`顺序锁定AST当前投影并校验集合完全相等。完整集合仍有效但归属版本变化返回`STALE`和当前完整事实；同`deviceId`当前Owner SN与冻结SN按同一比较键不一致时抛`OWNER_DATA_CORRUPTED`；缺失、状态不可用或错项目返回`INVALID`及稳定排序逐项原因，不返回部分事实。调用方Query/期望水位非法归为`INVALID_REQUEST`，Provider事实、逐项结果或结果组合损坏归为`OWNER_DATA_CORRUPTED`；Provider不可用使用AST公共异常，不得泄漏IMP内部异常。
