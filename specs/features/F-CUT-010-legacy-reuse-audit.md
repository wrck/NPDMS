...
F-CUT-010必须作为CUT-08完整纵向协同Feature，在现有任务工作台和P5详情上增加CUT自有引用/快照/证据闭环。旧CUT与工程物料实现均不能升级为外部备件权威事实；跨模块功能只保留接口并以测试替身验证正常链。
